/*
 * generated by Xtext 2.33.0
 */
parser grammar InternalGitHubActionsParser;

options {
	tokenVocab=InternalGitHubActionsLexer;
	superClass=AbstractInternalAntlrParser;
	backtrack=true;
}

@header {
package d.fe.up.pt.cicd.gha.dsl.parser.antlr.internal;

import org.eclipse.xtext.*;
import org.eclipse.xtext.parser.*;
import org.eclipse.xtext.parser.impl.*;
import org.eclipse.emf.ecore.util.EcoreUtil;
import org.eclipse.emf.ecore.EObject;
import org.eclipse.emf.common.util.Enumerator;
import org.eclipse.xtext.parser.antlr.AbstractInternalAntlrParser;
import org.eclipse.xtext.parser.antlr.XtextTokenStream;
import org.eclipse.xtext.parser.antlr.XtextTokenStream.HiddenTokens;
import org.eclipse.xtext.parser.antlr.IUnorderedGroupHelper.UnorderedGroupState;
import org.eclipse.xtext.parser.antlr.AntlrDatatypeRuleToken;
import d.fe.up.pt.cicd.gha.dsl.services.GitHubActionsGrammarAccess;

}

@members {

/*
  This grammar contains a lot of empty actions to work around a bug in ANTLR.
  Otherwise the ANTLR tool will create synpreds that cannot be compiled in some rare cases.
*/

 	private GitHubActionsGrammarAccess grammarAccess;

    public InternalGitHubActionsParser(TokenStream input, GitHubActionsGrammarAccess grammarAccess) {
        this(input);
        this.grammarAccess = grammarAccess;
        registerRules(grammarAccess.getGrammar());
    }

    @Override
    protected String getFirstRuleName() {
    	return "Workflow";
   	}

   	@Override
   	protected GitHubActionsGrammarAccess getGrammarAccess() {
   		return grammarAccess;
   	}

}

@rulecatch {
    catch (RecognitionException re) {
        recover(input,re);
        appendSkippedTokens();
    }
}

// Entry rule entryRuleWorkflow
entryRuleWorkflow returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getWorkflowAccess().getUnorderedGroup_0()
	);
}:
	{ newCompositeNode(grammarAccess.getWorkflowRule()); }
	iv_ruleWorkflow=ruleWorkflow
	{ $current=$iv_ruleWorkflow.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Workflow
ruleWorkflow returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getWorkflowAccess().getUnorderedGroup_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			(
				{ 
				  getUnorderedGroupHelper().enter(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
				}
				(
					(
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 0)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 0);
					}
								({true}?=>(otherlv_1=Name
								{
									newLeafNode(otherlv_1, grammarAccess.getWorkflowAccess().getNameKeyword_0_0_0());
								}
								otherlv_2=Colon
								{
									newLeafNode(otherlv_2, grammarAccess.getWorkflowAccess().getColonKeyword_0_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getNameExpressionParserRuleCall_0_0_2_0());
										}
										lv_name_3_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											set(
												$current,
												"name",
												lv_name_3_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								this_NEWLINE_4=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_4, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_0_3());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 1)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 1);
					}
								({true}?=>(otherlv_5=RunName
								{
									newLeafNode(otherlv_5, grammarAccess.getWorkflowAccess().getRunNameKeyword_0_1_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getWorkflowAccess().getColonKeyword_0_1_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getRunNameExpressionParserRuleCall_0_1_2_0());
										}
										lv_runName_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											set(
												$current,
												"runName",
												lv_runName_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								this_NEWLINE_8=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_8, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_1_3());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 2)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 2);
					}
								({true}?=>(otherlv_9=On
								{
									newLeafNode(otherlv_9, grammarAccess.getWorkflowAccess().getOnKeyword_0_2_0());
								}
								otherlv_10=Colon
								{
									newLeafNode(otherlv_10, grammarAccess.getWorkflowAccess().getColonKeyword_0_2_1());
								}
								(
									(
										(
											(
												{
													newCompositeNode(grammarAccess.getWorkflowAccess().getTriggersSimpleTriggerParserRuleCall_0_2_2_0_0_0());
												}
												lv_triggers_11_0=ruleSimpleTrigger
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getWorkflowRule());
													}
													add(
														$current,
														"triggers",
														lv_triggers_11_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.SimpleTrigger");
													afterParserOrEnumRuleCall();
												}
											)
										)
										this_NEWLINE_12=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_12, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_2_2_0_1());
										}
									)
									    |
									(
										otherlv_13=LeftSquareBracket
										{
											newLeafNode(otherlv_13, grammarAccess.getWorkflowAccess().getLeftSquareBracketKeyword_0_2_2_1_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getWorkflowAccess().getTriggersSimpleTriggerParserRuleCall_0_2_2_1_1_0());
												}
												lv_triggers_14_0=ruleSimpleTrigger
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getWorkflowRule());
													}
													add(
														$current,
														"triggers",
														lv_triggers_14_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.SimpleTrigger");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											otherlv_15=Comma
											{
												newLeafNode(otherlv_15, grammarAccess.getWorkflowAccess().getCommaKeyword_0_2_2_1_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getWorkflowAccess().getTriggersSimpleTriggerParserRuleCall_0_2_2_1_2_1_0());
													}
													lv_triggers_16_0=ruleSimpleTrigger
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getWorkflowRule());
														}
														add(
															$current,
															"triggers",
															lv_triggers_16_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.SimpleTrigger");
														afterParserOrEnumRuleCall();
													}
												)
											)
										)*
										otherlv_17=RightSquareBracket
										{
											newLeafNode(otherlv_17, grammarAccess.getWorkflowAccess().getRightSquareBracketKeyword_0_2_2_1_3());
										}
										this_NEWLINE_18=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_18, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_2_2_1_4());
										}
									)
									    |
									(
										this_NEWLINE_19=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_19, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_2_2_2_0());
										}
										this_BEGIN_20=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_20, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_2_2_2_1());
										}
										(
											otherlv_21=HyphenMinus
											{
												newLeafNode(otherlv_21, grammarAccess.getWorkflowAccess().getHyphenMinusKeyword_0_2_2_2_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getWorkflowAccess().getTriggersSimpleTriggerParserRuleCall_0_2_2_2_2_1_0());
													}
													lv_triggers_22_0=ruleSimpleTrigger
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getWorkflowRule());
														}
														add(
															$current,
															"triggers",
															lv_triggers_22_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.SimpleTrigger");
														afterParserOrEnumRuleCall();
													}
												)
											)
											this_NEWLINE_23=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_23, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_2_2_2_2_2());
											}
										)+
										this_END_24=RULE_END
										{
											newLeafNode(this_END_24, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_2_2_2_3());
										}
									)
									    |
									(
										(
											{
												newCompositeNode(grammarAccess.getWorkflowAccess().getTriggersOptionedTriggerParserRuleCall_0_2_2_3_0());
											}
											lv_triggers_25_0=ruleOptionedTrigger
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getWorkflowRule());
												}
												add(
													$current,
													"triggers",
													lv_triggers_25_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.OptionedTrigger");
												afterParserOrEnumRuleCall();
											}
										)
									)+
								)
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 3)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 3);
					}
								({true}?=>(otherlv_26=Permissions
								{
									newLeafNode(otherlv_26, grammarAccess.getWorkflowAccess().getPermissionsKeyword_0_3_0());
								}
								otherlv_27=Colon
								{
									newLeafNode(otherlv_27, grammarAccess.getWorkflowAccess().getColonKeyword_0_3_1());
								}
								this_NEWLINE_28=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_28, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_3_2());
								}
								this_BEGIN_29=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_29, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_3_3());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getPermissionsPermissionParserRuleCall_0_3_4_0());
										}
										lv_permissions_30_0=rulePermission
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											add(
												$current,
												"permissions",
												lv_permissions_30_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Permission");
											afterParserOrEnumRuleCall();
										}
									)
								)+
								this_END_31=RULE_END
								{
									newLeafNode(this_END_31, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_3_5());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 4)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 4);
					}
								({true}?=>(otherlv_32=Env
								{
									newLeafNode(otherlv_32, grammarAccess.getWorkflowAccess().getEnvKeyword_0_4_0());
								}
								otherlv_33=Colon
								{
									newLeafNode(otherlv_33, grammarAccess.getWorkflowAccess().getColonKeyword_0_4_1());
								}
								this_NEWLINE_34=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_34, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_4_2());
								}
								this_BEGIN_35=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_35, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_4_3());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_0_4_4_0());
										}
										lv_environmentVariables_36_0=ruleVariableAssignment
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											add(
												$current,
												"environmentVariables",
												lv_environmentVariables_36_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
											afterParserOrEnumRuleCall();
										}
									)
								)+
								this_END_37=RULE_END
								{
									newLeafNode(this_END_37, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_4_5());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 5)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 5);
					}
								({true}?=>(otherlv_38=Defaults
								{
									newLeafNode(otherlv_38, grammarAccess.getWorkflowAccess().getDefaultsKeyword_0_5_0());
								}
								otherlv_39=Colon
								{
									newLeafNode(otherlv_39, grammarAccess.getWorkflowAccess().getColonKeyword_0_5_1());
								}
								this_NEWLINE_40=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_40, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_5_2());
								}
								this_BEGIN_41=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_41, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_5_3());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getDefaultsDefaultsParserRuleCall_0_5_4_0());
										}
										lv_defaults_42_0=ruleDefaults
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											set(
												$current,
												"defaults",
												lv_defaults_42_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Defaults");
											afterParserOrEnumRuleCall();
										}
									)
								)
								this_END_43=RULE_END
								{
									newLeafNode(this_END_43, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_5_5());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 6)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 6);
					}
								({true}?=>(otherlv_44=Concurrency
								{
									newLeafNode(otherlv_44, grammarAccess.getWorkflowAccess().getConcurrencyKeyword_0_6_0());
								}
								otherlv_45=Colon
								{
									newLeafNode(otherlv_45, grammarAccess.getWorkflowAccess().getColonKeyword_0_6_1());
								}
								this_NEWLINE_46=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_46, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_6_2());
								}
								this_BEGIN_47=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_47, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_6_3());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getConcurrencyGroupConcurrencyGroupParserRuleCall_0_6_4_0());
										}
										lv_concurrencyGroup_48_0=ruleConcurrencyGroup
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											set(
												$current,
												"concurrencyGroup",
												lv_concurrencyGroup_48_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.ConcurrencyGroup");
											afterParserOrEnumRuleCall();
										}
									)
								)
								this_END_49=RULE_END
								{
									newLeafNode(this_END_49, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_6_5());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 7)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getWorkflowAccess().getUnorderedGroup_0(), 7);
					}
								({true}?=>(otherlv_50=Jobs
								{
									newLeafNode(otherlv_50, grammarAccess.getWorkflowAccess().getJobsKeyword_0_7_0());
								}
								otherlv_51=Colon
								{
									newLeafNode(otherlv_51, grammarAccess.getWorkflowAccess().getColonKeyword_0_7_1());
								}
								this_NEWLINE_52=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_52, grammarAccess.getWorkflowAccess().getNEWLINETerminalRuleCall_0_7_2());
								}
								this_BEGIN_53=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_53, grammarAccess.getWorkflowAccess().getBEGINTerminalRuleCall_0_7_3());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getWorkflowAccess().getJobsJobParserRuleCall_0_7_4_0());
										}
										lv_jobs_54_0=ruleJob
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getWorkflowRule());
											}
											add(
												$current,
												"jobs",
												lv_jobs_54_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Job");
											afterParserOrEnumRuleCall();
										}
									)
								)+
								this_END_55=RULE_END
								{
									newLeafNode(this_END_55, grammarAccess.getWorkflowAccess().getENDTerminalRuleCall_0_7_5());
								}
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
					}
				)
			)
					)+
					{getUnorderedGroupHelper().canLeave(grammarAccess.getWorkflowAccess().getUnorderedGroup_0())}?
				)
			)
				{ 
				  getUnorderedGroupHelper().leave(grammarAccess.getWorkflowAccess().getUnorderedGroup_0());
				}
		)
		(
			(RULE_END_OF_FILE)=>
			this_END_OF_FILE_56=RULE_END_OF_FILE
			{
				newLeafNode(this_END_OF_FILE_56, grammarAccess.getWorkflowAccess().getEND_OF_FILETerminalRuleCall_1());
			}
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleJob
entryRuleJob returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getJobRule()); }
	iv_ruleJob=ruleJob
	{ $current=$iv_ruleJob.current; }
	EOF;

// Rule Job
ruleJob returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getJobAccess().getScriptJobParserRuleCall_0());
		}
		this_ScriptJob_0=ruleScriptJob
		{
			$current = $this_ScriptJob_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getJobAccess().getReuseWorkflowJobParserRuleCall_1());
		}
		this_ReuseWorkflowJob_1=ruleReuseWorkflowJob
		{
			$current = $this_ReuseWorkflowJob_1.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleScriptJob
entryRuleScriptJob returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getScriptJobAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getScriptJobRule()); }
	iv_ruleScriptJob=ruleScriptJob
	{ $current=$iv_ruleScriptJob.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule ScriptJob
ruleScriptJob returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getScriptJobAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getScriptJobAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 0);
				}
							({true}?=>((
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getNameYAMLIDParserRuleCall_0_0_0());
									}
									lv_name_1_0=ruleYAMLID
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"name",
											lv_name_1_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
										afterParserOrEnumRuleCall();
									}
								)
							)
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getScriptJobAccess().getColonKeyword_0_1());
							}
							(
								this_NEWLINE_3=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_3, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_0_2());
								}
							)+
							this_BEGIN_4=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_4, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_0_3());
							}
							(
								otherlv_5=Name
								{
									newLeafNode(otherlv_5, grammarAccess.getScriptJobAccess().getNameKeyword_0_4_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getScriptJobAccess().getColonKeyword_0_4_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getScriptJobAccess().getJobNameExpressionParserRuleCall_0_4_2_0());
										}
										lv_jobName_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getScriptJobRule());
											}
											set(
												$current,
												"jobName",
												lv_jobName_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_8=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_8, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_0_4_3());
									}
								)+
							)?
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 1);
				}
							({true}?=>(otherlv_9=Permissions
							{
								newLeafNode(otherlv_9, grammarAccess.getScriptJobAccess().getPermissionsKeyword_1_0());
							}
							otherlv_10=Colon
							{
								newLeafNode(otherlv_10, grammarAccess.getScriptJobAccess().getColonKeyword_1_1());
							}
							(
								this_NEWLINE_11=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_11, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_1_2());
								}
							)+
							this_BEGIN_12=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_12, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_1_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getPermissionsPermissionParserRuleCall_1_4_0());
									}
									lv_permissions_13_0=rulePermission
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										add(
											$current,
											"permissions",
											lv_permissions_13_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Permission");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_14=RULE_END
							{
								newLeafNode(this_END_14, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_1_5());
							}
							(
								this_NEWLINE_15=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_15, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_1_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 2);
				}
							({true}?=>((
								otherlv_16=Needs
								{
									newLeafNode(otherlv_16, grammarAccess.getScriptJobAccess().getNeedsKeyword_2_0_0());
								}
								otherlv_17=Colon
								{
									newLeafNode(otherlv_17, grammarAccess.getScriptJobAccess().getColonKeyword_2_0_1());
								}
								(
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getScriptJobRule());
												}
											}
											otherlv_18=RULE_ID
											{
												newLeafNode(otherlv_18, grammarAccess.getScriptJobAccess().getDependsOnJobCrossReference_2_0_2_0_0());
											}
										)
									)
									(
										this_NEWLINE_19=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_19, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_2_0_2_1());
										}
									)+
								)
							)
							    |
							(
								otherlv_20=LeftSquareBracket
								{
									newLeafNode(otherlv_20, grammarAccess.getScriptJobAccess().getLeftSquareBracketKeyword_2_1_0());
								}
								(
									(
										{
											/* */
										}
										{
											if ($current==null) {
												$current = createModelElement(grammarAccess.getScriptJobRule());
											}
										}
										otherlv_21=RULE_ID
										{
											newLeafNode(otherlv_21, grammarAccess.getScriptJobAccess().getDependsOnJobCrossReference_2_1_1_0());
										}
									)
								)
								(
									otherlv_22=Comma
									{
										newLeafNode(otherlv_22, grammarAccess.getScriptJobAccess().getCommaKeyword_2_1_2_0());
									}
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getScriptJobRule());
												}
											}
											otherlv_23=RULE_ID
											{
												newLeafNode(otherlv_23, grammarAccess.getScriptJobAccess().getDependsOnJobCrossReference_2_1_2_1_0());
											}
										)
									)
								)*
								otherlv_24=RightSquareBracket
								{
									newLeafNode(otherlv_24, grammarAccess.getScriptJobAccess().getRightSquareBracketKeyword_2_1_3());
								}
								(
									this_NEWLINE_25=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_25, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_2_1_4());
									}
								)+
							)
							    |
							(
								(
									this_NEWLINE_26=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_26, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_2_2_0());
									}
								)+
								this_BEGIN_27=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_27, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_2_2_1());
								}
								(
									otherlv_28=HyphenMinus
									{
										newLeafNode(otherlv_28, grammarAccess.getScriptJobAccess().getHyphenMinusKeyword_2_2_2_0());
									}
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getScriptJobRule());
												}
											}
											otherlv_29=RULE_ID
											{
												newLeafNode(otherlv_29, grammarAccess.getScriptJobAccess().getDependsOnJobCrossReference_2_2_2_1_0());
											}
										)
									)
									(
										this_NEWLINE_30=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_30, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_2_2_2_2());
										}
									)+
								)+
								this_END_31=RULE_END
								{
									newLeafNode(this_END_31, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_2_2_3());
								}
								(
									this_NEWLINE_32=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_32, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_2_2_4());
									}
								)*
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 3);
				}
							({true}?=>((
								{
									newCompositeNode(grammarAccess.getScriptJobAccess().getIfConditionIfStatementParserRuleCall_3_0());
								}
								lv_ifCondition_33_0=ruleIfStatement
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getScriptJobRule());
									}
									set(
										$current,
										"ifCondition",
										lv_ifCondition_33_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.IfStatement");
									afterParserOrEnumRuleCall();
								}
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 4)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 4);
				}
							({true}?=>(otherlv_34=RunsOn
							{
								newLeafNode(otherlv_34, grammarAccess.getScriptJobAccess().getRunsOnKeyword_4_0());
							}
							otherlv_35=Colon
							{
								newLeafNode(otherlv_35, grammarAccess.getScriptJobAccess().getColonKeyword_4_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getAgentAgentParserRuleCall_4_2_0());
									}
									lv_agent_36_0=ruleAgent
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"agent",
											lv_agent_36_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Agent");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 5)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 5);
				}
							({true}?=>(otherlv_37=Environment
							{
								newLeafNode(otherlv_37, grammarAccess.getScriptJobAccess().getEnvironmentKeyword_5_0());
							}
							otherlv_38=Colon
							{
								newLeafNode(otherlv_38, grammarAccess.getScriptJobAccess().getColonKeyword_5_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getStagingEnvironmentStagingEnvironmentParserRuleCall_5_2_0());
									}
									lv_stagingEnvironment_39_0=ruleStagingEnvironment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"stagingEnvironment",
											lv_stagingEnvironment_39_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.StagingEnvironment");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 6)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 6);
				}
							({true}?=>(otherlv_40=Concurrency
							{
								newLeafNode(otherlv_40, grammarAccess.getScriptJobAccess().getConcurrencyKeyword_6_0());
							}
							otherlv_41=Colon
							{
								newLeafNode(otherlv_41, grammarAccess.getScriptJobAccess().getColonKeyword_6_1());
							}
							(
								this_NEWLINE_42=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_42, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_6_2());
								}
							)+
							this_BEGIN_43=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_43, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_6_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getConcurrencyGroupConcurrencyGroupParserRuleCall_6_4_0());
									}
									lv_concurrencyGroup_44_0=ruleConcurrencyGroup
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"concurrencyGroup",
											lv_concurrencyGroup_44_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.ConcurrencyGroup");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_45=RULE_END
							{
								newLeafNode(this_END_45, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_6_5());
							}
							(
								this_NEWLINE_46=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_46, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_6_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 7)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 7);
				}
							({true}?=>(otherlv_47=Env
							{
								newLeafNode(otherlv_47, grammarAccess.getScriptJobAccess().getEnvKeyword_7_0());
							}
							otherlv_48=Colon
							{
								newLeafNode(otherlv_48, grammarAccess.getScriptJobAccess().getColonKeyword_7_1());
							}
							(
								this_NEWLINE_49=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_49, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_7_2());
								}
							)+
							this_BEGIN_50=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_50, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_7_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_7_4_0());
									}
									lv_environmentVariables_51_0=ruleVariableAssignment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										add(
											$current,
											"environmentVariables",
											lv_environmentVariables_51_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_52=RULE_END
							{
								newLeafNode(this_END_52, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_7_5());
							}
							(
								this_NEWLINE_53=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_53, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_7_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 8)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 8);
				}
							({true}?=>(otherlv_54=Defaults
							{
								newLeafNode(otherlv_54, grammarAccess.getScriptJobAccess().getDefaultsKeyword_8_0());
							}
							otherlv_55=Colon
							{
								newLeafNode(otherlv_55, grammarAccess.getScriptJobAccess().getColonKeyword_8_1());
							}
							(
								this_NEWLINE_56=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_56, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_8_2());
								}
							)+
							this_BEGIN_57=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_57, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_8_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getDefaultsDefaultsParserRuleCall_8_4_0());
									}
									lv_defaults_58_0=ruleDefaults
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"defaults",
											lv_defaults_58_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Defaults");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_59=RULE_END
							{
								newLeafNode(this_END_59, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_8_5());
							}
							(
								this_NEWLINE_60=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_60, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_8_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 9)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 9);
				}
							({true}?=>(otherlv_61=Steps
							{
								newLeafNode(otherlv_61, grammarAccess.getScriptJobAccess().getStepsKeyword_9_0());
							}
							otherlv_62=Colon
							{
								newLeafNode(otherlv_62, grammarAccess.getScriptJobAccess().getColonKeyword_9_1());
							}
							(
								this_NEWLINE_63=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_63, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_9_2());
								}
							)+
							this_BEGIN_64=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_64, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_9_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getStepsStepParserRuleCall_9_4_0());
									}
									lv_steps_65_0=ruleStep
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										add(
											$current,
											"steps",
											lv_steps_65_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Step");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_66=RULE_END
							{
								newLeafNode(this_END_66, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_9_5());
							}
							(
								this_NEWLINE_67=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_67, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_9_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 10)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 10);
				}
							({true}?=>(otherlv_68=Strategy
							{
								newLeafNode(otherlv_68, grammarAccess.getScriptJobAccess().getStrategyKeyword_10_0());
							}
							otherlv_69=Colon
							{
								newLeafNode(otherlv_69, grammarAccess.getScriptJobAccess().getColonKeyword_10_1());
							}
							(
								this_NEWLINE_70=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_70, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_10_2());
								}
							)+
							this_BEGIN_71=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_71, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_10_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getStrategyStrategyParserRuleCall_10_4_0());
									}
									lv_strategy_72_0=ruleStrategy
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"strategy",
											lv_strategy_72_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Strategy");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_73=RULE_END
							{
								newLeafNode(this_END_73, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_10_5());
							}
							(
								this_NEWLINE_74=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_74, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_10_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 11)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 11);
				}
							({true}?=>(otherlv_75=Container
							{
								newLeafNode(otherlv_75, grammarAccess.getScriptJobAccess().getContainerKeyword_11_0());
							}
							otherlv_76=Colon
							{
								newLeafNode(otherlv_76, grammarAccess.getScriptJobAccess().getColonKeyword_11_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getContainerContainerParserRuleCall_11_2_0());
									}
									lv_container_77_0=ruleContainer
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"container",
											lv_container_77_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Container");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 12)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 12);
				}
							({true}?=>(otherlv_78=Services
							{
								newLeafNode(otherlv_78, grammarAccess.getScriptJobAccess().getServicesKeyword_12_0());
							}
							otherlv_79=Colon
							{
								newLeafNode(otherlv_79, grammarAccess.getScriptJobAccess().getColonKeyword_12_1());
							}
							(
								this_NEWLINE_80=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_80, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_12_2());
								}
							)+
							this_BEGIN_81=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_81, grammarAccess.getScriptJobAccess().getBEGINTerminalRuleCall_12_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getServicesServiceParserRuleCall_12_4_0());
									}
									lv_services_82_0=ruleService
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										add(
											$current,
											"services",
											lv_services_82_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Service");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_83=RULE_END
							{
								newLeafNode(this_END_83, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_12_5());
							}
							(
								this_NEWLINE_84=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_84, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_12_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 13)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 13);
				}
							({true}?=>(otherlv_85=ContinueOnError
							{
								newLeafNode(otherlv_85, grammarAccess.getScriptJobAccess().getContinueOnErrorKeyword_13_0());
							}
							otherlv_86=Colon
							{
								newLeafNode(otherlv_86, grammarAccess.getScriptJobAccess().getColonKeyword_13_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getScriptJobAccess().getContinueOnErrorExpressionParserRuleCall_13_2_0());
									}
									lv_continueOnError_87_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getScriptJobRule());
										}
										set(
											$current,
											"continueOnError",
											lv_continueOnError_87_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_88=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_88, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_13_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 14)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getScriptJobAccess().getUnorderedGroup(), 14);
				}
							({true}?=>((
								otherlv_89=TimeoutMinutes
								{
									newLeafNode(otherlv_89, grammarAccess.getScriptJobAccess().getTimeoutMinutesKeyword_14_0_0());
								}
								otherlv_90=Colon
								{
									newLeafNode(otherlv_90, grammarAccess.getScriptJobAccess().getColonKeyword_14_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getScriptJobAccess().getTimeoutMinutesExpressionParserRuleCall_14_0_2_0());
										}
										lv_timeoutMinutes_91_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getScriptJobRule());
											}
											set(
												$current,
												"timeoutMinutes",
												lv_timeoutMinutes_91_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_92=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_92, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_14_0_3());
									}
								)+
							)?
							this_END_93=RULE_END
							{
								newLeafNode(this_END_93, grammarAccess.getScriptJobAccess().getENDTerminalRuleCall_14_1());
							}
							(
								this_NEWLINE_94=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_94, grammarAccess.getScriptJobAccess().getNEWLINETerminalRuleCall_14_2());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getScriptJobAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getScriptJobAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getScriptJobAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleReuseWorkflowJob
entryRuleReuseWorkflowJob returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getReuseWorkflowJobRule()); }
	iv_ruleReuseWorkflowJob=ruleReuseWorkflowJob
	{ $current=$iv_ruleReuseWorkflowJob.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule ReuseWorkflowJob
ruleReuseWorkflowJob returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 0);
				}
							({true}?=>((
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getNameYAMLIDParserRuleCall_0_0_0());
									}
									lv_name_1_0=ruleYAMLID
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"name",
											lv_name_1_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
										afterParserOrEnumRuleCall();
									}
								)
							)
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_0_1());
							}
							(
								this_NEWLINE_3=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_3, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_0_2());
								}
							)+
							this_BEGIN_4=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_4, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_0_3());
							}
							(
								otherlv_5=Name
								{
									newLeafNode(otherlv_5, grammarAccess.getReuseWorkflowJobAccess().getNameKeyword_0_4_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_0_4_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getJobNameExpressionParserRuleCall_0_4_2_0());
										}
										lv_jobName_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
											}
											set(
												$current,
												"jobName",
												lv_jobName_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_8=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_8, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_0_4_3());
									}
								)+
							)?
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 1);
				}
							({true}?=>(otherlv_9=Permissions
							{
								newLeafNode(otherlv_9, grammarAccess.getReuseWorkflowJobAccess().getPermissionsKeyword_1_0());
							}
							otherlv_10=Colon
							{
								newLeafNode(otherlv_10, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_1_1());
							}
							(
								this_NEWLINE_11=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_11, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_1_2());
								}
							)+
							this_BEGIN_12=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_12, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_1_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getPermissionsPermissionParserRuleCall_1_4_0());
									}
									lv_permissions_13_0=rulePermission
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										add(
											$current,
											"permissions",
											lv_permissions_13_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Permission");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_14=RULE_END
							{
								newLeafNode(this_END_14, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_1_5());
							}
							(
								this_NEWLINE_15=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_15, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_1_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 2);
				}
							({true}?=>((
								otherlv_16=Needs
								{
									newLeafNode(otherlv_16, grammarAccess.getReuseWorkflowJobAccess().getNeedsKeyword_2_0_0());
								}
								otherlv_17=Colon
								{
									newLeafNode(otherlv_17, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_2_0_1());
								}
								(
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getReuseWorkflowJobRule());
												}
											}
											otherlv_18=RULE_ID
											{
												newLeafNode(otherlv_18, grammarAccess.getReuseWorkflowJobAccess().getDependsOnJobCrossReference_2_0_2_0_0());
											}
										)
									)
									(
										this_NEWLINE_19=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_19, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_2_0_2_1());
										}
									)+
								)
							)
							    |
							(
								otherlv_20=LeftSquareBracket
								{
									newLeafNode(otherlv_20, grammarAccess.getReuseWorkflowJobAccess().getLeftSquareBracketKeyword_2_1_0());
								}
								(
									(
										{
											/* */
										}
										{
											if ($current==null) {
												$current = createModelElement(grammarAccess.getReuseWorkflowJobRule());
											}
										}
										otherlv_21=RULE_ID
										{
											newLeafNode(otherlv_21, grammarAccess.getReuseWorkflowJobAccess().getDependsOnJobCrossReference_2_1_1_0());
										}
									)
								)
								(
									otherlv_22=Comma
									{
										newLeafNode(otherlv_22, grammarAccess.getReuseWorkflowJobAccess().getCommaKeyword_2_1_2_0());
									}
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getReuseWorkflowJobRule());
												}
											}
											otherlv_23=RULE_ID
											{
												newLeafNode(otherlv_23, grammarAccess.getReuseWorkflowJobAccess().getDependsOnJobCrossReference_2_1_2_1_0());
											}
										)
									)
								)*
								otherlv_24=RightSquareBracket
								{
									newLeafNode(otherlv_24, grammarAccess.getReuseWorkflowJobAccess().getRightSquareBracketKeyword_2_1_3());
								}
								(
									this_NEWLINE_25=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_25, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_2_1_4());
									}
								)+
							)
							    |
							(
								(
									this_NEWLINE_26=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_26, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_2_2_0());
									}
								)+
								this_BEGIN_27=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_27, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_2_2_1());
								}
								(
									otherlv_28=HyphenMinus
									{
										newLeafNode(otherlv_28, grammarAccess.getReuseWorkflowJobAccess().getHyphenMinusKeyword_2_2_2_0());
									}
									(
										(
											{
												/* */
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getReuseWorkflowJobRule());
												}
											}
											otherlv_29=RULE_ID
											{
												newLeafNode(otherlv_29, grammarAccess.getReuseWorkflowJobAccess().getDependsOnJobCrossReference_2_2_2_1_0());
											}
										)
									)
									(
										this_NEWLINE_30=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_30, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_2_2_2_2());
										}
									)+
								)+
								this_END_31=RULE_END
								{
									newLeafNode(this_END_31, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_2_2_3());
								}
								(
									this_NEWLINE_32=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_32, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_2_2_4());
									}
								)*
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 3);
				}
							({true}?=>((
								{
									newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getIfConditionIfStatementParserRuleCall_3_0());
								}
								lv_ifCondition_33_0=ruleIfStatement
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
									}
									set(
										$current,
										"ifCondition",
										lv_ifCondition_33_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.IfStatement");
									afterParserOrEnumRuleCall();
								}
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 4)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 4);
				}
							({true}?=>(otherlv_34=RunsOn
							{
								newLeafNode(otherlv_34, grammarAccess.getReuseWorkflowJobAccess().getRunsOnKeyword_4_0());
							}
							otherlv_35=Colon
							{
								newLeafNode(otherlv_35, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_4_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getAgentAgentParserRuleCall_4_2_0());
									}
									lv_agent_36_0=ruleAgent
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"agent",
											lv_agent_36_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Agent");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 5)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 5);
				}
							({true}?=>(otherlv_37=Environment
							{
								newLeafNode(otherlv_37, grammarAccess.getReuseWorkflowJobAccess().getEnvironmentKeyword_5_0());
							}
							otherlv_38=Colon
							{
								newLeafNode(otherlv_38, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_5_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getStagingEnvironmentStagingEnvironmentParserRuleCall_5_2_0());
									}
									lv_stagingEnvironment_39_0=ruleStagingEnvironment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"stagingEnvironment",
											lv_stagingEnvironment_39_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.StagingEnvironment");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 6)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 6);
				}
							({true}?=>(otherlv_40=Concurrency
							{
								newLeafNode(otherlv_40, grammarAccess.getReuseWorkflowJobAccess().getConcurrencyKeyword_6_0());
							}
							otherlv_41=Colon
							{
								newLeafNode(otherlv_41, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_6_1());
							}
							(
								this_NEWLINE_42=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_42, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_6_2());
								}
							)+
							this_BEGIN_43=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_43, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_6_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getConcurrencyGroupConcurrencyGroupParserRuleCall_6_4_0());
									}
									lv_concurrencyGroup_44_0=ruleConcurrencyGroup
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"concurrencyGroup",
											lv_concurrencyGroup_44_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.ConcurrencyGroup");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_45=RULE_END
							{
								newLeafNode(this_END_45, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_6_5());
							}
							(
								this_NEWLINE_46=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_46, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_6_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 7)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 7);
				}
							({true}?=>(otherlv_47=Env
							{
								newLeafNode(otherlv_47, grammarAccess.getReuseWorkflowJobAccess().getEnvKeyword_7_0());
							}
							otherlv_48=Colon
							{
								newLeafNode(otherlv_48, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_7_1());
							}
							(
								this_NEWLINE_49=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_49, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_7_2());
								}
							)+
							this_BEGIN_50=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_50, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_7_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_7_4_0());
									}
									lv_environmentVariables_51_0=ruleVariableAssignment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										add(
											$current,
											"environmentVariables",
											lv_environmentVariables_51_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_52=RULE_END
							{
								newLeafNode(this_END_52, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_7_5());
							}
							(
								this_NEWLINE_53=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_53, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_7_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 8)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 8);
				}
							({true}?=>(otherlv_54=Defaults
							{
								newLeafNode(otherlv_54, grammarAccess.getReuseWorkflowJobAccess().getDefaultsKeyword_8_0());
							}
							otherlv_55=Colon
							{
								newLeafNode(otherlv_55, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_8_1());
							}
							(
								this_NEWLINE_56=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_56, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_8_2());
								}
							)+
							this_BEGIN_57=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_57, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_8_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getDefaultsDefaultsParserRuleCall_8_4_0());
									}
									lv_defaults_58_0=ruleDefaults
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"defaults",
											lv_defaults_58_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Defaults");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_59=RULE_END
							{
								newLeafNode(this_END_59, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_8_5());
							}
							(
								this_NEWLINE_60=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_60, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_8_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 9)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 9);
				}
							({true}?=>(otherlv_61=Strategy
							{
								newLeafNode(otherlv_61, grammarAccess.getReuseWorkflowJobAccess().getStrategyKeyword_9_0());
							}
							otherlv_62=Colon
							{
								newLeafNode(otherlv_62, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_9_1());
							}
							(
								this_NEWLINE_63=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_63, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_9_2());
								}
							)+
							this_BEGIN_64=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_64, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_9_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getStrategyStrategyParserRuleCall_9_4_0());
									}
									lv_strategy_65_0=ruleStrategy
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"strategy",
											lv_strategy_65_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Strategy");
										afterParserOrEnumRuleCall();
									}
								)
							)
							this_END_66=RULE_END
							{
								newLeafNode(this_END_66, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_9_5());
							}
							(
								this_NEWLINE_67=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_67, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_9_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 10)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 10);
				}
							({true}?=>(otherlv_68=Container
							{
								newLeafNode(otherlv_68, grammarAccess.getReuseWorkflowJobAccess().getContainerKeyword_10_0());
							}
							otherlv_69=Colon
							{
								newLeafNode(otherlv_69, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_10_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getContainerContainerParserRuleCall_10_2_0());
									}
									lv_container_70_0=ruleContainer
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"container",
											lv_container_70_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Container");
										afterParserOrEnumRuleCall();
									}
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 11)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 11);
				}
							({true}?=>(otherlv_71=Services
							{
								newLeafNode(otherlv_71, grammarAccess.getReuseWorkflowJobAccess().getServicesKeyword_11_0());
							}
							otherlv_72=Colon
							{
								newLeafNode(otherlv_72, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_11_1());
							}
							(
								this_NEWLINE_73=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_73, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_11_2());
								}
							)+
							this_BEGIN_74=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_74, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_11_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getServicesServiceParserRuleCall_11_4_0());
									}
									lv_services_75_0=ruleService
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										add(
											$current,
											"services",
											lv_services_75_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Service");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_76=RULE_END
							{
								newLeafNode(this_END_76, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_11_5());
							}
							(
								this_NEWLINE_77=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_77, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_11_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 12)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 12);
				}
							({true}?=>(otherlv_78=Uses
							{
								newLeafNode(otherlv_78, grammarAccess.getReuseWorkflowJobAccess().getUsesKeyword_12_0());
							}
							otherlv_79=Colon
							{
								newLeafNode(otherlv_79, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_12_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getWorkflowPathExpressionParserRuleCall_12_2_0());
									}
									lv_workflowPath_80_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"workflowPath",
											lv_workflowPath_80_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_81=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_81, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_12_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 13)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 13);
				}
							({true}?=>(otherlv_82=With
							{
								newLeafNode(otherlv_82, grammarAccess.getReuseWorkflowJobAccess().getWithKeyword_13_0());
							}
							otherlv_83=Colon
							{
								newLeafNode(otherlv_83, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_13_1());
							}
							(
								this_NEWLINE_84=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_84, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_13_2());
								}
							)+
							this_BEGIN_85=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_85, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_13_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getArgsVariableAssignmentParserRuleCall_13_4_0());
									}
									lv_args_86_0=ruleVariableAssignment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										add(
											$current,
											"args",
											lv_args_86_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_87=RULE_END
							{
								newLeafNode(this_END_87, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_13_5());
							}
							(
								this_NEWLINE_88=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_88, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_13_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 14)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 14);
				}
							({true}?=>(otherlv_89=Secrets
							{
								newLeafNode(otherlv_89, grammarAccess.getReuseWorkflowJobAccess().getSecretsKeyword_14_0());
							}
							otherlv_90=Colon
							{
								newLeafNode(otherlv_90, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_14_1());
							}
							(
								(
									(
										(
											lv_inheritSecrets_91_0=Inherit
											{
												newLeafNode(lv_inheritSecrets_91_0, grammarAccess.getReuseWorkflowJobAccess().getInheritSecretsInheritKeyword_14_2_0_0_0());
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getReuseWorkflowJobRule());
												}
												setWithLastConsumed($current, "inheritSecrets", lv_inheritSecrets_91_0 != null, "inherit");
											}
										)
									)
									(
										this_NEWLINE_92=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_92, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_14_2_0_1());
										}
									)+
								)
								    |
								(
									(
										this_NEWLINE_93=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_93, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_14_2_1_0());
										}
									)+
									this_BEGIN_94=RULE_BEGIN
									{
										newLeafNode(this_BEGIN_94, grammarAccess.getReuseWorkflowJobAccess().getBEGINTerminalRuleCall_14_2_1_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getSecretsVariableAssignmentParserRuleCall_14_2_1_2_0());
											}
											lv_secrets_95_0=ruleVariableAssignment
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
												}
												add(
													$current,
													"secrets",
													lv_secrets_95_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
												afterParserOrEnumRuleCall();
											}
										)
									)+
									this_END_96=RULE_END
									{
										newLeafNode(this_END_96, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_14_2_1_3());
									}
									(
										this_NEWLINE_97=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_97, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_14_2_1_4());
										}
									)*
								)
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 15)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 15);
				}
							({true}?=>(otherlv_98=ContinueOnError
							{
								newLeafNode(otherlv_98, grammarAccess.getReuseWorkflowJobAccess().getContinueOnErrorKeyword_15_0());
							}
							otherlv_99=Colon
							{
								newLeafNode(otherlv_99, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_15_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getContinueOnErrorExpressionParserRuleCall_15_2_0());
									}
									lv_continueOnError_100_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
										}
										set(
											$current,
											"continueOnError",
											lv_continueOnError_100_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_101=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_101, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_15_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 16)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup(), 16);
				}
							({true}?=>((
								otherlv_102=TimeoutMinutes
								{
									newLeafNode(otherlv_102, grammarAccess.getReuseWorkflowJobAccess().getTimeoutMinutesKeyword_16_0_0());
								}
								otherlv_103=Colon
								{
									newLeafNode(otherlv_103, grammarAccess.getReuseWorkflowJobAccess().getColonKeyword_16_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getReuseWorkflowJobAccess().getTimeoutMinutesExpressionParserRuleCall_16_0_2_0());
										}
										lv_timeoutMinutes_104_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getReuseWorkflowJobRule());
											}
											set(
												$current,
												"timeoutMinutes",
												lv_timeoutMinutes_104_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_105=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_105, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_16_0_3());
									}
								)+
							)?
							this_END_106=RULE_END
							{
								newLeafNode(this_END_106, grammarAccess.getReuseWorkflowJobAccess().getENDTerminalRuleCall_16_1());
							}
							(
								this_NEWLINE_107=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_107, grammarAccess.getReuseWorkflowJobAccess().getNEWLINETerminalRuleCall_16_2());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getReuseWorkflowJobAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleAgent
entryRuleAgent returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getAgentRule()); }
	iv_ruleAgent=ruleAgent
	{ $current=$iv_ruleAgent.current; }
	EOF;

// Rule Agent
ruleAgent returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				(
					(
						{
							newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_0_0_0_0());
						}
						lv_labels_0_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getAgentRule());
							}
							add(
								$current,
								"labels",
								lv_labels_0_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_1=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_1, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_0_0_1());
					}
				)+
			)
			    |
			(
				otherlv_2=LeftSquareBracket
				{
					newLeafNode(otherlv_2, grammarAccess.getAgentAccess().getLeftSquareBracketKeyword_0_1_0());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_0_1_1_0());
						}
						lv_labels_3_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getAgentRule());
							}
							add(
								$current,
								"labels",
								lv_labels_3_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					otherlv_4=Comma
					{
						newLeafNode(otherlv_4, grammarAccess.getAgentAccess().getCommaKeyword_0_1_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_0_1_2_1_0());
							}
							lv_labels_5_0=ruleExpression
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getAgentRule());
								}
								add(
									$current,
									"labels",
									lv_labels_5_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
								afterParserOrEnumRuleCall();
							}
						)
					)
				)*
				otherlv_6=RightSquareBracket
				{
					newLeafNode(otherlv_6, grammarAccess.getAgentAccess().getRightSquareBracketKeyword_0_1_3());
				}
				(
					this_NEWLINE_7=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_7, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_0_1_4());
					}
				)+
			)
			    |
			(
				(
					this_NEWLINE_8=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_8, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_0_2_0());
					}
				)+
				this_BEGIN_9=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_9, grammarAccess.getAgentAccess().getBEGINTerminalRuleCall_0_2_1());
				}
				(
					otherlv_10=HyphenMinus
					{
						newLeafNode(otherlv_10, grammarAccess.getAgentAccess().getHyphenMinusKeyword_0_2_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_0_2_2_1_0());
							}
							lv_labels_11_0=ruleExpression
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getAgentRule());
								}
								add(
									$current,
									"labels",
									lv_labels_11_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
								afterParserOrEnumRuleCall();
							}
						)
					)
					(
						this_NEWLINE_12=RULE_NEWLINE
						{
							newLeafNode(this_NEWLINE_12, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_0_2_2_2());
						}
					)+
				)+
				this_END_13=RULE_END
				{
					newLeafNode(this_END_13, grammarAccess.getAgentAccess().getENDTerminalRuleCall_0_2_3());
				}
				(
					this_NEWLINE_14=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_14, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_0_2_4());
					}
				)*
			)
		)
		    |
		(
			(
				(
					this_NEWLINE_15=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_15, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_0());
					}
				)+
				this_BEGIN_16=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_16, grammarAccess.getAgentAccess().getBEGINTerminalRuleCall_1_0_1());
				}
				(
					(
						(
							otherlv_17=Labels
							{
								newLeafNode(otherlv_17, grammarAccess.getAgentAccess().getLabelsKeyword_1_0_2_0_0_0());
							}
							otherlv_18=Colon
							{
								newLeafNode(otherlv_18, grammarAccess.getAgentAccess().getColonKeyword_1_0_2_0_0_1());
							}
							(
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_0_2_0_0_2_0_0());
										}
										lv_labels_19_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_19_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_20=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_20, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_0_0_2_1());
									}
								)+
							)
						)
						    |
						(
							otherlv_21=LeftSquareBracket
							{
								newLeafNode(otherlv_21, grammarAccess.getAgentAccess().getLeftSquareBracketKeyword_1_0_2_0_1_0());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_0_2_0_1_1_0());
									}
									lv_labels_22_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getAgentRule());
										}
										add(
											$current,
											"labels",
											lv_labels_22_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								otherlv_23=Comma
								{
									newLeafNode(otherlv_23, grammarAccess.getAgentAccess().getCommaKeyword_1_0_2_0_1_2_0());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_0_2_0_1_2_1_0());
										}
										lv_labels_24_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_24_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
							)*
							otherlv_25=RightSquareBracket
							{
								newLeafNode(otherlv_25, grammarAccess.getAgentAccess().getRightSquareBracketKeyword_1_0_2_0_1_3());
							}
							(
								this_NEWLINE_26=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_26, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_0_1_4());
								}
							)+
						)
						    |
						(
							(
								this_NEWLINE_27=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_27, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_0_2_0());
								}
							)+
							this_BEGIN_28=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_28, grammarAccess.getAgentAccess().getBEGINTerminalRuleCall_1_0_2_0_2_1());
							}
							(
								otherlv_29=HyphenMinus
								{
									newLeafNode(otherlv_29, grammarAccess.getAgentAccess().getHyphenMinusKeyword_1_0_2_0_2_2_0());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_0_2_0_2_2_1_0());
										}
										lv_labels_30_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_30_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_31=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_31, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_0_2_2_2());
									}
								)+
							)+
							this_END_32=RULE_END
							{
								newLeafNode(this_END_32, grammarAccess.getAgentAccess().getENDTerminalRuleCall_1_0_2_0_2_3());
							}
							(
								this_NEWLINE_33=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_33, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_0_2_4());
								}
							)*
						)
					)
					(
						otherlv_34=Group
						{
							newLeafNode(otherlv_34, grammarAccess.getAgentAccess().getGroupKeyword_1_0_2_1_0());
						}
						otherlv_35=Colon
						{
							newLeafNode(otherlv_35, grammarAccess.getAgentAccess().getColonKeyword_1_0_2_1_1());
						}
						(
							(
								{
									newCompositeNode(grammarAccess.getAgentAccess().getGroupExpressionParserRuleCall_1_0_2_1_2_0());
								}
								lv_group_36_0=ruleExpression
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getAgentRule());
									}
									set(
										$current,
										"group",
										lv_group_36_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
									afterParserOrEnumRuleCall();
								}
							)
						)
						(
							this_NEWLINE_37=RULE_NEWLINE
							{
								newLeafNode(this_NEWLINE_37, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_0_2_1_3());
							}
						)+
					)?
				)
			)
			    |
			(
				(
					(
						otherlv_38=Group
						{
							newLeafNode(otherlv_38, grammarAccess.getAgentAccess().getGroupKeyword_1_1_0_0_0());
						}
						otherlv_39=Colon
						{
							newLeafNode(otherlv_39, grammarAccess.getAgentAccess().getColonKeyword_1_1_0_0_1());
						}
						(
							(
								{
									newCompositeNode(grammarAccess.getAgentAccess().getGroupExpressionParserRuleCall_1_1_0_0_2_0());
								}
								lv_group_40_0=ruleExpression
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getAgentRule());
									}
									set(
										$current,
										"group",
										lv_group_40_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
									afterParserOrEnumRuleCall();
								}
							)
						)
						(
							this_NEWLINE_41=RULE_NEWLINE
							{
								newLeafNode(this_NEWLINE_41, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_0_3());
							}
						)+
					)
					(
						(
							otherlv_42=Labels
							{
								newLeafNode(otherlv_42, grammarAccess.getAgentAccess().getLabelsKeyword_1_1_0_1_0_0());
							}
							otherlv_43=Colon
							{
								newLeafNode(otherlv_43, grammarAccess.getAgentAccess().getColonKeyword_1_1_0_1_0_1());
							}
							(
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_1_0_1_0_2_0_0());
										}
										lv_labels_44_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_44_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_45=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_45, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_1_0_2_1());
									}
								)+
							)
						)
						    |
						(
							otherlv_46=LeftSquareBracket
							{
								newLeafNode(otherlv_46, grammarAccess.getAgentAccess().getLeftSquareBracketKeyword_1_1_0_1_1_0());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_1_0_1_1_1_0());
									}
									lv_labels_47_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getAgentRule());
										}
										add(
											$current,
											"labels",
											lv_labels_47_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								otherlv_48=Comma
								{
									newLeafNode(otherlv_48, grammarAccess.getAgentAccess().getCommaKeyword_1_1_0_1_1_2_0());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_1_0_1_1_2_1_0());
										}
										lv_labels_49_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_49_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
							)*
							otherlv_50=RightSquareBracket
							{
								newLeafNode(otherlv_50, grammarAccess.getAgentAccess().getRightSquareBracketKeyword_1_1_0_1_1_3());
							}
							(
								this_NEWLINE_51=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_51, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_1_1_4());
								}
							)+
						)
						    |
						(
							(
								this_NEWLINE_52=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_52, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_1_2_0());
								}
							)+
							this_BEGIN_53=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_53, grammarAccess.getAgentAccess().getBEGINTerminalRuleCall_1_1_0_1_2_1());
							}
							(
								otherlv_54=HyphenMinus
								{
									newLeafNode(otherlv_54, grammarAccess.getAgentAccess().getHyphenMinusKeyword_1_1_0_1_2_2_0());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getAgentAccess().getLabelsExpressionParserRuleCall_1_1_0_1_2_2_1_0());
										}
										lv_labels_55_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getAgentRule());
											}
											add(
												$current,
												"labels",
												lv_labels_55_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_56=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_56, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_1_2_2_2());
									}
								)+
							)+
							this_END_57=RULE_END
							{
								newLeafNode(this_END_57, grammarAccess.getAgentAccess().getENDTerminalRuleCall_1_1_0_1_2_3());
							}
							(
								this_NEWLINE_58=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_58, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_0_1_2_4());
								}
							)*
						)
					)?
				)
				this_END_59=RULE_END
				{
					newLeafNode(this_END_59, grammarAccess.getAgentAccess().getENDTerminalRuleCall_1_1_1());
				}
				(
					this_NEWLINE_60=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_60, grammarAccess.getAgentAccess().getNEWLINETerminalRuleCall_1_1_2());
					}
				)*
			)
		)
	)
;

// Entry rule entryRuleStep
entryRuleStep returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getStepRule()); }
	iv_ruleStep=ruleStep
	{ $current=$iv_ruleStep.current; }
	EOF;

// Rule Step
ruleStep returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=HyphenMinus
		{
			newLeafNode(otherlv_0, grammarAccess.getStepAccess().getHyphenMinusKeyword_0());
		}
		this_BEGIN_1=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_1, grammarAccess.getStepAccess().getBEGINTerminalRuleCall_1());
		}
		(
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getStepAccess().getCommandParserRuleCall_2_0());
			}
			this_Command_2=ruleCommand
			{
				$current = $this_Command_2.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getStepAccess().getPackageParserRuleCall_2_1());
			}
			this_Package_3=rulePackage
			{
				$current = $this_Package_3.current;
				afterParserOrEnumRuleCall();
			}
		)
		this_END_4=RULE_END
		{
			newLeafNode(this_END_4, grammarAccess.getStepAccess().getENDTerminalRuleCall_3());
		}
	)
;

// Entry rule entryRuleCommand
entryRuleCommand returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getCommandAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getCommandRule()); }
	iv_ruleCommand=ruleCommand
	{ $current=$iv_ruleCommand.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Command
ruleCommand returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getCommandAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getCommandAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 0);
				}
							({true}?=>(otherlv_1=Id
							{
								newLeafNode(otherlv_1, grammarAccess.getCommandAccess().getIdKeyword_0_0());
							}
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getCommandAccess().getColonKeyword_0_1());
							}
							(
								(
									lv_id_3_0=RULE_STRING
									{
										newLeafNode(lv_id_3_0, grammarAccess.getCommandAccess().getIdSTRINGTerminalRuleCall_0_2_0());
									}
									{
										if ($current==null) {
											$current = createModelElement(grammarAccess.getCommandRule());
										}
										setWithLastConsumed(
											$current,
											"id",
											lv_id_3_0,
											"org.eclipse.xtext.common.Terminals.STRING");
									}
								)
							)
							(
								this_NEWLINE_4=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_4, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_0_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 1);
				}
							({true}?=>((
								{
									newCompositeNode(grammarAccess.getCommandAccess().getIfConditionIfStatementParserRuleCall_1_0());
								}
								lv_ifCondition_5_0=ruleIfStatement
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getCommandRule());
									}
									set(
										$current,
										"ifCondition",
										lv_ifCondition_5_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.IfStatement");
									afterParserOrEnumRuleCall();
								}
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 2);
				}
							({true}?=>(otherlv_6=Name
							{
								newLeafNode(otherlv_6, grammarAccess.getCommandAccess().getNameKeyword_2_0());
							}
							otherlv_7=Colon
							{
								newLeafNode(otherlv_7, grammarAccess.getCommandAccess().getColonKeyword_2_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getCommandAccess().getNameExpressionParserRuleCall_2_2_0());
									}
									lv_name_8_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getCommandRule());
										}
										set(
											$current,
											"name",
											lv_name_8_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_9=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_9, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_2_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 3);
				}
							({true}?=>(otherlv_10=Shell
							{
								newLeafNode(otherlv_10, grammarAccess.getCommandAccess().getShellKeyword_3_0());
							}
							otherlv_11=Colon
							{
								newLeafNode(otherlv_11, grammarAccess.getCommandAccess().getColonKeyword_3_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getCommandAccess().getShellExpressionParserRuleCall_3_2_0());
									}
									lv_shell_12_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getCommandRule());
										}
										set(
											$current,
											"shell",
											lv_shell_12_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_13=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_13, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_3_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 4)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 4);
				}
							({true}?=>(otherlv_14=WorkingDirectory
							{
								newLeafNode(otherlv_14, grammarAccess.getCommandAccess().getWorkingDirectoryKeyword_4_0());
							}
							otherlv_15=Colon
							{
								newLeafNode(otherlv_15, grammarAccess.getCommandAccess().getColonKeyword_4_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getCommandAccess().getWorkingDirectoryExpressionParserRuleCall_4_2_0());
									}
									lv_workingDirectory_16_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getCommandRule());
										}
										set(
											$current,
											"workingDirectory",
											lv_workingDirectory_16_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_17=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_17, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_4_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 5)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 5);
				}
							({true}?=>(otherlv_18=Env
							{
								newLeafNode(otherlv_18, grammarAccess.getCommandAccess().getEnvKeyword_5_0());
							}
							otherlv_19=Colon
							{
								newLeafNode(otherlv_19, grammarAccess.getCommandAccess().getColonKeyword_5_1());
							}
							(
								this_NEWLINE_20=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_20, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_5_2());
								}
							)+
							this_BEGIN_21=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_21, grammarAccess.getCommandAccess().getBEGINTerminalRuleCall_5_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getCommandAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_5_4_0());
									}
									lv_environmentVariables_22_0=ruleVariableAssignment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getCommandRule());
										}
										add(
											$current,
											"environmentVariables",
											lv_environmentVariables_22_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_23=RULE_END
							{
								newLeafNode(this_END_23, grammarAccess.getCommandAccess().getENDTerminalRuleCall_5_5());
							}
							(
								this_NEWLINE_24=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_24, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_5_6());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 6)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 6);
				}
							({true}?=>(otherlv_25=ContinueOnError
							{
								newLeafNode(otherlv_25, grammarAccess.getCommandAccess().getContinueOnErrorKeyword_6_0());
							}
							otherlv_26=Colon
							{
								newLeafNode(otherlv_26, grammarAccess.getCommandAccess().getColonKeyword_6_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getCommandAccess().getContinueOnErrorExpressionParserRuleCall_6_2_0());
									}
									lv_continueOnError_27_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getCommandRule());
										}
										set(
											$current,
											"continueOnError",
											lv_continueOnError_27_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_28=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_28, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_6_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCommandAccess().getUnorderedGroup(), 7)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCommandAccess().getUnorderedGroup(), 7);
				}
							({true}?=>((
								otherlv_29=TimeoutMinutes
								{
									newLeafNode(otherlv_29, grammarAccess.getCommandAccess().getTimeoutMinutesKeyword_7_0_0());
								}
								otherlv_30=Colon
								{
									newLeafNode(otherlv_30, grammarAccess.getCommandAccess().getColonKeyword_7_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getCommandAccess().getTimeoutMinutesExpressionParserRuleCall_7_0_2_0());
										}
										lv_timeoutMinutes_31_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getCommandRule());
											}
											set(
												$current,
												"timeoutMinutes",
												lv_timeoutMinutes_31_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_32=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_32, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_7_0_3());
									}
								)+
							)?
							(
								otherlv_33=Run
								{
									newLeafNode(otherlv_33, grammarAccess.getCommandAccess().getRunKeyword_7_1_0());
								}
								otherlv_34=Colon
								{
									newLeafNode(otherlv_34, grammarAccess.getCommandAccess().getColonKeyword_7_1_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getCommandAccess().getCommandExpressionParserRuleCall_7_1_2_0());
										}
										lv_command_35_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getCommandRule());
											}
											set(
												$current,
												"command",
												lv_command_35_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_36=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_36, grammarAccess.getCommandAccess().getNEWLINETerminalRuleCall_7_1_3());
									}
								)+
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCommandAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getCommandAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getCommandAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRulePackage
entryRulePackage returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getPackageAccess().getUnorderedGroup(), 
	grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4()
	);
}:
	{ newCompositeNode(grammarAccess.getPackageRule()); }
	iv_rulePackage=rulePackage
	{ $current=$iv_rulePackage.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Package
rulePackage returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getPackageAccess().getUnorderedGroup(), 
	grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getPackageAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 0);
				}
							({true}?=>(otherlv_1=Id
							{
								newLeafNode(otherlv_1, grammarAccess.getPackageAccess().getIdKeyword_0_0());
							}
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getPackageAccess().getColonKeyword_0_1());
							}
							(
								(
									lv_id_3_0=RULE_STRING
									{
										newLeafNode(lv_id_3_0, grammarAccess.getPackageAccess().getIdSTRINGTerminalRuleCall_0_2_0());
									}
									{
										if ($current==null) {
											$current = createModelElement(grammarAccess.getPackageRule());
										}
										setWithLastConsumed(
											$current,
											"id",
											lv_id_3_0,
											"org.eclipse.xtext.common.Terminals.STRING");
									}
								)
							)
							(
								this_NEWLINE_4=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_4, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_0_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 1);
				}
							({true}?=>((
								{
									newCompositeNode(grammarAccess.getPackageAccess().getIfConditionIfStatementParserRuleCall_1_0());
								}
								lv_ifCondition_5_0=ruleIfStatement
								{
									if ($current==null) {
										$current = createModelElementForParent(grammarAccess.getPackageRule());
									}
									set(
										$current,
										"ifCondition",
										lv_ifCondition_5_0,
										"d.fe.up.pt.cicd.gha.dsl.GitHubActions.IfStatement");
									afterParserOrEnumRuleCall();
								}
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 2);
				}
							({true}?=>(otherlv_6=Name
							{
								newLeafNode(otherlv_6, grammarAccess.getPackageAccess().getNameKeyword_2_0());
							}
							otherlv_7=Colon
							{
								newLeafNode(otherlv_7, grammarAccess.getPackageAccess().getColonKeyword_2_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getPackageAccess().getNameExpressionParserRuleCall_2_2_0());
									}
									lv_name_8_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getPackageRule());
										}
										set(
											$current,
											"name",
											lv_name_8_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_9=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_9, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_2_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 3);
				}
							({true}?=>(otherlv_10=Shell
							{
								newLeafNode(otherlv_10, grammarAccess.getPackageAccess().getShellKeyword_3_0());
							}
							otherlv_11=Colon
							{
								newLeafNode(otherlv_11, grammarAccess.getPackageAccess().getColonKeyword_3_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getPackageAccess().getShellExpressionParserRuleCall_3_2_0());
									}
									lv_shell_12_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getPackageRule());
										}
										set(
											$current,
											"shell",
											lv_shell_12_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_13=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_13, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_3_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 4)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 4);
				}
							({true}?=>(otherlv_14=WorkingDirectory
							{
								newLeafNode(otherlv_14, grammarAccess.getPackageAccess().getWorkingDirectoryKeyword_4_0());
							}
							otherlv_15=Colon
							{
								newLeafNode(otherlv_15, grammarAccess.getPackageAccess().getColonKeyword_4_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getPackageAccess().getWorkingDirectoryExpressionParserRuleCall_4_2_0());
									}
									lv_workingDirectory_16_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getPackageRule());
										}
										set(
											$current,
											"workingDirectory",
											lv_workingDirectory_16_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_17=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_17, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_4_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 5)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 5);
				}
							({true}?=>(otherlv_18=Env
							{
								newLeafNode(otherlv_18, grammarAccess.getPackageAccess().getEnvKeyword_5_0());
							}
							otherlv_19=Colon
							{
								newLeafNode(otherlv_19, grammarAccess.getPackageAccess().getColonKeyword_5_1());
							}
							(
								this_NEWLINE_20=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_20, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_5_2());
								}
							)+
							this_BEGIN_21=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_21, grammarAccess.getPackageAccess().getBEGINTerminalRuleCall_5_3());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getPackageAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_5_4_0());
									}
									lv_environmentVariables_22_0=ruleVariableAssignment
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getPackageRule());
										}
										add(
											$current,
											"environmentVariables",
											lv_environmentVariables_22_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
										afterParserOrEnumRuleCall();
									}
								)
							)+
							this_END_23=RULE_END
							{
								newLeafNode(this_END_23, grammarAccess.getPackageAccess().getENDTerminalRuleCall_5_5());
							}
							this_NEWLINE_24=RULE_NEWLINE
							{
								newLeafNode(this_NEWLINE_24, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_5_6());
							}
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 6)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 6);
				}
							({true}?=>(otherlv_25=ContinueOnError
							{
								newLeafNode(otherlv_25, grammarAccess.getPackageAccess().getContinueOnErrorKeyword_6_0());
							}
							otherlv_26=Colon
							{
								newLeafNode(otherlv_26, grammarAccess.getPackageAccess().getColonKeyword_6_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getPackageAccess().getContinueOnErrorExpressionParserRuleCall_6_2_0());
									}
									lv_continueOnError_27_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getPackageRule());
										}
										set(
											$current,
											"continueOnError",
											lv_continueOnError_27_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_28=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_28, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_6_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 7)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 7);
				}
							({true}?=>((
								otherlv_29=TimeoutMinutes
								{
									newLeafNode(otherlv_29, grammarAccess.getPackageAccess().getTimeoutMinutesKeyword_7_0_0());
								}
								otherlv_30=Colon
								{
									newLeafNode(otherlv_30, grammarAccess.getPackageAccess().getColonKeyword_7_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getPackageAccess().getTimeoutMinutesExpressionParserRuleCall_7_0_2_0());
										}
										lv_timeoutMinutes_31_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getPackageRule());
											}
											set(
												$current,
												"timeoutMinutes",
												lv_timeoutMinutes_31_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_32=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_32, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_7_0_3());
									}
								)+
							)?
							(
								otherlv_33=Uses
								{
									newLeafNode(otherlv_33, grammarAccess.getPackageAccess().getUsesKeyword_7_1_0());
								}
								otherlv_34=Colon
								{
									newLeafNode(otherlv_34, grammarAccess.getPackageAccess().getColonKeyword_7_1_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getPackageAccess().getUsesExpressionParserRuleCall_7_1_2_0());
										}
										lv_uses_35_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getPackageRule());
											}
											set(
												$current,
												"uses",
												lv_uses_35_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_36=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_36, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_7_1_3());
									}
								)+
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup(), 8)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup(), 8);
				}
							({true}?=>((
								otherlv_37=With
								{
									newLeafNode(otherlv_37, grammarAccess.getPackageAccess().getWithKeyword_8_0_0());
								}
								otherlv_38=Colon
								{
									newLeafNode(otherlv_38, grammarAccess.getPackageAccess().getColonKeyword_8_0_1());
								}
								(
									this_NEWLINE_39=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_39, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_8_0_2());
									}
								)+
								this_BEGIN_40=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_40, grammarAccess.getPackageAccess().getBEGINTerminalRuleCall_8_0_3());
								}
								(
									(
										{ 
										  getUnorderedGroupHelper().enter(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4());
										}
										(
											(
									(
										{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4(), 0)}?=>(
											{
												getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4(), 0);
											}
														({true}?=>(otherlv_42=Entrypoint
														{
															newLeafNode(otherlv_42, grammarAccess.getPackageAccess().getEntrypointKeyword_8_0_4_0_0());
														}
														otherlv_43=Colon
														{
															newLeafNode(otherlv_43, grammarAccess.getPackageAccess().getColonKeyword_8_0_4_0_1());
														}
														(
															(
																{
																	newCompositeNode(grammarAccess.getPackageAccess().getEntrypointExpressionParserRuleCall_8_0_4_0_2_0());
																}
																lv_entrypoint_44_0=ruleExpression
																{
																	if ($current==null) {
																		$current = createModelElementForParent(grammarAccess.getPackageRule());
																	}
																	set(
																		$current,
																		"entrypoint",
																		lv_entrypoint_44_0,
																		"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																	afterParserOrEnumRuleCall();
																}
															)
														)
														(
															this_NEWLINE_45=RULE_NEWLINE
															{
																newLeafNode(this_NEWLINE_45, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_8_0_4_0_3());
															}
														)+
														))
											{ 
												getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4());
											}
										)
									)|
									(
										{getUnorderedGroupHelper().canSelect(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4(), 1)}?=>(
											{
												getUnorderedGroupHelper().select(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4(), 1);
											}
														({true}?=>(otherlv_46=Args
														{
															newLeafNode(otherlv_46, grammarAccess.getPackageAccess().getArgsKeyword_8_0_4_1_0());
														}
														otherlv_47=Colon
														{
															newLeafNode(otherlv_47, grammarAccess.getPackageAccess().getColonKeyword_8_0_4_1_1());
														}
														(
															(
																{
																	newCompositeNode(grammarAccess.getPackageAccess().getContainerArgsExpressionParserRuleCall_8_0_4_1_2_0());
																}
																lv_containerArgs_48_0=ruleExpression
																{
																	if ($current==null) {
																		$current = createModelElementForParent(grammarAccess.getPackageRule());
																	}
																	set(
																		$current,
																		"containerArgs",
																		lv_containerArgs_48_0,
																		"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																	afterParserOrEnumRuleCall();
																}
															)
														)
														(
															this_NEWLINE_49=RULE_NEWLINE
															{
																newLeafNode(this_NEWLINE_49, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_8_0_4_1_3());
															}
														)+
														))
											{ 
												getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4());
											}
										)
									)
											)+
											{getUnorderedGroupHelper().canLeave(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4())}?
										)
									)
										{ 
										  getUnorderedGroupHelper().leave(grammarAccess.getPackageAccess().getUnorderedGroup_8_0_4());
										}
								)
							)
							    |
							(
								(
									(
										{
											newCompositeNode(grammarAccess.getPackageAccess().getArgsVariableAssignmentParserRuleCall_8_1_0_0());
										}
										lv_args_50_0=ruleVariableAssignment
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getPackageRule());
											}
											add(
												$current,
												"args",
												lv_args_50_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
											afterParserOrEnumRuleCall();
										}
									)
								)+
								this_END_51=RULE_END
								{
									newLeafNode(this_END_51, grammarAccess.getPackageAccess().getENDTerminalRuleCall_8_1_1());
								}
								(
									this_NEWLINE_52=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_52, grammarAccess.getPackageAccess().getNEWLINETerminalRuleCall_8_1_2());
									}
								)*
							)
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getPackageAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getPackageAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getPackageAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleIfStatement
entryRuleIfStatement returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getIfStatementRule()); }
	iv_ruleIfStatement=ruleIfStatement
	{ $current=$iv_ruleIfStatement.current; }
	EOF;

// Rule IfStatement
ruleIfStatement returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=If
		{
			newLeafNode(otherlv_0, grammarAccess.getIfStatementAccess().getIfKeyword_0());
		}
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getIfStatementAccess().getColonKeyword_1());
		}
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getIfStatementAccess().getExpressionParserRuleCall_2());
		}
		this_Expression_2=ruleExpression
		{
			$current = $this_Expression_2.current;
			afterParserOrEnumRuleCall();
		}
		(
			this_NEWLINE_3=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_3, grammarAccess.getIfStatementAccess().getNEWLINETerminalRuleCall_3());
			}
		)+
	)
;

// Entry rule entryRuleConcurrencyGroup
entryRuleConcurrencyGroup returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getConcurrencyGroupRule()); }
	iv_ruleConcurrencyGroup=ruleConcurrencyGroup
	{ $current=$iv_ruleConcurrencyGroup.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule ConcurrencyGroup
ruleConcurrencyGroup returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup(), 0);
				}
							({true}?=>(otherlv_1=Group
							{
								newLeafNode(otherlv_1, grammarAccess.getConcurrencyGroupAccess().getGroupKeyword_0_0());
							}
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getConcurrencyGroupAccess().getColonKeyword_0_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getConcurrencyGroupAccess().getNameExpressionParserRuleCall_0_2_0());
									}
									lv_name_3_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getConcurrencyGroupRule());
										}
										set(
											$current,
											"name",
											lv_name_3_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_4=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_4, grammarAccess.getConcurrencyGroupAccess().getNEWLINETerminalRuleCall_0_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup(), 1);
				}
							({true}?=>(otherlv_5=CancelInProgress
							{
								newLeafNode(otherlv_5, grammarAccess.getConcurrencyGroupAccess().getCancelInProgressKeyword_1_0());
							}
							otherlv_6=Colon
							{
								newLeafNode(otherlv_6, grammarAccess.getConcurrencyGroupAccess().getColonKeyword_1_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getConcurrencyGroupAccess().getCancelInProgressExpressionParserRuleCall_1_2_0());
									}
									lv_cancelInProgress_7_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getConcurrencyGroupRule());
										}
										set(
											$current,
											"cancelInProgress",
											lv_cancelInProgress_7_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_8=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_8, grammarAccess.getConcurrencyGroupAccess().getNEWLINETerminalRuleCall_1_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getConcurrencyGroupAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleSimpleTrigger
entryRuleSimpleTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimpleTriggerRule()); }
	iv_ruleSimpleTrigger=ruleSimpleTrigger
	{ $current=$iv_ruleSimpleTrigger.current; }
	EOF;

// Rule SimpleTrigger
ruleSimpleTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getSimpleTriggerAccess().getSimpleWorkflowDispatchTriggerParserRuleCall_0_0());
			}
			this_SimpleWorkflowDispatchTrigger_0=ruleSimpleWorkflowDispatchTrigger
			{
				$current = $this_SimpleWorkflowDispatchTrigger_0.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getSimpleTriggerAccess().getSimpleWorkflowCallTriggerParserRuleCall_0_1());
			}
			this_SimpleWorkflowCallTrigger_1=ruleSimpleWorkflowCallTrigger
			{
				$current = $this_SimpleWorkflowCallTrigger_1.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getSimpleTriggerAccess().getSimplePushTriggerTriggerParserRuleCall_0_2());
			}
			this_SimplePushTriggerTrigger_2=ruleSimplePushTriggerTrigger
			{
				$current = $this_SimplePushTriggerTrigger_2.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getSimpleTriggerAccess().getSimplePullRequestTriggerParserRuleCall_0_3());
			}
			this_SimplePullRequestTrigger_3=ruleSimplePullRequestTrigger
			{
				$current = $this_SimplePullRequestTrigger_3.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getSimpleTriggerAccess().getSimpleStandardEventTriggerParserRuleCall_0_4());
			}
			this_SimpleStandardEventTrigger_4=ruleSimpleStandardEventTrigger
			{
				$current = $this_SimpleStandardEventTrigger_4.current;
				afterParserOrEnumRuleCall();
			}
		)
		(
			this_NEWLINE_5=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_5, grammarAccess.getSimpleTriggerAccess().getNEWLINETerminalRuleCall_1());
			}
		)+
	)
;

// Entry rule entryRuleSimplePullRequestTrigger
entryRuleSimplePullRequestTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimplePullRequestTriggerRule()); }
	iv_ruleSimplePullRequestTrigger=ruleSimplePullRequestTrigger
	{ $current=$iv_ruleSimplePullRequestTrigger.current; }
	EOF;

// Rule SimplePullRequestTrigger
ruleSimplePullRequestTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getSimplePullRequestTriggerAccess().getPullRequestTriggerAction_0_0(),
						$current);
				}
			)
			otherlv_1=Pull_request
			{
				newLeafNode(otherlv_1, grammarAccess.getSimplePullRequestTriggerAccess().getPull_requestKeyword_0_1());
			}
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getSimplePullRequestTriggerAccess().getPullRequestTriggerAction_1_0(),
						$current);
				}
			)
			otherlv_3=Pull_request_target
			{
				newLeafNode(otherlv_3, grammarAccess.getSimplePullRequestTriggerAccess().getPull_request_targetKeyword_1_1());
			}
		)
	)
;

// Entry rule entryRuleSimplePushTriggerTrigger
entryRuleSimplePushTriggerTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimplePushTriggerTriggerRule()); }
	iv_ruleSimplePushTriggerTrigger=ruleSimplePushTriggerTrigger
	{ $current=$iv_ruleSimplePushTriggerTrigger.current; }
	EOF;

// Rule SimplePushTriggerTrigger
ruleSimplePushTriggerTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getSimplePushTriggerTriggerAccess().getPushTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Push
		{
			newLeafNode(otherlv_1, grammarAccess.getSimplePushTriggerTriggerAccess().getPushKeyword_1());
		}
	)
;

// Entry rule entryRuleSimpleWorkflowCallTrigger
entryRuleSimpleWorkflowCallTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimpleWorkflowCallTriggerRule()); }
	iv_ruleSimpleWorkflowCallTrigger=ruleSimpleWorkflowCallTrigger
	{ $current=$iv_ruleSimpleWorkflowCallTrigger.current; }
	EOF;

// Rule SimpleWorkflowCallTrigger
ruleSimpleWorkflowCallTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getSimpleWorkflowCallTriggerAccess().getWorkflowCallTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Workflow_call
		{
			newLeafNode(otherlv_1, grammarAccess.getSimpleWorkflowCallTriggerAccess().getWorkflow_callKeyword_1());
		}
	)
;

// Entry rule entryRuleSimpleWorkflowDispatchTrigger
entryRuleSimpleWorkflowDispatchTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimpleWorkflowDispatchTriggerRule()); }
	iv_ruleSimpleWorkflowDispatchTrigger=ruleSimpleWorkflowDispatchTrigger
	{ $current=$iv_ruleSimpleWorkflowDispatchTrigger.current; }
	EOF;

// Rule SimpleWorkflowDispatchTrigger
ruleSimpleWorkflowDispatchTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getSimpleWorkflowDispatchTriggerAccess().getWorkflowDispatchTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Workflow_dispatch
		{
			newLeafNode(otherlv_1, grammarAccess.getSimpleWorkflowDispatchTriggerAccess().getWorkflow_dispatchKeyword_1());
		}
	)
;

// Entry rule entryRuleSimpleStandardEventTrigger
entryRuleSimpleStandardEventTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSimpleStandardEventTriggerRule()); }
	iv_ruleSimpleStandardEventTrigger=ruleSimpleStandardEventTrigger
	{ $current=$iv_ruleSimpleStandardEventTrigger.current; }
	EOF;

// Rule SimpleStandardEventTrigger
ruleSimpleStandardEventTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				newCompositeNode(grammarAccess.getSimpleStandardEventTriggerAccess().getEventEVENTEnumRuleCall_0());
			}
			lv_event_0_0=ruleEVENT
			{
				if ($current==null) {
					$current = createModelElementForParent(grammarAccess.getSimpleStandardEventTriggerRule());
				}
				set(
					$current,
					"event",
					lv_event_0_0,
					"d.fe.up.pt.cicd.gha.dsl.GitHubActions.EVENT");
				afterParserOrEnumRuleCall();
			}
		)
	)
;

// Entry rule entryRuleOptionedTrigger
entryRuleOptionedTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOptionedTriggerRule()); }
	iv_ruleOptionedTrigger=ruleOptionedTrigger
	{ $current=$iv_ruleOptionedTrigger.current; }
	EOF;

// Rule OptionedTrigger
ruleOptionedTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			this_NEWLINE_0=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_0, grammarAccess.getOptionedTriggerAccess().getNEWLINETerminalRuleCall_0());
			}
		)+
		this_BEGIN_1=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_1, grammarAccess.getOptionedTriggerAccess().getBEGINTerminalRuleCall_1());
		}
		(
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedPullRequestTriggerParserRuleCall_2_0());
			}
			this_OptionedPullRequestTrigger_2=ruleOptionedPullRequestTrigger
			{
				$current = $this_OptionedPullRequestTrigger_2.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedPushTriggerParserRuleCall_2_1());
			}
			this_OptionedPushTrigger_3=ruleOptionedPushTrigger
			{
				$current = $this_OptionedPushTrigger_3.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedScheduleTriggerParserRuleCall_2_2());
			}
			this_OptionedScheduleTrigger_4=ruleOptionedScheduleTrigger
			{
				$current = $this_OptionedScheduleTrigger_4.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedStandardEventTriggerParserRuleCall_2_3());
			}
			this_OptionedStandardEventTrigger_5=ruleOptionedStandardEventTrigger
			{
				$current = $this_OptionedStandardEventTrigger_5.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedWorkflowCallTriggerParserRuleCall_2_4());
			}
			this_OptionedWorkflowCallTrigger_6=ruleOptionedWorkflowCallTrigger
			{
				$current = $this_OptionedWorkflowCallTrigger_6.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedWorkflowRunTriggerParserRuleCall_2_5());
			}
			this_OptionedWorkflowRunTrigger_7=ruleOptionedWorkflowRunTrigger
			{
				$current = $this_OptionedWorkflowRunTrigger_7.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getOptionedTriggerAccess().getOptionedWorkflowDispatchTriggerParserRuleCall_2_6());
			}
			this_OptionedWorkflowDispatchTrigger_8=ruleOptionedWorkflowDispatchTrigger
			{
				$current = $this_OptionedWorkflowDispatchTrigger_8.current;
				afterParserOrEnumRuleCall();
			}
		)
		this_END_9=RULE_END
		{
			newLeafNode(this_END_9, grammarAccess.getOptionedTriggerAccess().getENDTerminalRuleCall_3());
		}
		(
			this_NEWLINE_10=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_10, grammarAccess.getOptionedTriggerAccess().getNEWLINETerminalRuleCall_4());
			}
		)*
	)
;

// Entry rule entryRuleOptionedStandardEventTrigger
entryRuleOptionedStandardEventTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOptionedStandardEventTriggerRule()); }
	iv_ruleOptionedStandardEventTrigger=ruleOptionedStandardEventTrigger
	{ $current=$iv_ruleOptionedStandardEventTrigger.current; }
	EOF;

// Rule OptionedStandardEventTrigger
ruleOptionedStandardEventTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				(
					{
						newCompositeNode(grammarAccess.getOptionedStandardEventTriggerAccess().getEventEVENTEnumRuleCall_0_0_0());
					}
					lv_event_0_0=ruleEVENT
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getOptionedStandardEventTriggerRule());
						}
						set(
							$current,
							"event",
							lv_event_0_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.EVENT");
						afterParserOrEnumRuleCall();
					}
				)
			)
			otherlv_1=Colon
			{
				newLeafNode(otherlv_1, grammarAccess.getOptionedStandardEventTriggerAccess().getColonKeyword_0_1());
			}
			(
				this_NEWLINE_2=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_2, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_0_2());
				}
			)+
			this_BEGIN_3=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_3, grammarAccess.getOptionedStandardEventTriggerAccess().getBEGINTerminalRuleCall_0_3());
			}
			otherlv_4=Types
			{
				newLeafNode(otherlv_4, grammarAccess.getOptionedStandardEventTriggerAccess().getTypesKeyword_0_4());
			}
			otherlv_5=Colon
			{
				newLeafNode(otherlv_5, grammarAccess.getOptionedStandardEventTriggerAccess().getColonKeyword_0_5());
			}
			(
				(
					(
						{
							newCompositeNode(grammarAccess.getOptionedStandardEventTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_0_6_0_0());
						}
						lv_eventTypes_6_0=ruleWEBHOOK_ACTIVITY_TYPE
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getOptionedStandardEventTriggerRule());
							}
							add(
								$current,
								"eventTypes",
								lv_eventTypes_6_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_7=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_7, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_0_6_1());
					}
				)+
			)
		)
		    |
		(
			otherlv_8=LeftSquareBracket
			{
				newLeafNode(otherlv_8, grammarAccess.getOptionedStandardEventTriggerAccess().getLeftSquareBracketKeyword_1_0());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getOptionedStandardEventTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_1_1_0());
					}
					lv_eventTypes_9_0=ruleWEBHOOK_ACTIVITY_TYPE
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getOptionedStandardEventTriggerRule());
						}
						add(
							$current,
							"eventTypes",
							lv_eventTypes_9_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				otherlv_10=Comma
				{
					newLeafNode(otherlv_10, grammarAccess.getOptionedStandardEventTriggerAccess().getCommaKeyword_1_2_0());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getOptionedStandardEventTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_1_2_1_0());
						}
						lv_eventTypes_11_0=ruleWEBHOOK_ACTIVITY_TYPE
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getOptionedStandardEventTriggerRule());
							}
							add(
								$current,
								"eventTypes",
								lv_eventTypes_11_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
							afterParserOrEnumRuleCall();
						}
					)
				)
			)*
			otherlv_12=RightSquareBracket
			{
				newLeafNode(otherlv_12, grammarAccess.getOptionedStandardEventTriggerAccess().getRightSquareBracketKeyword_1_3());
			}
			(
				this_NEWLINE_13=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_13, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_1_4());
				}
			)+
		)
		    |
		(
			(
				(
					this_NEWLINE_14=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_14, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_2_0_0());
					}
				)+
				this_BEGIN_15=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_15, grammarAccess.getOptionedStandardEventTriggerAccess().getBEGINTerminalRuleCall_2_0_1());
				}
				(
					otherlv_16=HyphenMinus
					{
						newLeafNode(otherlv_16, grammarAccess.getOptionedStandardEventTriggerAccess().getHyphenMinusKeyword_2_0_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getOptionedStandardEventTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_2_0_2_1_0());
							}
							lv_eventTypes_17_0=ruleWEBHOOK_ACTIVITY_TYPE
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getOptionedStandardEventTriggerRule());
								}
								add(
									$current,
									"eventTypes",
									lv_eventTypes_17_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
								afterParserOrEnumRuleCall();
							}
						)
					)
					(
						this_NEWLINE_18=RULE_NEWLINE
						{
							newLeafNode(this_NEWLINE_18, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_2_0_2_2());
						}
					)+
				)+
				this_END_19=RULE_END
				{
					newLeafNode(this_END_19, grammarAccess.getOptionedStandardEventTriggerAccess().getENDTerminalRuleCall_2_0_3());
				}
				(
					this_NEWLINE_20=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_20, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_2_0_4());
					}
				)*
			)
			this_END_21=RULE_END
			{
				newLeafNode(this_END_21, grammarAccess.getOptionedStandardEventTriggerAccess().getENDTerminalRuleCall_2_1());
			}
			(
				this_NEWLINE_22=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_22, grammarAccess.getOptionedStandardEventTriggerAccess().getNEWLINETerminalRuleCall_2_2());
				}
			)*
		)
	)
;

// Entry rule entryRuleOptionedWorkflowRunTrigger
entryRuleOptionedWorkflowRunTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOptionedWorkflowRunTriggerRule()); }
	iv_ruleOptionedWorkflowRunTrigger=ruleOptionedWorkflowRunTrigger
	{ $current=$iv_ruleOptionedWorkflowRunTrigger.current; }
	EOF;

// Rule OptionedWorkflowRunTrigger
ruleOptionedWorkflowRunTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			otherlv_0=Workflow_run
			{
				newLeafNode(otherlv_0, grammarAccess.getOptionedWorkflowRunTriggerAccess().getWorkflow_runKeyword_0_0());
			}
			otherlv_1=Colon
			{
				newLeafNode(otherlv_1, grammarAccess.getOptionedWorkflowRunTriggerAccess().getColonKeyword_0_1());
			}
			(
				this_NEWLINE_2=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_2, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_0_2());
				}
			)+
			this_BEGIN_3=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_3, grammarAccess.getOptionedWorkflowRunTriggerAccess().getBEGINTerminalRuleCall_0_3());
			}
			(
				otherlv_4=Branches
				{
					newLeafNode(otherlv_4, grammarAccess.getOptionedWorkflowRunTriggerAccess().getBranchesKeyword_0_4_0());
				}
				    |
				(
					(
						lv_ignoreSpecifiedBranches_5_0=BranchesIgnore
						{
							newLeafNode(lv_ignoreSpecifiedBranches_5_0, grammarAccess.getOptionedWorkflowRunTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_0_4_1_0());
						}
						{
							if ($current==null) {
								$current = createModelElement(grammarAccess.getOptionedWorkflowRunTriggerRule());
							}
							setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_5_0 != null, "branches-ignore");
						}
					)
				)
			)
			otherlv_6=Colon
			{
				newLeafNode(otherlv_6, grammarAccess.getOptionedWorkflowRunTriggerAccess().getColonKeyword_0_5());
			}
			(
				(
					(
						{
							newCompositeNode(grammarAccess.getOptionedWorkflowRunTriggerAccess().getBranchesExpressionParserRuleCall_0_6_0_0());
						}
						lv_branches_7_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getOptionedWorkflowRunTriggerRule());
							}
							add(
								$current,
								"branches",
								lv_branches_7_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_8=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_8, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_0_6_1());
					}
				)+
			)
		)
		    |
		(
			otherlv_9=LeftSquareBracket
			{
				newLeafNode(otherlv_9, grammarAccess.getOptionedWorkflowRunTriggerAccess().getLeftSquareBracketKeyword_1_0());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getOptionedWorkflowRunTriggerAccess().getBranchesExpressionParserRuleCall_1_1_0());
					}
					lv_branches_10_0=ruleExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getOptionedWorkflowRunTriggerRule());
						}
						add(
							$current,
							"branches",
							lv_branches_10_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				otherlv_11=Comma
				{
					newLeafNode(otherlv_11, grammarAccess.getOptionedWorkflowRunTriggerAccess().getCommaKeyword_1_2_0());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getOptionedWorkflowRunTriggerAccess().getBranchesExpressionParserRuleCall_1_2_1_0());
						}
						lv_branches_12_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getOptionedWorkflowRunTriggerRule());
							}
							add(
								$current,
								"branches",
								lv_branches_12_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
			)*
			otherlv_13=RightSquareBracket
			{
				newLeafNode(otherlv_13, grammarAccess.getOptionedWorkflowRunTriggerAccess().getRightSquareBracketKeyword_1_3());
			}
			(
				this_NEWLINE_14=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_14, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_1_4());
				}
			)+
		)
		    |
		(
			(
				(
					this_NEWLINE_15=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_15, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_2_0_0());
					}
				)+
				this_BEGIN_16=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_16, grammarAccess.getOptionedWorkflowRunTriggerAccess().getBEGINTerminalRuleCall_2_0_1());
				}
				(
					otherlv_17=HyphenMinus
					{
						newLeafNode(otherlv_17, grammarAccess.getOptionedWorkflowRunTriggerAccess().getHyphenMinusKeyword_2_0_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getOptionedWorkflowRunTriggerAccess().getBranchesExpressionParserRuleCall_2_0_2_1_0());
							}
							lv_branches_18_0=ruleExpression
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getOptionedWorkflowRunTriggerRule());
								}
								add(
									$current,
									"branches",
									lv_branches_18_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
								afterParserOrEnumRuleCall();
							}
						)
					)
					(
						this_NEWLINE_19=RULE_NEWLINE
						{
							newLeafNode(this_NEWLINE_19, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_2_0_2_2());
						}
					)+
				)+
				this_END_20=RULE_END
				{
					newLeafNode(this_END_20, grammarAccess.getOptionedWorkflowRunTriggerAccess().getENDTerminalRuleCall_2_0_3());
				}
				(
					this_NEWLINE_21=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_21, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_2_0_4());
					}
				)*
			)
			this_END_22=RULE_END
			{
				newLeafNode(this_END_22, grammarAccess.getOptionedWorkflowRunTriggerAccess().getENDTerminalRuleCall_2_1());
			}
			(
				this_NEWLINE_23=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_23, grammarAccess.getOptionedWorkflowRunTriggerAccess().getNEWLINETerminalRuleCall_2_2());
				}
			)*
		)
	)
;

// Entry rule entryRuleOptionedPullRequestTrigger
entryRuleOptionedPullRequestTrigger returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0()
	);
}:
	{ newCompositeNode(grammarAccess.getOptionedPullRequestTriggerRule()); }
	iv_ruleOptionedPullRequestTrigger=ruleOptionedPullRequestTrigger
	{ $current=$iv_ruleOptionedPullRequestTrigger.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule OptionedPullRequestTrigger
ruleOptionedPullRequestTrigger returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getOptionedPullRequestTriggerAccess().getPullRequestTriggerAction_0(),
					$current);
			}
		)
		(
			otherlv_1=Pull_request
			{
				newLeafNode(otherlv_1, grammarAccess.getOptionedPullRequestTriggerAccess().getPull_requestKeyword_1_0());
			}
			    |
			otherlv_2=Pull_request_target
			{
				newLeafNode(otherlv_2, grammarAccess.getOptionedPullRequestTriggerAccess().getPull_request_targetKeyword_1_1());
			}
		)
		otherlv_3=Colon
		{
			newLeafNode(otherlv_3, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_2());
		}
		(
			(
				(
					this_NEWLINE_4=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_4, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_0());
					}
				)+
				this_BEGIN_5=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_5, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_0_1());
				}
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 0);
							}
										({true}?=>((
											(
												otherlv_7=Types
												{
													newLeafNode(otherlv_7, grammarAccess.getOptionedPullRequestTriggerAccess().getTypesKeyword_3_0_2_0_0_0_0());
												}
												otherlv_8=Colon
												{
													newLeafNode(otherlv_8, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_0_2_0_0_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_0_2_0_0_0_2_0_0());
															}
															lv_eventTypes_9_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_9_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_10=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_10, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_11=LeftSquareBracket
												{
													newLeafNode(otherlv_11, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_0_2_0_0_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_0_2_0_0_1_1_0());
														}
														lv_eventTypes_12_0=ruleWEBHOOK_ACTIVITY_TYPE
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"eventTypes",
																lv_eventTypes_12_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_13=Comma
													{
														newLeafNode(otherlv_13, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_0_2_0_0_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_0_2_0_0_1_2_1_0());
															}
															lv_eventTypes_14_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_14_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_15=RightSquareBracket
												{
													newLeafNode(otherlv_15, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_0_2_0_0_1_3());
												}
												(
													this_NEWLINE_16=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_16, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_17=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_17, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_0());
													}
												)+
												this_BEGIN_18=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_18, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_0_2_1());
												}
												(
													otherlv_19=HyphenMinus
													{
														newLeafNode(otherlv_19, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_0_2_0_0_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_0_2_0_0_2_2_1_0());
															}
															lv_eventTypes_20_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_20_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_21=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_21, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_2_2());
														}
													)+
												)+
												this_END_22=RULE_END
												{
													newLeafNode(this_END_22, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_0_2_0_0_2_3());
												}
												(
													this_NEWLINE_23=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_23, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_4());
													}
												)*
											)
										)
										(
											(
												(
													otherlv_24=Branches
													{
														newLeafNode(otherlv_24, grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesKeyword_3_0_2_0_1_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedBranches_25_0=BranchesIgnore
															{
																newLeafNode(lv_ignoreSpecifiedBranches_25_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_0_2_0_1_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_25_0 != null, "branches-ignore");
															}
														)
													)
												)
												otherlv_26=Colon
												{
													newLeafNode(otherlv_26, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_0_2_0_1_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_1_0_2_0_0());
															}
															lv_branches_27_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_27_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_28=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_28, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_29=LeftSquareBracket
												{
													newLeafNode(otherlv_29, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_0_2_0_1_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_1_1_1_0());
														}
														lv_branches_30_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_30_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_31=Comma
													{
														newLeafNode(otherlv_31, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_0_2_0_1_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_1_1_2_1_0());
															}
															lv_branches_32_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_32_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_33=RightSquareBracket
												{
													newLeafNode(otherlv_33, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_0_2_0_1_1_3());
												}
												(
													this_NEWLINE_34=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_34, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_35=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_35, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_0());
													}
												)+
												this_BEGIN_36=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_36, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_1_2_1());
												}
												(
													otherlv_37=HyphenMinus
													{
														newLeafNode(otherlv_37, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_0_2_0_1_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_1_2_2_1_0());
															}
															lv_branches_38_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_38_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_39=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_39, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_2_2());
														}
													)+
												)+
												this_END_40=RULE_END
												{
													newLeafNode(this_END_40, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_0_2_0_1_2_3());
												}
												(
													this_NEWLINE_41=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_41, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_4());
													}
												)*
											)
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2(), 1);
							}
										({true}?=>((
											(
												otherlv_42=Paths
												{
													newLeafNode(otherlv_42, grammarAccess.getOptionedPullRequestTriggerAccess().getPathsKeyword_3_0_2_1_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedPaths_43_0=PathsIgnore
														{
															newLeafNode(lv_ignoreSpecifiedPaths_43_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_0_2_1_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_43_0 != null, "paths-ignore");
														}
													)
												)
											)
											otherlv_44=Colon
											{
												newLeafNode(otherlv_44, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_0_2_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_1_0_2_0_0());
														}
														lv_paths_45_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_45_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_46=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_46, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_47=LeftSquareBracket
											{
												newLeafNode(otherlv_47, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_0_2_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_1_1_1_0());
													}
													lv_paths_48_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_48_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_49=Comma
												{
													newLeafNode(otherlv_49, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_0_2_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_1_1_2_1_0());
														}
														lv_paths_50_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_50_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_51=RightSquareBracket
											{
												newLeafNode(otherlv_51, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_0_2_1_1_3());
											}
											(
												this_NEWLINE_52=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_52, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_53=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_53, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_0());
												}
											)+
											this_BEGIN_54=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_54, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_0_2_1_2_1());
											}
											(
												otherlv_55=HyphenMinus
												{
													newLeafNode(otherlv_55, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_0_2_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_1_2_2_1_0());
														}
														lv_paths_56_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_56_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_57=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_57, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_2_2());
													}
												)+
											)+
											this_END_58=RULE_END
											{
												newLeafNode(this_END_58, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_0_2_1_2_3());
											}
											(
												this_NEWLINE_59=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_59, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_4());
												}
											)*
										)
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_0_2());
						}
				)
			)
			    |
			(
				(
					{ 
					  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1());
					}
					(
						(
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 0)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 0);
						}
									({true}?=>((
										(
											(
												otherlv_61=Branches
												{
													newLeafNode(otherlv_61, grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesKeyword_3_1_0_0_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedBranches_62_0=BranchesIgnore
														{
															newLeafNode(lv_ignoreSpecifiedBranches_62_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_1_0_0_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_62_0 != null, "branches-ignore");
														}
													)
												)
											)
											otherlv_63=Colon
											{
												newLeafNode(otherlv_63, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_1_0_0_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_0_0_2_0_0());
														}
														lv_branches_64_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_64_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_65=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_65, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_66=LeftSquareBracket
											{
												newLeafNode(otherlv_66, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_1_0_0_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_0_1_1_0());
													}
													lv_branches_67_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"branches",
															lv_branches_67_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_68=Comma
												{
													newLeafNode(otherlv_68, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_1_0_0_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_0_1_2_1_0());
														}
														lv_branches_69_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_69_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_70=RightSquareBracket
											{
												newLeafNode(otherlv_70, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_1_0_0_1_3());
											}
											(
												this_NEWLINE_71=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_71, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_72=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_72, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_0());
												}
											)+
											this_BEGIN_73=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_73, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_1_0_0_2_1());
											}
											(
												otherlv_74=HyphenMinus
												{
													newLeafNode(otherlv_74, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_1_0_0_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_0_2_2_1_0());
														}
														lv_branches_75_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_75_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_76=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_76, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_2_2());
													}
												)+
											)+
											this_END_77=RULE_END
											{
												newLeafNode(this_END_77, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_1_0_0_2_3());
											}
											(
												this_NEWLINE_78=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_78, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_4());
												}
											)*
										)
									)
									(
										(
											otherlv_79=Types
											{
												newLeafNode(otherlv_79, grammarAccess.getOptionedPullRequestTriggerAccess().getTypesKeyword_3_1_0_1_0_0());
											}
											otherlv_80=Colon
											{
												newLeafNode(otherlv_80, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_1_0_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_1_0_1_0_2_0_0());
														}
														lv_eventTypes_81_0=ruleWEBHOOK_ACTIVITY_TYPE
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"eventTypes",
																lv_eventTypes_81_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_82=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_82, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_83=LeftSquareBracket
											{
												newLeafNode(otherlv_83, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_1_0_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_1_0_1_1_1_0());
													}
													lv_eventTypes_84_0=ruleWEBHOOK_ACTIVITY_TYPE
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"eventTypes",
															lv_eventTypes_84_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_85=Comma
												{
													newLeafNode(otherlv_85, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_1_0_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_1_0_1_1_2_1_0());
														}
														lv_eventTypes_86_0=ruleWEBHOOK_ACTIVITY_TYPE
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"eventTypes",
																lv_eventTypes_86_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_87=RightSquareBracket
											{
												newLeafNode(otherlv_87, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_1_0_1_1_3());
											}
											(
												this_NEWLINE_88=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_88, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_89=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_89, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_0());
												}
											)+
											this_BEGIN_90=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_90, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_1_0_1_2_1());
											}
											(
												otherlv_91=HyphenMinus
												{
													newLeafNode(otherlv_91, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_1_0_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_1_0_1_2_2_1_0());
														}
														lv_eventTypes_92_0=ruleWEBHOOK_ACTIVITY_TYPE
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"eventTypes",
																lv_eventTypes_92_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_93=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_93, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_2_2());
													}
												)+
											)+
											this_END_94=RULE_END
											{
												newLeafNode(this_END_94, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_1_0_1_2_3());
											}
											(
												this_NEWLINE_95=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_95, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_4());
												}
											)*
										)
									)?
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)|
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 1)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1(), 1);
						}
									({true}?=>((
										(
											otherlv_96=Paths
											{
												newLeafNode(otherlv_96, grammarAccess.getOptionedPullRequestTriggerAccess().getPathsKeyword_3_1_1_0_0_0());
											}
											    |
											(
												(
													lv_ignoreSpecifiedPaths_97_0=PathsIgnore
													{
														newLeafNode(lv_ignoreSpecifiedPaths_97_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_1_1_0_0_1_0());
													}
													{
														if ($current==null) {
															$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_97_0 != null, "paths-ignore");
													}
												)
											)
										)
										otherlv_98=Colon
										{
											newLeafNode(otherlv_98, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_1_1_0_1());
										}
										(
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_1_1_0_2_0_0());
													}
													lv_paths_99_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_99_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												this_NEWLINE_100=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_100, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_0_2_1());
												}
											)+
										)
									)
									    |
									(
										otherlv_101=LeftSquareBracket
										{
											newLeafNode(otherlv_101, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_1_1_1_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_1_1_1_1_0());
												}
												lv_paths_102_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
													}
													add(
														$current,
														"paths",
														lv_paths_102_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											otherlv_103=Comma
											{
												newLeafNode(otherlv_103, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_1_1_1_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_1_1_1_2_1_0());
													}
													lv_paths_104_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_104_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
										)*
										otherlv_105=RightSquareBracket
										{
											newLeafNode(otherlv_105, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_1_1_1_3());
										}
										(
											this_NEWLINE_106=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_106, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_1_4());
											}
										)+
									)
									    |
									(
										(
											this_NEWLINE_107=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_107, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_0());
											}
										)+
										this_BEGIN_108=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_108, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_1_1_2_1());
										}
										(
											otherlv_109=HyphenMinus
											{
												newLeafNode(otherlv_109, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_1_1_2_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_1_1_2_2_1_0());
													}
													lv_paths_110_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_110_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												this_NEWLINE_111=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_111, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_2_2());
												}
											)+
										)+
										this_END_112=RULE_END
										{
											newLeafNode(this_END_112, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_1_1_2_3());
										}
										(
											this_NEWLINE_113=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_113, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_4());
											}
										)*
									)
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)
						)+
						{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1())}?
					)
				)
					{ 
					  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_1());
					}
			)
			    |
			(
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0(), 0);
							}
										({true}?=>((
											(
												(
													otherlv_115=Paths
													{
														newLeafNode(otherlv_115, grammarAccess.getOptionedPullRequestTriggerAccess().getPathsKeyword_3_2_0_0_0_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedPaths_116_0=PathsIgnore
															{
																newLeafNode(lv_ignoreSpecifiedPaths_116_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_2_0_0_0_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_116_0 != null, "paths-ignore");
															}
														)
													)
												)
												otherlv_117=Colon
												{
													newLeafNode(otherlv_117, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_2_0_0_0_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_0_0_0_2_0_0());
															}
															lv_paths_118_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_118_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_119=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_119, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_120=LeftSquareBracket
												{
													newLeafNode(otherlv_120, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_2_0_0_0_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_0_0_1_1_0());
														}
														lv_paths_121_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_121_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_122=Comma
													{
														newLeafNode(otherlv_122, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_2_0_0_0_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_0_0_1_2_1_0());
															}
															lv_paths_123_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_123_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_124=RightSquareBracket
												{
													newLeafNode(otherlv_124, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_2_0_0_0_1_3());
												}
												(
													this_NEWLINE_125=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_125, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_126=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_126, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_0());
													}
												)+
												this_BEGIN_127=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_127, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_0_2_1());
												}
												(
													otherlv_128=HyphenMinus
													{
														newLeafNode(otherlv_128, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_2_0_0_0_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_0_0_2_2_1_0());
															}
															lv_paths_129_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_129_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_130=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_130, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_2_2());
														}
													)+
												)+
												this_END_131=RULE_END
												{
													newLeafNode(this_END_131, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_2_0_0_0_2_3());
												}
												(
													this_NEWLINE_132=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_132, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_4());
													}
												)*
											)
										)
										(
											(
												otherlv_133=Types
												{
													newLeafNode(otherlv_133, grammarAccess.getOptionedPullRequestTriggerAccess().getTypesKeyword_3_2_0_0_1_0_0());
												}
												otherlv_134=Colon
												{
													newLeafNode(otherlv_134, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_2_0_0_1_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_2_0_0_1_0_2_0_0());
															}
															lv_eventTypes_135_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_135_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_136=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_136, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_137=LeftSquareBracket
												{
													newLeafNode(otherlv_137, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_2_0_0_1_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_2_0_0_1_1_1_0());
														}
														lv_eventTypes_138_0=ruleWEBHOOK_ACTIVITY_TYPE
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"eventTypes",
																lv_eventTypes_138_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_139=Comma
													{
														newLeafNode(otherlv_139, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_2_0_0_1_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_2_0_0_1_1_2_1_0());
															}
															lv_eventTypes_140_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_140_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_141=RightSquareBracket
												{
													newLeafNode(otherlv_141, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_2_0_0_1_1_3());
												}
												(
													this_NEWLINE_142=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_142, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_143=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_143, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_0());
													}
												)+
												this_BEGIN_144=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_144, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_1_2_1());
												}
												(
													otherlv_145=HyphenMinus
													{
														newLeafNode(otherlv_145, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_2_0_0_1_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getEventTypesWEBHOOK_ACTIVITY_TYPEEnumRuleCall_3_2_0_0_1_2_2_1_0());
															}
															lv_eventTypes_146_0=ruleWEBHOOK_ACTIVITY_TYPE
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
																}
																add(
																	$current,
																	"eventTypes",
																	lv_eventTypes_146_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.WEBHOOK_ACTIVITY_TYPE");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_147=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_147, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_2_2());
														}
													)+
												)+
												this_END_148=RULE_END
												{
													newLeafNode(this_END_148, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_2_0_0_1_2_3());
												}
												(
													this_NEWLINE_149=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_149, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_4());
													}
												)*
											)
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0(), 1);
							}
										({true}?=>((
											(
												otherlv_150=Branches
												{
													newLeafNode(otherlv_150, grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesKeyword_3_2_0_1_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedBranches_151_0=BranchesIgnore
														{
															newLeafNode(lv_ignoreSpecifiedBranches_151_0, grammarAccess.getOptionedPullRequestTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_2_0_1_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_151_0 != null, "branches-ignore");
														}
													)
												)
											)
											otherlv_152=Colon
											{
												newLeafNode(otherlv_152, grammarAccess.getOptionedPullRequestTriggerAccess().getColonKeyword_3_2_0_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_1_0_2_0_0());
														}
														lv_branches_153_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_153_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_154=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_154, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_155=LeftSquareBracket
											{
												newLeafNode(otherlv_155, grammarAccess.getOptionedPullRequestTriggerAccess().getLeftSquareBracketKeyword_3_2_0_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_1_1_1_0());
													}
													lv_branches_156_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
														}
														add(
															$current,
															"branches",
															lv_branches_156_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_157=Comma
												{
													newLeafNode(otherlv_157, grammarAccess.getOptionedPullRequestTriggerAccess().getCommaKeyword_3_2_0_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_1_1_2_1_0());
														}
														lv_branches_158_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_158_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_159=RightSquareBracket
											{
												newLeafNode(otherlv_159, grammarAccess.getOptionedPullRequestTriggerAccess().getRightSquareBracketKeyword_3_2_0_1_1_3());
											}
											(
												this_NEWLINE_160=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_160, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_161=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_161, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_0());
												}
											)+
											this_BEGIN_162=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_162, grammarAccess.getOptionedPullRequestTriggerAccess().getBEGINTerminalRuleCall_3_2_0_1_2_1());
											}
											(
												otherlv_163=HyphenMinus
												{
													newLeafNode(otherlv_163, grammarAccess.getOptionedPullRequestTriggerAccess().getHyphenMinusKeyword_3_2_0_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPullRequestTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_1_2_2_1_0());
														}
														lv_branches_164_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPullRequestTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_164_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_165=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_165, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_2_2());
													}
												)+
											)+
											this_END_166=RULE_END
											{
												newLeafNode(this_END_166, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_2_0_1_2_3());
											}
											(
												this_NEWLINE_167=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_167, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_4());
												}
											)*
										)
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPullRequestTriggerAccess().getUnorderedGroup_3_2_0());
						}
				)
				this_END_168=RULE_END
				{
					newLeafNode(this_END_168, grammarAccess.getOptionedPullRequestTriggerAccess().getENDTerminalRuleCall_3_2_1());
				}
				(
					this_NEWLINE_169=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_169, grammarAccess.getOptionedPullRequestTriggerAccess().getNEWLINETerminalRuleCall_3_2_2());
					}
				)*
			)
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleOptionedPushTrigger
entryRuleOptionedPushTrigger returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0()
	);
}:
	{ newCompositeNode(grammarAccess.getOptionedPushTriggerRule()); }
	iv_ruleOptionedPushTrigger=ruleOptionedPushTrigger
	{ $current=$iv_ruleOptionedPushTrigger.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule OptionedPushTrigger
ruleOptionedPushTrigger returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getOptionedPushTriggerAccess().getPushTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Push
		{
			newLeafNode(otherlv_1, grammarAccess.getOptionedPushTriggerAccess().getPushKeyword_1());
		}
		otherlv_2=Colon
		{
			newLeafNode(otherlv_2, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_2());
		}
		(
			(
				(
					this_NEWLINE_3=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_3, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_0());
					}
				)+
				this_BEGIN_4=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_4, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_0_1());
				}
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 0);
							}
										({true}?=>((
											(
												(
													otherlv_6=Branches
													{
														newLeafNode(otherlv_6, grammarAccess.getOptionedPushTriggerAccess().getBranchesKeyword_3_0_2_0_0_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedBranches_7_0=BranchesIgnore
															{
																newLeafNode(lv_ignoreSpecifiedBranches_7_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_0_2_0_0_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_7_0 != null, "branches-ignore");
															}
														)
													)
												)
												otherlv_8=Colon
												{
													newLeafNode(otherlv_8, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_0_2_0_0_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_0_0_2_0_0());
															}
															lv_branches_9_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_9_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_10=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_10, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_11=LeftSquareBracket
												{
													newLeafNode(otherlv_11, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_0_2_0_0_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_0_1_1_0());
														}
														lv_branches_12_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_12_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_13=Comma
													{
														newLeafNode(otherlv_13, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_0_2_0_0_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_0_1_2_1_0());
															}
															lv_branches_14_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_14_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_15=RightSquareBracket
												{
													newLeafNode(otherlv_15, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_0_2_0_0_1_3());
												}
												(
													this_NEWLINE_16=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_16, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_17=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_17, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_0());
													}
												)+
												this_BEGIN_18=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_18, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_0_2_1());
												}
												(
													otherlv_19=HyphenMinus
													{
														newLeafNode(otherlv_19, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_0_2_0_0_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_0_2_0_0_2_2_1_0());
															}
															lv_branches_20_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_20_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_21=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_21, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_2_2());
														}
													)+
												)+
												this_END_22=RULE_END
												{
													newLeafNode(this_END_22, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_0_2_0_0_2_3());
												}
												(
													this_NEWLINE_23=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_23, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2_4());
													}
												)*
											)
										)
										(
											(
												(
													otherlv_24=Paths
													{
														newLeafNode(otherlv_24, grammarAccess.getOptionedPushTriggerAccess().getPathsKeyword_3_0_2_0_1_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedPaths_25_0=PathsIgnore
															{
																newLeafNode(lv_ignoreSpecifiedPaths_25_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_0_2_0_1_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_25_0 != null, "paths-ignore");
															}
														)
													)
												)
												otherlv_26=Colon
												{
													newLeafNode(otherlv_26, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_0_2_0_1_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_0_1_0_2_0_0());
															}
															lv_paths_27_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_27_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_28=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_28, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_29=LeftSquareBracket
												{
													newLeafNode(otherlv_29, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_0_2_0_1_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_0_1_1_1_0());
														}
														lv_paths_30_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_30_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_31=Comma
													{
														newLeafNode(otherlv_31, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_0_2_0_1_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_0_1_1_2_1_0());
															}
															lv_paths_32_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_32_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_33=RightSquareBracket
												{
													newLeafNode(otherlv_33, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_0_2_0_1_1_3());
												}
												(
													this_NEWLINE_34=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_34, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_35=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_35, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_0());
													}
												)+
												this_BEGIN_36=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_36, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_1_2_1());
												}
												(
													otherlv_37=HyphenMinus
													{
														newLeafNode(otherlv_37, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_0_2_0_1_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_0_2_0_1_2_2_1_0());
															}
															lv_paths_38_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"paths",
																	lv_paths_38_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_39=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_39, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_2_2());
														}
													)+
												)+
												this_END_40=RULE_END
												{
													newLeafNode(this_END_40, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_0_2_0_1_2_3());
												}
												(
													this_NEWLINE_41=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_41, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2_4());
													}
												)*
											)
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2(), 1);
							}
										({true}?=>((
											(
												otherlv_42=Tags
												{
													newLeafNode(otherlv_42, grammarAccess.getOptionedPushTriggerAccess().getTagsKeyword_3_0_2_1_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedTags_43_0=TagsIgnore
														{
															newLeafNode(lv_ignoreSpecifiedTags_43_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedTagsTagsIgnoreKeyword_3_0_2_1_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedTags", lv_ignoreSpecifiedTags_43_0 != null, "tags-ignore");
														}
													)
												)
											)
											otherlv_44=Colon
											{
												newLeafNode(otherlv_44, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_0_2_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_0_2_1_0_2_0_0());
														}
														lv_tags_45_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"tags",
																lv_tags_45_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_46=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_46, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_47=LeftSquareBracket
											{
												newLeafNode(otherlv_47, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_0_2_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_0_2_1_1_1_0());
													}
													lv_tags_48_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"tags",
															lv_tags_48_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_49=Comma
												{
													newLeafNode(otherlv_49, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_0_2_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_0_2_1_1_2_1_0());
														}
														lv_tags_50_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"tags",
																lv_tags_50_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_51=RightSquareBracket
											{
												newLeafNode(otherlv_51, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_0_2_1_1_3());
											}
											(
												this_NEWLINE_52=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_52, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_53=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_53, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_0());
												}
											)+
											this_BEGIN_54=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_54, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_0_2_1_2_1());
											}
											(
												otherlv_55=HyphenMinus
												{
													newLeafNode(otherlv_55, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_0_2_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_0_2_1_2_2_1_0());
														}
														lv_tags_56_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"tags",
																lv_tags_56_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_57=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_57, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_2_2());
													}
												)+
											)+
											this_END_58=RULE_END
											{
												newLeafNode(this_END_58, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_0_2_1_2_3());
											}
											(
												this_NEWLINE_59=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_59, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2_4());
												}
											)*
										)
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_0_2());
						}
				)
			)
			    |
			(
				(
					{ 
					  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1());
					}
					(
						(
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 0)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 0);
						}
									({true}?=>((
										(
											(
												otherlv_61=Paths
												{
													newLeafNode(otherlv_61, grammarAccess.getOptionedPushTriggerAccess().getPathsKeyword_3_1_0_0_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedPaths_62_0=PathsIgnore
														{
															newLeafNode(lv_ignoreSpecifiedPaths_62_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_1_0_0_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_62_0 != null, "paths-ignore");
														}
													)
												)
											)
											otherlv_63=Colon
											{
												newLeafNode(otherlv_63, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_1_0_0_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_1_0_0_0_2_0_0());
														}
														lv_paths_64_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_64_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_65=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_65, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_66=LeftSquareBracket
											{
												newLeafNode(otherlv_66, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_1_0_0_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_1_0_0_1_1_0());
													}
													lv_paths_67_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_67_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_68=Comma
												{
													newLeafNode(otherlv_68, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_1_0_0_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_1_0_0_1_2_1_0());
														}
														lv_paths_69_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_69_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_70=RightSquareBracket
											{
												newLeafNode(otherlv_70, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_1_0_0_1_3());
											}
											(
												this_NEWLINE_71=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_71, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_72=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_72, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_0());
												}
											)+
											this_BEGIN_73=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_73, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_1_0_0_2_1());
											}
											(
												otherlv_74=HyphenMinus
												{
													newLeafNode(otherlv_74, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_1_0_0_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_1_0_0_2_2_1_0());
														}
														lv_paths_75_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_75_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_76=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_76, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_2_2());
													}
												)+
											)+
											this_END_77=RULE_END
											{
												newLeafNode(this_END_77, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_1_0_0_2_3());
											}
											(
												this_NEWLINE_78=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_78, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2_4());
												}
											)*
										)
									)
									(
										(
											(
												otherlv_79=Branches
												{
													newLeafNode(otherlv_79, grammarAccess.getOptionedPushTriggerAccess().getBranchesKeyword_3_1_0_1_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedBranches_80_0=BranchesIgnore
														{
															newLeafNode(lv_ignoreSpecifiedBranches_80_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_1_0_1_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_80_0 != null, "branches-ignore");
														}
													)
												)
											)
											otherlv_81=Colon
											{
												newLeafNode(otherlv_81, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_1_0_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_1_0_2_0_0());
														}
														lv_branches_82_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_82_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_83=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_83, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_84=LeftSquareBracket
											{
												newLeafNode(otherlv_84, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_1_0_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_1_1_1_0());
													}
													lv_branches_85_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"branches",
															lv_branches_85_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_86=Comma
												{
													newLeafNode(otherlv_86, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_1_0_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_1_1_2_1_0());
														}
														lv_branches_87_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_87_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_88=RightSquareBracket
											{
												newLeafNode(otherlv_88, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_1_0_1_1_3());
											}
											(
												this_NEWLINE_89=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_89, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_90=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_90, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_0());
												}
											)+
											this_BEGIN_91=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_91, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_1_0_1_2_1());
											}
											(
												otherlv_92=HyphenMinus
												{
													newLeafNode(otherlv_92, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_1_0_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_1_0_1_2_2_1_0());
														}
														lv_branches_93_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_93_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_94=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_94, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_2_2());
													}
												)+
											)+
											this_END_95=RULE_END
											{
												newLeafNode(this_END_95, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_1_0_1_2_3());
											}
											(
												this_NEWLINE_96=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_96, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2_4());
												}
											)*
										)
									)?
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)|
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 1)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1(), 1);
						}
									({true}?=>((
										(
											otherlv_97=Tags
											{
												newLeafNode(otherlv_97, grammarAccess.getOptionedPushTriggerAccess().getTagsKeyword_3_1_1_0_0_0());
											}
											    |
											(
												(
													lv_ignoreSpecifiedTags_98_0=TagsIgnore
													{
														newLeafNode(lv_ignoreSpecifiedTags_98_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedTagsTagsIgnoreKeyword_3_1_1_0_0_1_0());
													}
													{
														if ($current==null) {
															$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
														}
														setWithLastConsumed($current, "ignoreSpecifiedTags", lv_ignoreSpecifiedTags_98_0 != null, "tags-ignore");
													}
												)
											)
										)
										otherlv_99=Colon
										{
											newLeafNode(otherlv_99, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_1_1_0_1());
										}
										(
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_1_1_0_2_0_0());
													}
													lv_tags_100_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"tags",
															lv_tags_100_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												this_NEWLINE_101=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_101, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_0_2_1());
												}
											)+
										)
									)
									    |
									(
										otherlv_102=LeftSquareBracket
										{
											newLeafNode(otherlv_102, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_1_1_1_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_1_1_1_1_0());
												}
												lv_tags_103_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
													}
													add(
														$current,
														"tags",
														lv_tags_103_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											otherlv_104=Comma
											{
												newLeafNode(otherlv_104, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_1_1_1_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_1_1_1_2_1_0());
													}
													lv_tags_105_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"tags",
															lv_tags_105_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
										)*
										otherlv_106=RightSquareBracket
										{
											newLeafNode(otherlv_106, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_1_1_1_3());
										}
										(
											this_NEWLINE_107=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_107, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_1_4());
											}
										)+
									)
									    |
									(
										(
											this_NEWLINE_108=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_108, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_0());
											}
										)+
										this_BEGIN_109=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_109, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_1_1_2_1());
										}
										(
											otherlv_110=HyphenMinus
											{
												newLeafNode(otherlv_110, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_1_1_2_2_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_1_1_2_2_1_0());
													}
													lv_tags_111_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"tags",
															lv_tags_111_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												this_NEWLINE_112=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_112, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_2_2());
												}
											)+
										)+
										this_END_113=RULE_END
										{
											newLeafNode(this_END_113, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_1_1_2_3());
										}
										(
											this_NEWLINE_114=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_114, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2_4());
											}
										)*
									)
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)
						)+
						{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1())}?
					)
				)
					{ 
					  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_1());
					}
			)
			    |
			(
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0(), 0);
							}
										({true}?=>((
											(
												(
													otherlv_116=Tags
													{
														newLeafNode(otherlv_116, grammarAccess.getOptionedPushTriggerAccess().getTagsKeyword_3_2_0_0_0_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedTags_117_0=TagsIgnore
															{
																newLeafNode(lv_ignoreSpecifiedTags_117_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedTagsTagsIgnoreKeyword_3_2_0_0_0_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedTags", lv_ignoreSpecifiedTags_117_0 != null, "tags-ignore");
															}
														)
													)
												)
												otherlv_118=Colon
												{
													newLeafNode(otherlv_118, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_2_0_0_0_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_2_0_0_0_0_2_0_0());
															}
															lv_tags_119_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"tags",
																	lv_tags_119_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_120=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_120, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_121=LeftSquareBracket
												{
													newLeafNode(otherlv_121, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_2_0_0_0_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_2_0_0_0_1_1_0());
														}
														lv_tags_122_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"tags",
																lv_tags_122_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_123=Comma
													{
														newLeafNode(otherlv_123, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_2_0_0_0_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_2_0_0_0_1_2_1_0());
															}
															lv_tags_124_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"tags",
																	lv_tags_124_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_125=RightSquareBracket
												{
													newLeafNode(otherlv_125, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_2_0_0_0_1_3());
												}
												(
													this_NEWLINE_126=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_126, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_127=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_127, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_0());
													}
												)+
												this_BEGIN_128=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_128, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_0_2_1());
												}
												(
													otherlv_129=HyphenMinus
													{
														newLeafNode(otherlv_129, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_2_0_0_0_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getTagsExpressionParserRuleCall_3_2_0_0_0_2_2_1_0());
															}
															lv_tags_130_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"tags",
																	lv_tags_130_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_131=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_131, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_2_2());
														}
													)+
												)+
												this_END_132=RULE_END
												{
													newLeafNode(this_END_132, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_2_0_0_0_2_3());
												}
												(
													this_NEWLINE_133=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_133, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2_4());
													}
												)*
											)
										)
										(
											(
												(
													otherlv_134=Branches
													{
														newLeafNode(otherlv_134, grammarAccess.getOptionedPushTriggerAccess().getBranchesKeyword_3_2_0_0_1_0_0_0());
													}
													    |
													(
														(
															lv_ignoreSpecifiedBranches_135_0=BranchesIgnore
															{
																newLeafNode(lv_ignoreSpecifiedBranches_135_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedBranchesBranchesIgnoreKeyword_3_2_0_0_1_0_0_1_0());
															}
															{
																if ($current==null) {
																	$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
																}
																setWithLastConsumed($current, "ignoreSpecifiedBranches", lv_ignoreSpecifiedBranches_135_0 != null, "branches-ignore");
															}
														)
													)
												)
												otherlv_136=Colon
												{
													newLeafNode(otherlv_136, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_2_0_0_1_0_1());
												}
												(
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_0_1_0_2_0_0());
															}
															lv_branches_137_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_137_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_138=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_138, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_0_2_1());
														}
													)+
												)
											)
											    |
											(
												otherlv_139=LeftSquareBracket
												{
													newLeafNode(otherlv_139, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_2_0_0_1_1_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_0_1_1_1_0());
														}
														lv_branches_140_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"branches",
																lv_branches_140_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													otherlv_141=Comma
													{
														newLeafNode(otherlv_141, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_2_0_0_1_1_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_0_1_1_2_1_0());
															}
															lv_branches_142_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_142_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
												)*
												otherlv_143=RightSquareBracket
												{
													newLeafNode(otherlv_143, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_2_0_0_1_1_3());
												}
												(
													this_NEWLINE_144=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_144, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_1_4());
													}
												)+
											)
											    |
											(
												(
													this_NEWLINE_145=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_145, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_0());
													}
												)+
												this_BEGIN_146=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_146, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_1_2_1());
												}
												(
													otherlv_147=HyphenMinus
													{
														newLeafNode(otherlv_147, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_2_0_0_1_2_2_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getBranchesExpressionParserRuleCall_3_2_0_0_1_2_2_1_0());
															}
															lv_branches_148_0=ruleExpression
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
																}
																add(
																	$current,
																	"branches",
																	lv_branches_148_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																afterParserOrEnumRuleCall();
															}
														)
													)
													(
														this_NEWLINE_149=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_149, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_2_2());
														}
													)+
												)+
												this_END_150=RULE_END
												{
													newLeafNode(this_END_150, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_2_0_0_1_2_3());
												}
												(
													this_NEWLINE_151=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_151, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2_4());
													}
												)*
											)
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0(), 1);
							}
										({true}?=>((
											(
												otherlv_152=Paths
												{
													newLeafNode(otherlv_152, grammarAccess.getOptionedPushTriggerAccess().getPathsKeyword_3_2_0_1_0_0_0());
												}
												    |
												(
													(
														lv_ignoreSpecifiedPaths_153_0=PathsIgnore
														{
															newLeafNode(lv_ignoreSpecifiedPaths_153_0, grammarAccess.getOptionedPushTriggerAccess().getIgnoreSpecifiedPathsPathsIgnoreKeyword_3_2_0_1_0_0_1_0());
														}
														{
															if ($current==null) {
																$current = createModelElement(grammarAccess.getOptionedPushTriggerRule());
															}
															setWithLastConsumed($current, "ignoreSpecifiedPaths", lv_ignoreSpecifiedPaths_153_0 != null, "paths-ignore");
														}
													)
												)
											)
											otherlv_154=Colon
											{
												newLeafNode(otherlv_154, grammarAccess.getOptionedPushTriggerAccess().getColonKeyword_3_2_0_1_0_1());
											}
											(
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_1_0_2_0_0());
														}
														lv_paths_155_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_155_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_156=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_156, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_0_2_1());
													}
												)+
											)
										)
										    |
										(
											otherlv_157=LeftSquareBracket
											{
												newLeafNode(otherlv_157, grammarAccess.getOptionedPushTriggerAccess().getLeftSquareBracketKeyword_3_2_0_1_1_0());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_1_1_1_0());
													}
													lv_paths_158_0=ruleExpression
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
														}
														add(
															$current,
															"paths",
															lv_paths_158_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
														afterParserOrEnumRuleCall();
													}
												)
											)
											(
												otherlv_159=Comma
												{
													newLeafNode(otherlv_159, grammarAccess.getOptionedPushTriggerAccess().getCommaKeyword_3_2_0_1_1_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_1_1_2_1_0());
														}
														lv_paths_160_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_160_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
											)*
											otherlv_161=RightSquareBracket
											{
												newLeafNode(otherlv_161, grammarAccess.getOptionedPushTriggerAccess().getRightSquareBracketKeyword_3_2_0_1_1_3());
											}
											(
												this_NEWLINE_162=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_162, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_1_4());
												}
											)+
										)
										    |
										(
											(
												this_NEWLINE_163=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_163, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_0());
												}
											)+
											this_BEGIN_164=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_164, grammarAccess.getOptionedPushTriggerAccess().getBEGINTerminalRuleCall_3_2_0_1_2_1());
											}
											(
												otherlv_165=HyphenMinus
												{
													newLeafNode(otherlv_165, grammarAccess.getOptionedPushTriggerAccess().getHyphenMinusKeyword_3_2_0_1_2_2_0());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getOptionedPushTriggerAccess().getPathsExpressionParserRuleCall_3_2_0_1_2_2_1_0());
														}
														lv_paths_166_0=ruleExpression
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getOptionedPushTriggerRule());
															}
															add(
																$current,
																"paths",
																lv_paths_166_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
															afterParserOrEnumRuleCall();
														}
													)
												)
												(
													this_NEWLINE_167=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_167, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_2_2());
													}
												)+
											)+
											this_END_168=RULE_END
											{
												newLeafNode(this_END_168, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_2_0_1_2_3());
											}
											(
												this_NEWLINE_169=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_169, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2_4());
												}
											)*
										)
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedPushTriggerAccess().getUnorderedGroup_3_2_0());
						}
				)
				this_END_170=RULE_END
				{
					newLeafNode(this_END_170, grammarAccess.getOptionedPushTriggerAccess().getENDTerminalRuleCall_3_2_1());
				}
				(
					this_NEWLINE_171=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_171, grammarAccess.getOptionedPushTriggerAccess().getNEWLINETerminalRuleCall_3_2_2());
					}
				)*
			)
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleOptionedScheduleTrigger
entryRuleOptionedScheduleTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOptionedScheduleTriggerRule()); }
	iv_ruleOptionedScheduleTrigger=ruleOptionedScheduleTrigger
	{ $current=$iv_ruleOptionedScheduleTrigger.current; }
	EOF;

// Rule OptionedScheduleTrigger
ruleOptionedScheduleTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=Schedule
		{
			newLeafNode(otherlv_0, grammarAccess.getOptionedScheduleTriggerAccess().getScheduleKeyword_0());
		}
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getOptionedScheduleTriggerAccess().getColonKeyword_1());
		}
		(
			(
				this_NEWLINE_2=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_2, grammarAccess.getOptionedScheduleTriggerAccess().getNEWLINETerminalRuleCall_2_0());
				}
			)+
			this_BEGIN_3=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_3, grammarAccess.getOptionedScheduleTriggerAccess().getBEGINTerminalRuleCall_2_1());
			}
			(
				otherlv_4=HyphenMinus
				{
					newLeafNode(otherlv_4, grammarAccess.getOptionedScheduleTriggerAccess().getHyphenMinusKeyword_2_2_0());
				}
				otherlv_5=Cron
				{
					newLeafNode(otherlv_5, grammarAccess.getOptionedScheduleTriggerAccess().getCronKeyword_2_2_1());
				}
				otherlv_6=Colon
				{
					newLeafNode(otherlv_6, grammarAccess.getOptionedScheduleTriggerAccess().getColonKeyword_2_2_2());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getOptionedScheduleTriggerAccess().getCronsExpressionParserRuleCall_2_2_3_0());
						}
						lv_crons_7_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getOptionedScheduleTriggerRule());
							}
							add(
								$current,
								"crons",
								lv_crons_7_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_8=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_8, grammarAccess.getOptionedScheduleTriggerAccess().getNEWLINETerminalRuleCall_2_2_4());
					}
				)+
			)+
			this_END_9=RULE_END
			{
				newLeafNode(this_END_9, grammarAccess.getOptionedScheduleTriggerAccess().getENDTerminalRuleCall_2_3());
			}
			(
				this_NEWLINE_10=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_10, grammarAccess.getOptionedScheduleTriggerAccess().getNEWLINETerminalRuleCall_2_4());
				}
			)*
		)
	)
;

// Entry rule entryRuleOptionedWorkflowCallTrigger
entryRuleOptionedWorkflowCallTrigger returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0()
	);
}:
	{ newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerRule()); }
	iv_ruleOptionedWorkflowCallTrigger=ruleOptionedWorkflowCallTrigger
	{ $current=$iv_ruleOptionedWorkflowCallTrigger.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule OptionedWorkflowCallTrigger
ruleOptionedWorkflowCallTrigger returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 
	grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getOptionedWorkflowCallTriggerAccess().getWorkflowCallTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Workflow_call
		{
			newLeafNode(otherlv_1, grammarAccess.getOptionedWorkflowCallTriggerAccess().getWorkflow_callKeyword_1());
		}
		otherlv_2=Colon
		{
			newLeafNode(otherlv_2, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_2());
		}
		(
			(
				(
					this_NEWLINE_3=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_3, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_0());
					}
				)+
				this_BEGIN_4=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_4, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_0_1());
				}
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 0);
							}
										({true}?=>((
											otherlv_6=Inputs
											{
												newLeafNode(otherlv_6, grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsKeyword_3_0_2_0_0_0());
											}
											otherlv_7=Colon
											{
												newLeafNode(otherlv_7, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_0_2_0_0_1());
											}
											(
												this_NEWLINE_8=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_8, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_2());
												}
											)+
											this_BEGIN_9=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_9, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_0_3());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsInputParserRuleCall_3_0_2_0_0_4_0());
													}
													lv_inputs_10_0=ruleInput
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
														}
														add(
															$current,
															"inputs",
															lv_inputs_10_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Input");
														afterParserOrEnumRuleCall();
													}
												)
											)+
											this_END_11=RULE_END
											{
												newLeafNode(this_END_11, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_0_2_0_0_5());
											}
											(
												this_NEWLINE_12=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_12, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_0_6());
												}
											)*
										)
										(
											otherlv_13=Outputs
											{
												newLeafNode(otherlv_13, grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsKeyword_3_0_2_0_1_0());
											}
											otherlv_14=Colon
											{
												newLeafNode(otherlv_14, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_0_2_0_1_1());
											}
											(
												this_NEWLINE_15=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_15, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_2());
												}
											)+
											this_BEGIN_16=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_16, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_0_2_0_1_3());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsOutputParserRuleCall_3_0_2_0_1_4_0());
													}
													lv_outputs_17_0=ruleOutput
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
														}
														add(
															$current,
															"outputs",
															lv_outputs_17_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Output");
														afterParserOrEnumRuleCall();
													}
												)
											)+
											this_END_18=RULE_END
											{
												newLeafNode(this_END_18, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_0_2_0_1_5());
											}
											(
												this_NEWLINE_19=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_19, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_0_1_6());
												}
											)*
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2(), 1);
							}
										({true}?=>(otherlv_20=Secrets
										{
											newLeafNode(otherlv_20, grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsKeyword_3_0_2_1_0());
										}
										otherlv_21=Colon
										{
											newLeafNode(otherlv_21, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_0_2_1_1());
										}
										(
											this_NEWLINE_22=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_22, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_2());
											}
										)+
										this_BEGIN_23=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_23, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_0_2_1_3());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsSecretParserRuleCall_3_0_2_1_4_0());
												}
												lv_secrets_24_0=ruleSecret
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
													}
													add(
														$current,
														"secrets",
														lv_secrets_24_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Secret");
													afterParserOrEnumRuleCall();
												}
											)
										)+
										this_END_25=RULE_END
										{
											newLeafNode(this_END_25, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_0_2_1_5());
										}
										(
											this_NEWLINE_26=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_26, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_0_2_1_6());
											}
										)*
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_0_2());
						}
				)
			)
			    |
			(
				(
					{ 
					  getUnorderedGroupHelper().enter(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1());
					}
					(
						(
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 0)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 0);
						}
									({true}?=>((
										otherlv_28=Outputs
										{
											newLeafNode(otherlv_28, grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsKeyword_3_1_0_0_0());
										}
										otherlv_29=Colon
										{
											newLeafNode(otherlv_29, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_1_0_0_1());
										}
										(
											this_NEWLINE_30=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_30, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_2());
											}
										)+
										this_BEGIN_31=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_31, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_1_0_0_3());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsOutputParserRuleCall_3_1_0_0_4_0());
												}
												lv_outputs_32_0=ruleOutput
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
													}
													add(
														$current,
														"outputs",
														lv_outputs_32_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Output");
													afterParserOrEnumRuleCall();
												}
											)
										)+
										this_END_33=RULE_END
										{
											newLeafNode(this_END_33, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_1_0_0_5());
										}
										(
											this_NEWLINE_34=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_34, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_0_6());
											}
										)*
									)
									(
										otherlv_35=Inputs
										{
											newLeafNode(otherlv_35, grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsKeyword_3_1_0_1_0());
										}
										otherlv_36=Colon
										{
											newLeafNode(otherlv_36, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_1_0_1_1());
										}
										(
											this_NEWLINE_37=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_37, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_2());
											}
										)+
										this_BEGIN_38=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_38, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_1_0_1_3());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsInputParserRuleCall_3_1_0_1_4_0());
												}
												lv_inputs_39_0=ruleInput
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
													}
													add(
														$current,
														"inputs",
														lv_inputs_39_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Input");
													afterParserOrEnumRuleCall();
												}
											)
										)+
										this_END_40=RULE_END
										{
											newLeafNode(this_END_40, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_1_0_1_5());
										}
										(
											this_NEWLINE_41=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_41, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_0_1_6());
											}
										)*
									)?
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)|
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 1)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1(), 1);
						}
									({true}?=>(otherlv_42=Secrets
									{
										newLeafNode(otherlv_42, grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsKeyword_3_1_1_0());
									}
									otherlv_43=Colon
									{
										newLeafNode(otherlv_43, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_1_1_1());
									}
									(
										this_NEWLINE_44=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_44, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_2());
										}
									)+
									this_BEGIN_45=RULE_BEGIN
									{
										newLeafNode(this_BEGIN_45, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_1_1_3());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsSecretParserRuleCall_3_1_1_4_0());
											}
											lv_secrets_46_0=ruleSecret
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
												}
												add(
													$current,
													"secrets",
													lv_secrets_46_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Secret");
												afterParserOrEnumRuleCall();
											}
										)
									)+
									this_END_47=RULE_END
									{
										newLeafNode(this_END_47, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_1_1_5());
									}
									(
										this_NEWLINE_48=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_48, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_1_1_6());
										}
									)*
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1());
						}
					)
				)
						)+
						{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1())}?
					)
				)
					{ 
					  getUnorderedGroupHelper().leave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_1());
					}
			)
			    |
			(
				(
					(
						{ 
						  getUnorderedGroupHelper().enter(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0());
						}
						(
							(
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0(), 0)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0(), 0);
							}
										({true}?=>((
											otherlv_50=Secrets
											{
												newLeafNode(otherlv_50, grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsKeyword_3_2_0_0_0_0());
											}
											otherlv_51=Colon
											{
												newLeafNode(otherlv_51, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_2_0_0_0_1());
											}
											(
												this_NEWLINE_52=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_52, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_2());
												}
											)+
											this_BEGIN_53=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_53, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_0_3());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getSecretsSecretParserRuleCall_3_2_0_0_0_4_0());
													}
													lv_secrets_54_0=ruleSecret
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
														}
														add(
															$current,
															"secrets",
															lv_secrets_54_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Secret");
														afterParserOrEnumRuleCall();
													}
												)
											)+
											this_END_55=RULE_END
											{
												newLeafNode(this_END_55, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_2_0_0_0_5());
											}
											(
												this_NEWLINE_56=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_56, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_0_6());
												}
											)*
										)
										(
											otherlv_57=Inputs
											{
												newLeafNode(otherlv_57, grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsKeyword_3_2_0_0_1_0());
											}
											otherlv_58=Colon
											{
												newLeafNode(otherlv_58, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_2_0_0_1_1());
											}
											(
												this_NEWLINE_59=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_59, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_2());
												}
											)+
											this_BEGIN_60=RULE_BEGIN
											{
												newLeafNode(this_BEGIN_60, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_2_0_0_1_3());
											}
											(
												(
													{
														newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getInputsInputParserRuleCall_3_2_0_0_1_4_0());
													}
													lv_inputs_61_0=ruleInput
													{
														if ($current==null) {
															$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
														}
														add(
															$current,
															"inputs",
															lv_inputs_61_0,
															"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Input");
														afterParserOrEnumRuleCall();
													}
												)
											)+
											this_END_62=RULE_END
											{
												newLeafNode(this_END_62, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_2_0_0_1_5());
											}
											(
												this_NEWLINE_63=RULE_NEWLINE
												{
													newLeafNode(this_NEWLINE_63, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_0_1_6());
												}
											)*
										)?
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)|
					(
						{getUnorderedGroupHelper().canSelect(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0(), 1)}?=>(
							{
								getUnorderedGroupHelper().select(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0(), 1);
							}
										({true}?=>(otherlv_64=Outputs
										{
											newLeafNode(otherlv_64, grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsKeyword_3_2_0_1_0());
										}
										otherlv_65=Colon
										{
											newLeafNode(otherlv_65, grammarAccess.getOptionedWorkflowCallTriggerAccess().getColonKeyword_3_2_0_1_1());
										}
										(
											this_NEWLINE_66=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_66, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_2());
											}
										)+
										this_BEGIN_67=RULE_BEGIN
										{
											newLeafNode(this_BEGIN_67, grammarAccess.getOptionedWorkflowCallTriggerAccess().getBEGINTerminalRuleCall_3_2_0_1_3());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getOptionedWorkflowCallTriggerAccess().getOutputsOutputParserRuleCall_3_2_0_1_4_0());
												}
												lv_outputs_68_0=ruleOutput
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getOptionedWorkflowCallTriggerRule());
													}
													add(
														$current,
														"outputs",
														lv_outputs_68_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Output");
													afterParserOrEnumRuleCall();
												}
											)
										)+
										this_END_69=RULE_END
										{
											newLeafNode(this_END_69, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_2_0_1_5());
										}
										(
											this_NEWLINE_70=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_70, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_0_1_6());
											}
										)*
										))
							{ 
								getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0());
							}
						)
					)
							)+
							{getUnorderedGroupHelper().canLeave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0())}?
						)
					)
						{ 
						  getUnorderedGroupHelper().leave(grammarAccess.getOptionedWorkflowCallTriggerAccess().getUnorderedGroup_3_2_0());
						}
				)
				this_END_71=RULE_END
				{
					newLeafNode(this_END_71, grammarAccess.getOptionedWorkflowCallTriggerAccess().getENDTerminalRuleCall_3_2_1());
				}
				(
					this_NEWLINE_72=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_72, grammarAccess.getOptionedWorkflowCallTriggerAccess().getNEWLINETerminalRuleCall_3_2_2());
					}
				)*
			)
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleOptionedWorkflowDispatchTrigger
entryRuleOptionedWorkflowDispatchTrigger returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOptionedWorkflowDispatchTriggerRule()); }
	iv_ruleOptionedWorkflowDispatchTrigger=ruleOptionedWorkflowDispatchTrigger
	{ $current=$iv_ruleOptionedWorkflowDispatchTrigger.current; }
	EOF;

// Rule OptionedWorkflowDispatchTrigger
ruleOptionedWorkflowDispatchTrigger returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getWorkflowDispatchTriggerAction_0(),
					$current);
			}
		)
		otherlv_1=Workflow_dispatch
		{
			newLeafNode(otherlv_1, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getWorkflow_dispatchKeyword_1());
		}
		otherlv_2=Colon
		{
			newLeafNode(otherlv_2, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getColonKeyword_2());
		}
		(
			this_NEWLINE_3=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_3, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getNEWLINETerminalRuleCall_3());
			}
		)+
		this_BEGIN_4=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_4, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getBEGINTerminalRuleCall_4());
		}
		(
			otherlv_5=Inputs
			{
				newLeafNode(otherlv_5, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getInputsKeyword_5_0());
			}
			otherlv_6=Colon
			{
				newLeafNode(otherlv_6, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getColonKeyword_5_1());
			}
			(
				this_NEWLINE_7=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_7, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getNEWLINETerminalRuleCall_5_2());
				}
			)+
			this_BEGIN_8=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_8, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getBEGINTerminalRuleCall_5_3());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getInputsInputParserRuleCall_5_4_0());
					}
					lv_inputs_9_0=ruleInput
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getOptionedWorkflowDispatchTriggerRule());
						}
						add(
							$current,
							"inputs",
							lv_inputs_9_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Input");
						afterParserOrEnumRuleCall();
					}
				)
			)+
			this_END_10=RULE_END
			{
				newLeafNode(this_END_10, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getENDTerminalRuleCall_5_5());
			}
			(
				this_NEWLINE_11=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_11, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getNEWLINETerminalRuleCall_5_6());
				}
			)*
		)
		this_END_12=RULE_END
		{
			newLeafNode(this_END_12, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getENDTerminalRuleCall_6());
		}
		(
			this_NEWLINE_13=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_13, grammarAccess.getOptionedWorkflowDispatchTriggerAccess().getNEWLINETerminalRuleCall_7());
			}
		)*
	)
;

// Entry rule entryRuleInput
entryRuleInput returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getInputAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getInputRule()); }
	iv_ruleInput=ruleInput
	{ $current=$iv_ruleInput.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Input
ruleInput returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getInputAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getInputAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getInputAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getInputAccess().getUnorderedGroup(), 0);
				}
							({true}?=>((
								(
									{
										newCompositeNode(grammarAccess.getInputAccess().getIdYAMLIDParserRuleCall_0_0_0());
									}
									lv_id_1_0=ruleYAMLID
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getInputRule());
										}
										set(
											$current,
											"id",
											lv_id_1_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
										afterParserOrEnumRuleCall();
									}
								)
							)
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getInputAccess().getColonKeyword_0_1());
							}
							(
								this_NEWLINE_3=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_3, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_0_2());
								}
							)+
							this_BEGIN_4=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_4, grammarAccess.getInputAccess().getBEGINTerminalRuleCall_0_3());
							}
							(
								otherlv_5=Description
								{
									newLeafNode(otherlv_5, grammarAccess.getInputAccess().getDescriptionKeyword_0_4_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getInputAccess().getColonKeyword_0_4_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getInputAccess().getDescriptionExpressionParserRuleCall_0_4_2_0());
										}
										lv_description_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getInputRule());
											}
											set(
												$current,
												"description",
												lv_description_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_8=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_8, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_0_4_3());
									}
								)+
							)?
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getInputAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getInputAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getInputAccess().getUnorderedGroup(), 1);
				}
							({true}?=>(otherlv_9=Default
							{
								newLeafNode(otherlv_9, grammarAccess.getInputAccess().getDefaultKeyword_1_0());
							}
							otherlv_10=Colon
							{
								newLeafNode(otherlv_10, grammarAccess.getInputAccess().getColonKeyword_1_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getInputAccess().getDefaultExpressionParserRuleCall_1_2_0());
									}
									lv_default_11_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getInputRule());
										}
										set(
											$current,
											"default",
											lv_default_11_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_12=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_12, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_1_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getInputAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getInputAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getInputAccess().getUnorderedGroup(), 2);
				}
							({true}?=>(otherlv_13=Required
							{
								newLeafNode(otherlv_13, grammarAccess.getInputAccess().getRequiredKeyword_2_0());
							}
							otherlv_14=Colon
							{
								newLeafNode(otherlv_14, grammarAccess.getInputAccess().getColonKeyword_2_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getInputAccess().getIsRequiredExpressionParserRuleCall_2_2_0());
									}
									lv_isRequired_15_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getInputRule());
										}
										set(
											$current,
											"isRequired",
											lv_isRequired_15_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_16=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_16, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_2_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getInputAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getInputAccess().getUnorderedGroup(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getInputAccess().getUnorderedGroup(), 3);
				}
							({true}?=>(otherlv_17=Type
							{
								newLeafNode(otherlv_17, grammarAccess.getInputAccess().getTypeKeyword_3_0());
							}
							otherlv_18=Colon
							{
								newLeafNode(otherlv_18, grammarAccess.getInputAccess().getColonKeyword_3_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getInputAccess().getTypeTYPEEnumRuleCall_3_2_0());
									}
									lv_type_19_0=ruleTYPE
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getInputRule());
										}
										set(
											$current,
											"type",
											lv_type_19_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.TYPE");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_20=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_20, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_3_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getInputAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getInputAccess().getUnorderedGroup(), 4)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getInputAccess().getUnorderedGroup(), 4);
				}
							({true}?=>((
								(
									otherlv_21=Options
									{
										newLeafNode(otherlv_21, grammarAccess.getInputAccess().getOptionsKeyword_4_0_0_0());
									}
									otherlv_22=Colon
									{
										newLeafNode(otherlv_22, grammarAccess.getInputAccess().getColonKeyword_4_0_0_1());
									}
									(
										(
											(
												lv_options_23_0=RULE_STRING
												{
													newLeafNode(lv_options_23_0, grammarAccess.getInputAccess().getOptionsSTRINGTerminalRuleCall_4_0_0_2_0_0());
												}
												{
													if ($current==null) {
														$current = createModelElement(grammarAccess.getInputRule());
													}
													addWithLastConsumed(
														$current,
														"options",
														lv_options_23_0,
														"org.eclipse.xtext.common.Terminals.STRING");
												}
											)
										)
										(
											this_NEWLINE_24=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_24, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_0_0_2_1());
											}
										)+
									)
								)
								    |
								(
									otherlv_25=LeftSquareBracket
									{
										newLeafNode(otherlv_25, grammarAccess.getInputAccess().getLeftSquareBracketKeyword_4_0_1_0());
									}
									(
										(
											lv_options_26_0=RULE_STRING
											{
												newLeafNode(lv_options_26_0, grammarAccess.getInputAccess().getOptionsSTRINGTerminalRuleCall_4_0_1_1_0());
											}
											{
												if ($current==null) {
													$current = createModelElement(grammarAccess.getInputRule());
												}
												addWithLastConsumed(
													$current,
													"options",
													lv_options_26_0,
													"org.eclipse.xtext.common.Terminals.STRING");
											}
										)
									)
									(
										otherlv_27=Comma
										{
											newLeafNode(otherlv_27, grammarAccess.getInputAccess().getCommaKeyword_4_0_1_2_0());
										}
										(
											(
												lv_options_28_0=RULE_STRING
												{
													newLeafNode(lv_options_28_0, grammarAccess.getInputAccess().getOptionsSTRINGTerminalRuleCall_4_0_1_2_1_0());
												}
												{
													if ($current==null) {
														$current = createModelElement(grammarAccess.getInputRule());
													}
													addWithLastConsumed(
														$current,
														"options",
														lv_options_28_0,
														"org.eclipse.xtext.common.Terminals.STRING");
												}
											)
										)
									)*
									otherlv_29=RightSquareBracket
									{
										newLeafNode(otherlv_29, grammarAccess.getInputAccess().getRightSquareBracketKeyword_4_0_1_3());
									}
									(
										this_NEWLINE_30=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_30, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_0_1_4());
										}
									)+
								)
								    |
								(
									(
										this_NEWLINE_31=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_31, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_0_2_0());
										}
									)+
									this_BEGIN_32=RULE_BEGIN
									{
										newLeafNode(this_BEGIN_32, grammarAccess.getInputAccess().getBEGINTerminalRuleCall_4_0_2_1());
									}
									(
										otherlv_33=HyphenMinus
										{
											newLeafNode(otherlv_33, grammarAccess.getInputAccess().getHyphenMinusKeyword_4_0_2_2_0());
										}
										(
											(
												lv_options_34_0=RULE_STRING
												{
													newLeafNode(lv_options_34_0, grammarAccess.getInputAccess().getOptionsSTRINGTerminalRuleCall_4_0_2_2_1_0());
												}
												{
													if ($current==null) {
														$current = createModelElement(grammarAccess.getInputRule());
													}
													addWithLastConsumed(
														$current,
														"options",
														lv_options_34_0,
														"org.eclipse.xtext.common.Terminals.STRING");
												}
											)
										)
										(
											this_NEWLINE_35=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_35, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_0_2_2_2());
											}
										)+
									)+
									this_END_36=RULE_END
									{
										newLeafNode(this_END_36, grammarAccess.getInputAccess().getENDTerminalRuleCall_4_0_2_3());
									}
									(
										this_NEWLINE_37=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_37, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_0_2_4());
										}
									)*
								)
							)
							this_END_38=RULE_END
							{
								newLeafNode(this_END_38, grammarAccess.getInputAccess().getENDTerminalRuleCall_4_1());
							}
							(
								this_NEWLINE_39=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_39, grammarAccess.getInputAccess().getNEWLINETerminalRuleCall_4_2());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getInputAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getInputAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getInputAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleOutput
entryRuleOutput returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOutputAccess().getUnorderedGroup()
	);
}:
	{ newCompositeNode(grammarAccess.getOutputRule()); }
	iv_ruleOutput=ruleOutput
	{ $current=$iv_ruleOutput.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Output
ruleOutput returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getOutputAccess().getUnorderedGroup()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getOutputAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getOutputAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getOutputAccess().getUnorderedGroup(), 0);
				}
							({true}?=>((
								(
									{
										newCompositeNode(grammarAccess.getOutputAccess().getIdYAMLIDParserRuleCall_0_0_0());
									}
									lv_id_1_0=ruleYAMLID
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getOutputRule());
										}
										set(
											$current,
											"id",
											lv_id_1_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
										afterParserOrEnumRuleCall();
									}
								)
							)
							otherlv_2=Colon
							{
								newLeafNode(otherlv_2, grammarAccess.getOutputAccess().getColonKeyword_0_1());
							}
							(
								this_NEWLINE_3=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_3, grammarAccess.getOutputAccess().getNEWLINETerminalRuleCall_0_2());
								}
							)+
							this_BEGIN_4=RULE_BEGIN
							{
								newLeafNode(this_BEGIN_4, grammarAccess.getOutputAccess().getBEGINTerminalRuleCall_0_3());
							}
							(
								otherlv_5=Description
								{
									newLeafNode(otherlv_5, grammarAccess.getOutputAccess().getDescriptionKeyword_0_4_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getOutputAccess().getColonKeyword_0_4_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getOutputAccess().getDescriptionExpressionParserRuleCall_0_4_2_0());
										}
										lv_description_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getOutputRule());
											}
											set(
												$current,
												"description",
												lv_description_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_8=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_8, grammarAccess.getOutputAccess().getNEWLINETerminalRuleCall_0_4_3());
									}
								)+
							)?
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOutputAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getOutputAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getOutputAccess().getUnorderedGroup(), 1);
				}
							({true}?=>((
								otherlv_9=Value
								{
									newLeafNode(otherlv_9, grammarAccess.getOutputAccess().getValueKeyword_1_0_0());
								}
								otherlv_10=Colon
								{
									newLeafNode(otherlv_10, grammarAccess.getOutputAccess().getColonKeyword_1_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getOutputAccess().getValueExpressionParserRuleCall_1_0_2_0());
										}
										lv_value_11_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getOutputRule());
											}
											set(
												$current,
												"value",
												lv_value_11_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_12=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_12, grammarAccess.getOutputAccess().getNEWLINETerminalRuleCall_1_0_3());
									}
								)+
							)
							this_END_13=RULE_END
							{
								newLeafNode(this_END_13, grammarAccess.getOutputAccess().getENDTerminalRuleCall_1_1());
							}
							(
								this_NEWLINE_14=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_14, grammarAccess.getOutputAccess().getNEWLINETerminalRuleCall_1_2());
								}
							)*
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getOutputAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getOutputAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getOutputAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleSecret
entryRuleSecret returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 
	grammarAccess.getSecretAccess().getUnorderedGroup_1_0()
	);
}:
	{ newCompositeNode(grammarAccess.getSecretRule()); }
	iv_ruleSecret=ruleSecret
	{ $current=$iv_ruleSecret.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Secret
ruleSecret returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 
	grammarAccess.getSecretAccess().getUnorderedGroup_1_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			(
				(
					{
						newCompositeNode(grammarAccess.getSecretAccess().getIdYAMLIDParserRuleCall_0_0_0());
					}
					lv_id_0_0=ruleYAMLID
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getSecretRule());
						}
						set(
							$current,
							"id",
							lv_id_0_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
						afterParserOrEnumRuleCall();
					}
				)
			)
			otherlv_1=Colon
			{
				newLeafNode(otherlv_1, grammarAccess.getSecretAccess().getColonKeyword_0_1());
			}
			(
				this_NEWLINE_2=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_2, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_0_2());
				}
			)+
			this_BEGIN_3=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_3, grammarAccess.getSecretAccess().getBEGINTerminalRuleCall_0_3());
			}
			(
				(
					{ 
					  getUnorderedGroupHelper().enter(grammarAccess.getSecretAccess().getUnorderedGroup_0_4());
					}
					(
						(
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 0)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 0);
						}
									({true}?=>(otherlv_5=Description
									{
										newLeafNode(otherlv_5, grammarAccess.getSecretAccess().getDescriptionKeyword_0_4_0_0());
									}
									otherlv_6=Colon
									{
										newLeafNode(otherlv_6, grammarAccess.getSecretAccess().getColonKeyword_0_4_0_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getSecretAccess().getDescriptionExpressionParserRuleCall_0_4_0_2_0());
											}
											lv_description_7_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getSecretRule());
												}
												set(
													$current,
													"description",
													lv_description_7_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										this_NEWLINE_8=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_8, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_0_4_0_3());
										}
									)+
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getSecretAccess().getUnorderedGroup_0_4());
						}
					)
				)|
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 1)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getSecretAccess().getUnorderedGroup_0_4(), 1);
						}
									({true}?=>(otherlv_9=Required
									{
										newLeafNode(otherlv_9, grammarAccess.getSecretAccess().getRequiredKeyword_0_4_1_0());
									}
									otherlv_10=Colon
									{
										newLeafNode(otherlv_10, grammarAccess.getSecretAccess().getColonKeyword_0_4_1_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getSecretAccess().getIsRequiredExpressionParserRuleCall_0_4_1_2_0());
											}
											lv_isRequired_11_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getSecretRule());
												}
												set(
													$current,
													"isRequired",
													lv_isRequired_11_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										this_NEWLINE_12=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_12, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_0_4_1_3());
										}
									)+
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getSecretAccess().getUnorderedGroup_0_4());
						}
					)
				)
						)+
						{getUnorderedGroupHelper().canLeave(grammarAccess.getSecretAccess().getUnorderedGroup_0_4())}?
					)
				)
					{ 
					  getUnorderedGroupHelper().leave(grammarAccess.getSecretAccess().getUnorderedGroup_0_4());
					}
			)
		)
		    |
		(
			(
				(
					{ 
					  getUnorderedGroupHelper().enter(grammarAccess.getSecretAccess().getUnorderedGroup_1_0());
					}
					(
						(
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getSecretAccess().getUnorderedGroup_1_0(), 0)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getSecretAccess().getUnorderedGroup_1_0(), 0);
						}
									({true}?=>(otherlv_14=Required
									{
										newLeafNode(otherlv_14, grammarAccess.getSecretAccess().getRequiredKeyword_1_0_0_0());
									}
									otherlv_15=Colon
									{
										newLeafNode(otherlv_15, grammarAccess.getSecretAccess().getColonKeyword_1_0_0_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getSecretAccess().getIsRequiredExpressionParserRuleCall_1_0_0_2_0());
											}
											lv_isRequired_16_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getSecretRule());
												}
												set(
													$current,
													"isRequired",
													lv_isRequired_16_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										this_NEWLINE_17=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_17, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_1_0_0_3());
										}
									)+
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getSecretAccess().getUnorderedGroup_1_0());
						}
					)
				)|
				(
					{getUnorderedGroupHelper().canSelect(grammarAccess.getSecretAccess().getUnorderedGroup_1_0(), 1)}?=>(
						{
							getUnorderedGroupHelper().select(grammarAccess.getSecretAccess().getUnorderedGroup_1_0(), 1);
						}
									({true}?=>(otherlv_18=Description
									{
										newLeafNode(otherlv_18, grammarAccess.getSecretAccess().getDescriptionKeyword_1_0_1_0());
									}
									otherlv_19=Colon
									{
										newLeafNode(otherlv_19, grammarAccess.getSecretAccess().getColonKeyword_1_0_1_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getSecretAccess().getDescriptionExpressionParserRuleCall_1_0_1_2_0());
											}
											lv_description_20_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getSecretRule());
												}
												set(
													$current,
													"description",
													lv_description_20_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										this_NEWLINE_21=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_21, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_1_0_1_3());
										}
									)+
									))
						{ 
							getUnorderedGroupHelper().returnFromSelection(grammarAccess.getSecretAccess().getUnorderedGroup_1_0());
						}
					)
				)
						)+
						{getUnorderedGroupHelper().canLeave(grammarAccess.getSecretAccess().getUnorderedGroup_1_0())}?
					)
				)
					{ 
					  getUnorderedGroupHelper().leave(grammarAccess.getSecretAccess().getUnorderedGroup_1_0());
					}
			)
			this_END_22=RULE_END
			{
				newLeafNode(this_END_22, grammarAccess.getSecretAccess().getENDTerminalRuleCall_1_1());
			}
			(
				this_NEWLINE_23=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_23, grammarAccess.getSecretAccess().getNEWLINETerminalRuleCall_1_2());
				}
			)*
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRulePermission
entryRulePermission returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getPermissionRule()); }
	iv_rulePermission=rulePermission
	{ $current=$iv_rulePermission.current; }
	EOF;

// Rule Permission
rulePermission returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					newCompositeNode(grammarAccess.getPermissionAccess().getKeyPERMISSION_SCOPEEnumRuleCall_0_0());
				}
				lv_key_0_0=rulePERMISSION_SCOPE
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getPermissionRule());
					}
					set(
						$current,
						"key",
						lv_key_0_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.PERMISSION_SCOPE");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getPermissionAccess().getColonKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getPermissionAccess().getValuePERMISSION_VALUEEnumRuleCall_2_0());
				}
				lv_value_2_0=rulePERMISSION_VALUE
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getPermissionRule());
					}
					set(
						$current,
						"value",
						lv_value_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.PERMISSION_VALUE");
					afterParserOrEnumRuleCall();
				}
			)
		)
		(
			this_NEWLINE_3=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_3, grammarAccess.getPermissionAccess().getNEWLINETerminalRuleCall_3());
			}
		)+
	)
;

// Entry rule entryRuleDefaults
entryRuleDefaults returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getDefaultsAccess().getUnorderedGroup_4()
	);
}:
	{ newCompositeNode(grammarAccess.getDefaultsRule()); }
	iv_ruleDefaults=ruleDefaults
	{ $current=$iv_ruleDefaults.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Defaults
ruleDefaults returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getDefaultsAccess().getUnorderedGroup_4()
	);
}
@after {
	leaveRule();
}:
	(
		otherlv_0=Run
		{
			newLeafNode(otherlv_0, grammarAccess.getDefaultsAccess().getRunKeyword_0());
		}
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getDefaultsAccess().getColonKeyword_1());
		}
		(
			this_NEWLINE_2=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_2, grammarAccess.getDefaultsAccess().getNEWLINETerminalRuleCall_2());
			}
		)+
		this_BEGIN_3=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_3, grammarAccess.getDefaultsAccess().getBEGINTerminalRuleCall_3());
		}
		(
			(
				{ 
				  getUnorderedGroupHelper().enter(grammarAccess.getDefaultsAccess().getUnorderedGroup_4());
				}
				(
					(
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getDefaultsAccess().getUnorderedGroup_4(), 0)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getDefaultsAccess().getUnorderedGroup_4(), 0);
					}
								({true}?=>(otherlv_5=Shell
								{
									newLeafNode(otherlv_5, grammarAccess.getDefaultsAccess().getShellKeyword_4_0_0());
								}
								otherlv_6=Colon
								{
									newLeafNode(otherlv_6, grammarAccess.getDefaultsAccess().getColonKeyword_4_0_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getDefaultsAccess().getShellExpressionParserRuleCall_4_0_2_0());
										}
										lv_shell_7_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getDefaultsRule());
											}
											set(
												$current,
												"shell",
												lv_shell_7_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_8=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_8, grammarAccess.getDefaultsAccess().getNEWLINETerminalRuleCall_4_0_3());
									}
								)+
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getDefaultsAccess().getUnorderedGroup_4());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getDefaultsAccess().getUnorderedGroup_4(), 1)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getDefaultsAccess().getUnorderedGroup_4(), 1);
					}
								({true}?=>(otherlv_9=WorkingDirectory
								{
									newLeafNode(otherlv_9, grammarAccess.getDefaultsAccess().getWorkingDirectoryKeyword_4_1_0());
								}
								otherlv_10=Colon
								{
									newLeafNode(otherlv_10, grammarAccess.getDefaultsAccess().getColonKeyword_4_1_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getDefaultsAccess().getWorkingDirectoryExpressionParserRuleCall_4_1_2_0());
										}
										lv_workingDirectory_11_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getDefaultsRule());
											}
											set(
												$current,
												"workingDirectory",
												lv_workingDirectory_11_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_12=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_12, grammarAccess.getDefaultsAccess().getNEWLINETerminalRuleCall_4_1_3());
									}
								)+
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getDefaultsAccess().getUnorderedGroup_4());
					}
				)
			)
					)+
					{getUnorderedGroupHelper().canLeave(grammarAccess.getDefaultsAccess().getUnorderedGroup_4())}?
				)
			)
				{ 
				  getUnorderedGroupHelper().leave(grammarAccess.getDefaultsAccess().getUnorderedGroup_4());
				}
		)
		this_END_13=RULE_END
		{
			newLeafNode(this_END_13, grammarAccess.getDefaultsAccess().getENDTerminalRuleCall_5());
		}
		(
			this_NEWLINE_14=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_14, grammarAccess.getDefaultsAccess().getNEWLINETerminalRuleCall_6());
			}
		)*
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleStagingEnvironment
entryRuleStagingEnvironment returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getStagingEnvironmentRule()); }
	iv_ruleStagingEnvironment=ruleStagingEnvironment
	{ $current=$iv_ruleStagingEnvironment.current; }
	EOF;

// Rule StagingEnvironment
ruleStagingEnvironment returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				(
					{
						newCompositeNode(grammarAccess.getStagingEnvironmentAccess().getNameExpressionParserRuleCall_0_0_0());
					}
					lv_name_0_0=ruleExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getStagingEnvironmentRule());
						}
						set(
							$current,
							"name",
							lv_name_0_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				this_NEWLINE_1=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_1, grammarAccess.getStagingEnvironmentAccess().getNEWLINETerminalRuleCall_0_1());
				}
			)+
		)
		    |
		(
			(
				this_NEWLINE_2=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_2, grammarAccess.getStagingEnvironmentAccess().getNEWLINETerminalRuleCall_1_0());
				}
			)+
			this_BEGIN_3=RULE_BEGIN
			{
				newLeafNode(this_BEGIN_3, grammarAccess.getStagingEnvironmentAccess().getBEGINTerminalRuleCall_1_1());
			}
			(
				otherlv_4=Name
				{
					newLeafNode(otherlv_4, grammarAccess.getStagingEnvironmentAccess().getNameKeyword_1_2_0());
				}
				otherlv_5=Colon
				{
					newLeafNode(otherlv_5, grammarAccess.getStagingEnvironmentAccess().getColonKeyword_1_2_1());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getStagingEnvironmentAccess().getNameExpressionParserRuleCall_1_2_2_0());
						}
						lv_name_6_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getStagingEnvironmentRule());
							}
							set(
								$current,
								"name",
								lv_name_6_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_7=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_7, grammarAccess.getStagingEnvironmentAccess().getNEWLINETerminalRuleCall_1_2_3());
					}
				)+
			)
			(
				otherlv_8=Url
				{
					newLeafNode(otherlv_8, grammarAccess.getStagingEnvironmentAccess().getUrlKeyword_1_3_0());
				}
				otherlv_9=Colon
				{
					newLeafNode(otherlv_9, grammarAccess.getStagingEnvironmentAccess().getColonKeyword_1_3_1());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getStagingEnvironmentAccess().getUrlExpressionParserRuleCall_1_3_2_0());
						}
						lv_url_10_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getStagingEnvironmentRule());
							}
							set(
								$current,
								"url",
								lv_url_10_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_11=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_11, grammarAccess.getStagingEnvironmentAccess().getNEWLINETerminalRuleCall_1_3_3());
					}
				)+
			)?
			this_END_12=RULE_END
			{
				newLeafNode(this_END_12, grammarAccess.getStagingEnvironmentAccess().getENDTerminalRuleCall_1_4());
			}
			(
				this_NEWLINE_13=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_13, grammarAccess.getStagingEnvironmentAccess().getNEWLINETerminalRuleCall_1_5());
				}
			)*
		)
	)
;

// Entry rule entryRuleStrategy
entryRuleStrategy returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getStrategyAccess().getUnorderedGroup(), 
	grammarAccess.getStrategyAccess().getUnorderedGroup_0()
	);
}:
	{ newCompositeNode(grammarAccess.getStrategyRule()); }
	iv_ruleStrategy=ruleStrategy
	{ $current=$iv_ruleStrategy.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Strategy
ruleStrategy returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getStrategyAccess().getUnorderedGroup(), 
	grammarAccess.getStrategyAccess().getUnorderedGroup_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			{ 
			  getUnorderedGroupHelper().enter(grammarAccess.getStrategyAccess().getUnorderedGroup());
			}
			(
				(
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup(), 0);
				}
							({true}?=>((
								{ 
								  getUnorderedGroupHelper().enter(grammarAccess.getStrategyAccess().getUnorderedGroup_0());
								}
								(
									(
							(
								{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 0)}?=>(
									{
										getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 0);
									}
												({true}?=>(otherlv_2=Matrix
												{
													newLeafNode(otherlv_2, grammarAccess.getStrategyAccess().getMatrixKeyword_0_0_0());
												}
												otherlv_3=Colon
												{
													newLeafNode(otherlv_3, grammarAccess.getStrategyAccess().getColonKeyword_0_0_1());
												}
												(
													this_NEWLINE_4=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_4, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_0_2());
													}
												)+
												this_BEGIN_5=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_5, grammarAccess.getStrategyAccess().getBEGINTerminalRuleCall_0_0_3());
												}
												(
													otherlv_6=Includes
													{
														newLeafNode(otherlv_6, grammarAccess.getStrategyAccess().getIncludesKeyword_0_0_4_0());
													}
													otherlv_7=Colon
													{
														newLeafNode(otherlv_7, grammarAccess.getStrategyAccess().getColonKeyword_0_0_4_1());
													}
													(
														this_NEWLINE_8=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_8, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_0_4_2());
														}
													)+
													this_BEGIN_9=RULE_BEGIN
													{
														newLeafNode(this_BEGIN_9, grammarAccess.getStrategyAccess().getBEGINTerminalRuleCall_0_0_4_3());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getStrategyAccess().getIncludesMatrixCombinationParserRuleCall_0_0_4_4_0());
															}
															lv_includes_10_0=ruleMatrixCombination
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getStrategyRule());
																}
																add(
																	$current,
																	"includes",
																	lv_includes_10_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.MatrixCombination");
																afterParserOrEnumRuleCall();
															}
														)
													)
													this_END_11=RULE_END
													{
														newLeafNode(this_END_11, grammarAccess.getStrategyAccess().getENDTerminalRuleCall_0_0_4_5());
													}
													(
														this_NEWLINE_12=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_12, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_0_4_6());
														}
													)*
												)?
												))
									{ 
										getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup_0());
									}
								)
							)|
							(
								{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 1)}?=>(
									{
										getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 1);
									}
												({true}?=>(otherlv_13=Exludes
												{
													newLeafNode(otherlv_13, grammarAccess.getStrategyAccess().getExludesKeyword_0_1_0());
												}
												otherlv_14=Colon
												{
													newLeafNode(otherlv_14, grammarAccess.getStrategyAccess().getColonKeyword_0_1_1());
												}
												(
													this_NEWLINE_15=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_15, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_1_2());
													}
												)+
												this_BEGIN_16=RULE_BEGIN
												{
													newLeafNode(this_BEGIN_16, grammarAccess.getStrategyAccess().getBEGINTerminalRuleCall_0_1_3());
												}
												(
													(
														{
															newCompositeNode(grammarAccess.getStrategyAccess().getExcludesMatrixCombinationParserRuleCall_0_1_4_0());
														}
														lv_excludes_17_0=ruleMatrixCombination
														{
															if ($current==null) {
																$current = createModelElementForParent(grammarAccess.getStrategyRule());
															}
															add(
																$current,
																"excludes",
																lv_excludes_17_0,
																"d.fe.up.pt.cicd.gha.dsl.GitHubActions.MatrixCombination");
															afterParserOrEnumRuleCall();
														}
													)
												)
												this_END_18=RULE_END
												{
													newLeafNode(this_END_18, grammarAccess.getStrategyAccess().getENDTerminalRuleCall_0_1_5());
												}
												(
													this_NEWLINE_19=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_19, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_1_6());
													}
												)*
												))
									{ 
										getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup_0());
									}
								)
							)|
							(
								{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 2)}?=>(
									{
										getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup_0(), 2);
									}
												({true}?=>((
													this_BEGIN_20=RULE_BEGIN
													{
														newLeafNode(this_BEGIN_20, grammarAccess.getStrategyAccess().getBEGINTerminalRuleCall_0_2_0_0());
													}
													(
														(
															{
																newCompositeNode(grammarAccess.getStrategyAccess().getAxesMatrixAxisParserRuleCall_0_2_0_1_0());
															}
															lv_axes_21_0=ruleMatrixAxis
															{
																if ($current==null) {
																	$current = createModelElementForParent(grammarAccess.getStrategyRule());
																}
																add(
																	$current,
																	"axes",
																	lv_axes_21_0,
																	"d.fe.up.pt.cicd.gha.dsl.GitHubActions.MatrixAxis");
																afterParserOrEnumRuleCall();
															}
														)
													)+
													this_END_22=RULE_END
													{
														newLeafNode(this_END_22, grammarAccess.getStrategyAccess().getENDTerminalRuleCall_0_2_0_2());
													}
													(
														this_NEWLINE_23=RULE_NEWLINE
														{
															newLeafNode(this_NEWLINE_23, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_2_0_3());
														}
													)*
												)
												this_END_24=RULE_END
												{
													newLeafNode(this_END_24, grammarAccess.getStrategyAccess().getENDTerminalRuleCall_0_2_1());
												}
												(
													this_NEWLINE_25=RULE_NEWLINE
													{
														newLeafNode(this_NEWLINE_25, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_0_2_2());
													}
												)*
												))
									{ 
										getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup_0());
									}
								)
							)
									)+
									{getUnorderedGroupHelper().canLeave(grammarAccess.getStrategyAccess().getUnorderedGroup_0())}?
								)
							)
								{ 
								  getUnorderedGroupHelper().leave(grammarAccess.getStrategyAccess().getUnorderedGroup_0());
								}
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup(), 1);
				}
							({true}?=>(otherlv_26=FailFast
							{
								newLeafNode(otherlv_26, grammarAccess.getStrategyAccess().getFailFastKeyword_1_0());
							}
							otherlv_27=Colon
							{
								newLeafNode(otherlv_27, grammarAccess.getStrategyAccess().getColonKeyword_1_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getStrategyAccess().getFailFastExpressionParserRuleCall_1_2_0());
									}
									lv_failFast_28_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getStrategyRule());
										}
										set(
											$current,
											"failFast",
											lv_failFast_28_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_29=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_29, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_1_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup());
				}
			)
		)|
		(
			{getUnorderedGroupHelper().canSelect(grammarAccess.getStrategyAccess().getUnorderedGroup(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getStrategyAccess().getUnorderedGroup(), 2);
				}
							({true}?=>(otherlv_30=MaxParalell
							{
								newLeafNode(otherlv_30, grammarAccess.getStrategyAccess().getMaxParalellKeyword_2_0());
							}
							otherlv_31=Colon
							{
								newLeafNode(otherlv_31, grammarAccess.getStrategyAccess().getColonKeyword_2_1());
							}
							(
								(
									{
										newCompositeNode(grammarAccess.getStrategyAccess().getMaxParallelExpressionParserRuleCall_2_2_0());
									}
									lv_maxParallel_32_0=ruleExpression
									{
										if ($current==null) {
											$current = createModelElementForParent(grammarAccess.getStrategyRule());
										}
										set(
											$current,
											"maxParallel",
											lv_maxParallel_32_0,
											"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
										afterParserOrEnumRuleCall();
									}
								)
							)
							(
								this_NEWLINE_33=RULE_NEWLINE
								{
									newLeafNode(this_NEWLINE_33, grammarAccess.getStrategyAccess().getNEWLINETerminalRuleCall_2_3());
								}
							)+
							))
				{ 
					getUnorderedGroupHelper().returnFromSelection(grammarAccess.getStrategyAccess().getUnorderedGroup());
				}
			)
		)
				)+
				{getUnorderedGroupHelper().canLeave(grammarAccess.getStrategyAccess().getUnorderedGroup())}?
			)
		)
			{ 
			  getUnorderedGroupHelper().leave(grammarAccess.getStrategyAccess().getUnorderedGroup());
			}
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleMatrixAxis
entryRuleMatrixAxis returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getMatrixAxisRule()); }
	iv_ruleMatrixAxis=ruleMatrixAxis
	{ $current=$iv_ruleMatrixAxis.current; }
	EOF;

// Rule MatrixAxis
ruleMatrixAxis returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					newCompositeNode(grammarAccess.getMatrixAxisAccess().getNameYAMLIDParserRuleCall_0_0());
				}
				lv_name_0_0=ruleYAMLID
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getMatrixAxisRule());
					}
					set(
						$current,
						"name",
						lv_name_0_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getMatrixAxisAccess().getColonKeyword_1());
		}
		(
			(
				(
					(
						{
							newCompositeNode(grammarAccess.getMatrixAxisAccess().getCellsExpressionParserRuleCall_2_0_0_0());
						}
						lv_cells_2_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getMatrixAxisRule());
							}
							add(
								$current,
								"cells",
								lv_cells_2_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					this_NEWLINE_3=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_3, grammarAccess.getMatrixAxisAccess().getNEWLINETerminalRuleCall_2_0_1());
					}
				)+
			)
			    |
			(
				otherlv_4=LeftSquareBracket
				{
					newLeafNode(otherlv_4, grammarAccess.getMatrixAxisAccess().getLeftSquareBracketKeyword_2_1_0());
				}
				(
					(
						{
							newCompositeNode(grammarAccess.getMatrixAxisAccess().getCellsExpressionParserRuleCall_2_1_1_0());
						}
						lv_cells_5_0=ruleExpression
						{
							if ($current==null) {
								$current = createModelElementForParent(grammarAccess.getMatrixAxisRule());
							}
							add(
								$current,
								"cells",
								lv_cells_5_0,
								"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
							afterParserOrEnumRuleCall();
						}
					)
				)
				(
					otherlv_6=Comma
					{
						newLeafNode(otherlv_6, grammarAccess.getMatrixAxisAccess().getCommaKeyword_2_1_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getMatrixAxisAccess().getCellsExpressionParserRuleCall_2_1_2_1_0());
							}
							lv_cells_7_0=ruleExpression
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getMatrixAxisRule());
								}
								add(
									$current,
									"cells",
									lv_cells_7_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
								afterParserOrEnumRuleCall();
							}
						)
					)
				)*
				otherlv_8=RightSquareBracket
				{
					newLeafNode(otherlv_8, grammarAccess.getMatrixAxisAccess().getRightSquareBracketKeyword_2_1_3());
				}
				(
					this_NEWLINE_9=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_9, grammarAccess.getMatrixAxisAccess().getNEWLINETerminalRuleCall_2_1_4());
					}
				)+
			)
			    |
			(
				(
					this_NEWLINE_10=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_10, grammarAccess.getMatrixAxisAccess().getNEWLINETerminalRuleCall_2_2_0());
					}
				)+
				this_BEGIN_11=RULE_BEGIN
				{
					newLeafNode(this_BEGIN_11, grammarAccess.getMatrixAxisAccess().getBEGINTerminalRuleCall_2_2_1());
				}
				(
					otherlv_12=HyphenMinus
					{
						newLeafNode(otherlv_12, grammarAccess.getMatrixAxisAccess().getHyphenMinusKeyword_2_2_2_0());
					}
					(
						(
							{
								newCompositeNode(grammarAccess.getMatrixAxisAccess().getCellsExpressionParserRuleCall_2_2_2_1_0());
							}
							lv_cells_13_0=ruleExpression
							{
								if ($current==null) {
									$current = createModelElementForParent(grammarAccess.getMatrixAxisRule());
								}
								add(
									$current,
									"cells",
									lv_cells_13_0,
									"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
								afterParserOrEnumRuleCall();
							}
						)
					)
					(
						this_NEWLINE_14=RULE_NEWLINE
						{
							newLeafNode(this_NEWLINE_14, grammarAccess.getMatrixAxisAccess().getNEWLINETerminalRuleCall_2_2_2_2());
						}
					)+
				)+
				this_END_15=RULE_END
				{
					newLeafNode(this_END_15, grammarAccess.getMatrixAxisAccess().getENDTerminalRuleCall_2_2_3());
				}
				(
					this_NEWLINE_16=RULE_NEWLINE
					{
						newLeafNode(this_NEWLINE_16, grammarAccess.getMatrixAxisAccess().getNEWLINETerminalRuleCall_2_2_4());
					}
				)*
			)
		)
	)
;

// Entry rule entryRuleMatrixCombination
entryRuleMatrixCombination returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getMatrixCombinationRule()); }
	iv_ruleMatrixCombination=ruleMatrixCombination
	{ $current=$iv_ruleMatrixCombination.current; }
	EOF;

// Rule MatrixCombination
ruleMatrixCombination returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=HyphenMinus
		{
			newLeafNode(otherlv_0, grammarAccess.getMatrixCombinationAccess().getHyphenMinusKeyword_0());
		}
		this_BEGIN_1=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_1, grammarAccess.getMatrixCombinationAccess().getBEGINTerminalRuleCall_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getMatrixCombinationAccess().getEntriesVariableAssignmentParserRuleCall_2_0());
				}
				lv_entries_2_0=ruleVariableAssignment
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getMatrixCombinationRule());
					}
					add(
						$current,
						"entries",
						lv_entries_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
					afterParserOrEnumRuleCall();
				}
			)
		)
		this_END_3=RULE_END
		{
			newLeafNode(this_END_3, grammarAccess.getMatrixCombinationAccess().getENDTerminalRuleCall_3());
		}
	)
;

// Entry rule entryRuleContainer
entryRuleContainer returns [EObject current=null]@init {
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getContainerAccess().getUnorderedGroup_1(), 
	grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0()
	);
}:
	{ newCompositeNode(grammarAccess.getContainerRule()); }
	iv_ruleContainer=ruleContainer
	{ $current=$iv_ruleContainer.current; }
	EOF;
finally {
	myUnorderedGroupState.restore();
}

// Rule Container
ruleContainer returns [EObject current=null]
@init {
	enterRule();
	UnorderedGroupState myUnorderedGroupState = getUnorderedGroupHelper().snapShot(
	grammarAccess.getContainerAccess().getUnorderedGroup_1(), 
	grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0()
	);
}
@after {
	leaveRule();
}:
	(
		(
			(
				(
					{
						newCompositeNode(grammarAccess.getContainerAccess().getImageExpressionParserRuleCall_0_0_0());
					}
					lv_image_0_0=ruleExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getContainerRule());
						}
						set(
							$current,
							"image",
							lv_image_0_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				this_NEWLINE_1=RULE_NEWLINE
				{
					newLeafNode(this_NEWLINE_1, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_0_1());
				}
			)+
		)
		    |
		(
			(
				{ 
				  getUnorderedGroupHelper().enter(grammarAccess.getContainerAccess().getUnorderedGroup_1());
				}
				(
					(
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 0)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 0);
					}
								({true}?=>((
									this_NEWLINE_3=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_3, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_0_0());
									}
								)+
								this_BEGIN_4=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_4, grammarAccess.getContainerAccess().getBEGINTerminalRuleCall_1_0_1());
								}
								(
									otherlv_5=Image
									{
										newLeafNode(otherlv_5, grammarAccess.getContainerAccess().getImageKeyword_1_0_2_0());
									}
									otherlv_6=Colon
									{
										newLeafNode(otherlv_6, grammarAccess.getContainerAccess().getColonKeyword_1_0_2_1());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getContainerAccess().getImageExpressionParserRuleCall_1_0_2_2_0());
											}
											lv_image_7_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getContainerRule());
												}
												set(
													$current,
													"image",
													lv_image_7_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										this_NEWLINE_8=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_8, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_0_2_3());
										}
									)+
								)
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 1)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 1);
					}
								({true}?=>(otherlv_9=Env
								{
									newLeafNode(otherlv_9, grammarAccess.getContainerAccess().getEnvKeyword_1_1_0());
								}
								otherlv_10=Colon
								{
									newLeafNode(otherlv_10, grammarAccess.getContainerAccess().getColonKeyword_1_1_1());
								}
								this_BEGIN_11=RULE_BEGIN
								{
									newLeafNode(this_BEGIN_11, grammarAccess.getContainerAccess().getBEGINTerminalRuleCall_1_1_2());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getContainerAccess().getEnvironmentVariablesVariableAssignmentParserRuleCall_1_1_3_0());
										}
										lv_environmentVariables_12_0=ruleVariableAssignment
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getContainerRule());
											}
											add(
												$current,
												"environmentVariables",
												lv_environmentVariables_12_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableAssignment");
											afterParserOrEnumRuleCall();
										}
									)
								)+
								this_END_13=RULE_END
								{
									newLeafNode(this_END_13, grammarAccess.getContainerAccess().getENDTerminalRuleCall_1_1_4());
								}
								(
									this_NEWLINE_14=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_14, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_1_5());
									}
								)*
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 2)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 2);
					}
								({true}?=>((
									otherlv_15=Ports
									{
										newLeafNode(otherlv_15, grammarAccess.getContainerAccess().getPortsKeyword_1_2_0_0());
									}
									otherlv_16=Colon
									{
										newLeafNode(otherlv_16, grammarAccess.getContainerAccess().getColonKeyword_1_2_0_1());
									}
									(
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getPortsExpressionParserRuleCall_1_2_0_2_0_0());
												}
												lv_ports_17_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"ports",
														lv_ports_17_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											this_NEWLINE_18=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_18, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_2_0_2_1());
											}
										)+
									)
								)
								    |
								(
									otherlv_19=LeftSquareBracket
									{
										newLeafNode(otherlv_19, grammarAccess.getContainerAccess().getLeftSquareBracketKeyword_1_2_1_0());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getContainerAccess().getPortsExpressionParserRuleCall_1_2_1_1_0());
											}
											lv_ports_20_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getContainerRule());
												}
												add(
													$current,
													"ports",
													lv_ports_20_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										otherlv_21=Comma
										{
											newLeafNode(otherlv_21, grammarAccess.getContainerAccess().getCommaKeyword_1_2_1_2_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getPortsExpressionParserRuleCall_1_2_1_2_1_0());
												}
												lv_ports_22_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"ports",
														lv_ports_22_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
									)*
									otherlv_23=RightSquareBracket
									{
										newLeafNode(otherlv_23, grammarAccess.getContainerAccess().getRightSquareBracketKeyword_1_2_1_3());
									}
									(
										this_NEWLINE_24=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_24, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_2_1_4());
										}
									)+
								)
								    |
								(
									(
										this_NEWLINE_25=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_25, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_2_2_0());
										}
									)+
									this_BEGIN_26=RULE_BEGIN
									{
										newLeafNode(this_BEGIN_26, grammarAccess.getContainerAccess().getBEGINTerminalRuleCall_1_2_2_1());
									}
									(
										otherlv_27=HyphenMinus
										{
											newLeafNode(otherlv_27, grammarAccess.getContainerAccess().getHyphenMinusKeyword_1_2_2_2_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getPortsExpressionParserRuleCall_1_2_2_2_1_0());
												}
												lv_ports_28_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"ports",
														lv_ports_28_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											this_NEWLINE_29=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_29, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_2_2_2_2());
											}
										)+
									)+
									this_END_30=RULE_END
									{
										newLeafNode(this_END_30, grammarAccess.getContainerAccess().getENDTerminalRuleCall_1_2_2_3());
									}
									(
										this_NEWLINE_31=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_31, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_2_2_4());
										}
									)*
								)
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 3)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 3);
					}
								({true}?=>((
									otherlv_32=Volumes
									{
										newLeafNode(otherlv_32, grammarAccess.getContainerAccess().getVolumesKeyword_1_3_0_0());
									}
									otherlv_33=Colon
									{
										newLeafNode(otherlv_33, grammarAccess.getContainerAccess().getColonKeyword_1_3_0_1());
									}
									(
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getVolumesExpressionParserRuleCall_1_3_0_2_0_0());
												}
												lv_volumes_34_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"volumes",
														lv_volumes_34_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											this_NEWLINE_35=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_35, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_3_0_2_1());
											}
										)+
									)
								)
								    |
								(
									otherlv_36=LeftSquareBracket
									{
										newLeafNode(otherlv_36, grammarAccess.getContainerAccess().getLeftSquareBracketKeyword_1_3_1_0());
									}
									(
										(
											{
												newCompositeNode(grammarAccess.getContainerAccess().getVolumesExpressionParserRuleCall_1_3_1_1_0());
											}
											lv_volumes_37_0=ruleExpression
											{
												if ($current==null) {
													$current = createModelElementForParent(grammarAccess.getContainerRule());
												}
												add(
													$current,
													"volumes",
													lv_volumes_37_0,
													"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
												afterParserOrEnumRuleCall();
											}
										)
									)
									(
										otherlv_38=Comma
										{
											newLeafNode(otherlv_38, grammarAccess.getContainerAccess().getCommaKeyword_1_3_1_2_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getVolumesExpressionParserRuleCall_1_3_1_2_1_0());
												}
												lv_volumes_39_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"volumes",
														lv_volumes_39_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
									)*
									otherlv_40=RightSquareBracket
									{
										newLeafNode(otherlv_40, grammarAccess.getContainerAccess().getRightSquareBracketKeyword_1_3_1_3());
									}
									(
										this_NEWLINE_41=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_41, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_3_1_4());
										}
									)+
								)
								    |
								(
									(
										this_NEWLINE_42=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_42, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_3_2_0());
										}
									)*
									this_BEGIN_43=RULE_BEGIN
									{
										newLeafNode(this_BEGIN_43, grammarAccess.getContainerAccess().getBEGINTerminalRuleCall_1_3_2_1());
									}
									(
										otherlv_44=HyphenMinus
										{
											newLeafNode(otherlv_44, grammarAccess.getContainerAccess().getHyphenMinusKeyword_1_3_2_2_0());
										}
										(
											(
												{
													newCompositeNode(grammarAccess.getContainerAccess().getVolumesExpressionParserRuleCall_1_3_2_2_1_0());
												}
												lv_volumes_45_0=ruleExpression
												{
													if ($current==null) {
														$current = createModelElementForParent(grammarAccess.getContainerRule());
													}
													add(
														$current,
														"volumes",
														lv_volumes_45_0,
														"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
													afterParserOrEnumRuleCall();
												}
											)
										)
										(
											this_NEWLINE_46=RULE_NEWLINE
											{
												newLeafNode(this_NEWLINE_46, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_3_2_2_2());
											}
										)*
									)+
									this_END_47=RULE_END
									{
										newLeafNode(this_END_47, grammarAccess.getContainerAccess().getENDTerminalRuleCall_1_3_2_3());
									}
									(
										this_NEWLINE_48=RULE_NEWLINE
										{
											newLeafNode(this_NEWLINE_48, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_3_2_4());
										}
									)*
								)
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 4)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 4);
					}
								({true}?=>(otherlv_49=Options
								{
									newLeafNode(otherlv_49, grammarAccess.getContainerAccess().getOptionsKeyword_1_4_0());
								}
								otherlv_50=Colon
								{
									newLeafNode(otherlv_50, grammarAccess.getContainerAccess().getColonKeyword_1_4_1());
								}
								(
									(
										{
											newCompositeNode(grammarAccess.getContainerAccess().getOptionsExpressionParserRuleCall_1_4_2_0());
										}
										lv_options_51_0=ruleExpression
										{
											if ($current==null) {
												$current = createModelElementForParent(grammarAccess.getContainerRule());
											}
											set(
												$current,
												"options",
												lv_options_51_0,
												"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
											afterParserOrEnumRuleCall();
										}
									)
								)
								(
									this_NEWLINE_52=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_52, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_4_3());
									}
								)+
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)|
			(
				{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 5)}?=>(
					{
						getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1(), 5);
					}
								({true}?=>((
									(
										{ 
										  getUnorderedGroupHelper().enter(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0());
										}
										(
											(
									(
										{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0(), 0)}?=>(
											{
												getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0(), 0);
											}
														({true}?=>(otherlv_54=Credentials
														{
															newLeafNode(otherlv_54, grammarAccess.getContainerAccess().getCredentialsKeyword_1_5_0_0_0());
														}
														otherlv_55=Colon
														{
															newLeafNode(otherlv_55, grammarAccess.getContainerAccess().getColonKeyword_1_5_0_0_1());
														}
														(
															this_NEWLINE_56=RULE_NEWLINE
															{
																newLeafNode(this_NEWLINE_56, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_5_0_0_2());
															}
														)+
														this_BEGIN_57=RULE_BEGIN
														{
															newLeafNode(this_BEGIN_57, grammarAccess.getContainerAccess().getBEGINTerminalRuleCall_1_5_0_0_3());
														}
														(
															otherlv_58=Username
															{
																newLeafNode(otherlv_58, grammarAccess.getContainerAccess().getUsernameKeyword_1_5_0_0_4_0());
															}
															otherlv_59=Colon
															{
																newLeafNode(otherlv_59, grammarAccess.getContainerAccess().getColonKeyword_1_5_0_0_4_1());
															}
															(
																(
																	{
																		newCompositeNode(grammarAccess.getContainerAccess().getUsernameExpressionParserRuleCall_1_5_0_0_4_2_0());
																	}
																	lv_username_60_0=ruleExpression
																	{
																		if ($current==null) {
																			$current = createModelElementForParent(grammarAccess.getContainerRule());
																		}
																		set(
																			$current,
																			"username",
																			lv_username_60_0,
																			"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																		afterParserOrEnumRuleCall();
																	}
																)
															)
															(
																this_NEWLINE_61=RULE_NEWLINE
																{
																	newLeafNode(this_NEWLINE_61, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_5_0_0_4_3());
																}
															)+
														)
														))
											{ 
												getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0());
											}
										)
									)|
									(
										{getUnorderedGroupHelper().canSelect(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0(), 1)}?=>(
											{
												getUnorderedGroupHelper().select(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0(), 1);
											}
														({true}?=>((
															otherlv_62=Password
															{
																newLeafNode(otherlv_62, grammarAccess.getContainerAccess().getPasswordKeyword_1_5_0_1_0_0());
															}
															otherlv_63=Colon
															{
																newLeafNode(otherlv_63, grammarAccess.getContainerAccess().getColonKeyword_1_5_0_1_0_1());
															}
															(
																(
																	{
																		newCompositeNode(grammarAccess.getContainerAccess().getPasswordExpressionParserRuleCall_1_5_0_1_0_2_0());
																	}
																	lv_password_64_0=ruleExpression
																	{
																		if ($current==null) {
																			$current = createModelElementForParent(grammarAccess.getContainerRule());
																		}
																		set(
																			$current,
																			"password",
																			lv_password_64_0,
																			"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
																		afterParserOrEnumRuleCall();
																	}
																)
															)
															(
																this_NEWLINE_65=RULE_NEWLINE
																{
																	newLeafNode(this_NEWLINE_65, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_5_0_1_0_3());
																}
															)+
														)
														this_END_66=RULE_END
														{
															newLeafNode(this_END_66, grammarAccess.getContainerAccess().getENDTerminalRuleCall_1_5_0_1_1());
														}
														(
															this_NEWLINE_67=RULE_NEWLINE
															{
																newLeafNode(this_NEWLINE_67, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_5_0_1_2());
															}
														)*
														))
											{ 
												getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0());
											}
										)
									)
											)+
											{getUnorderedGroupHelper().canLeave(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0())}?
										)
									)
										{ 
										  getUnorderedGroupHelper().leave(grammarAccess.getContainerAccess().getUnorderedGroup_1_5_0());
										}
								)?
								this_END_68=RULE_END
								{
									newLeafNode(this_END_68, grammarAccess.getContainerAccess().getENDTerminalRuleCall_1_5_1());
								}
								(
									this_NEWLINE_69=RULE_NEWLINE
									{
										newLeafNode(this_NEWLINE_69, grammarAccess.getContainerAccess().getNEWLINETerminalRuleCall_1_5_2());
									}
								)*
								))
					{ 
						getUnorderedGroupHelper().returnFromSelection(grammarAccess.getContainerAccess().getUnorderedGroup_1());
					}
				)
			)
					)+
					{getUnorderedGroupHelper().canLeave(grammarAccess.getContainerAccess().getUnorderedGroup_1())}?
				)
			)
				{ 
				  getUnorderedGroupHelper().leave(grammarAccess.getContainerAccess().getUnorderedGroup_1());
				}
		)
	)
;
finally {
	myUnorderedGroupState.restore();
}

// Entry rule entryRuleService
entryRuleService returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getServiceRule()); }
	iv_ruleService=ruleService
	{ $current=$iv_ruleService.current; }
	EOF;

// Rule Service
ruleService returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					newCompositeNode(grammarAccess.getServiceAccess().getKeyYAMLIDParserRuleCall_0_0());
				}
				lv_key_0_0=ruleYAMLID
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getServiceRule());
					}
					set(
						$current,
						"key",
						lv_key_0_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getServiceAccess().getColonKeyword_1());
		}
		(
			this_NEWLINE_2=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_2, grammarAccess.getServiceAccess().getNEWLINETerminalRuleCall_2());
			}
		)+
		this_BEGIN_3=RULE_BEGIN
		{
			newLeafNode(this_BEGIN_3, grammarAccess.getServiceAccess().getBEGINTerminalRuleCall_3());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getServiceAccess().getValueContainerParserRuleCall_4_0());
				}
				lv_value_4_0=ruleContainer
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getServiceRule());
					}
					set(
						$current,
						"value",
						lv_value_4_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Container");
					afterParserOrEnumRuleCall();
				}
			)
		)
		this_END_5=RULE_END
		{
			newLeafNode(this_END_5, grammarAccess.getServiceAccess().getENDTerminalRuleCall_5());
		}
		(
			this_NEWLINE_6=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_6, grammarAccess.getServiceAccess().getNEWLINETerminalRuleCall_6());
			}
		)*
	)
;

// Entry rule entryRuleVariableAssignment
entryRuleVariableAssignment returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getVariableAssignmentRule()); }
	iv_ruleVariableAssignment=ruleVariableAssignment
	{ $current=$iv_ruleVariableAssignment.current; }
	EOF;

// Rule VariableAssignment
ruleVariableAssignment returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					newCompositeNode(grammarAccess.getVariableAssignmentAccess().getKeyYAMLIDParserRuleCall_0_0());
				}
				lv_key_0_0=ruleYAMLID
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getVariableAssignmentRule());
					}
					set(
						$current,
						"key",
						lv_key_0_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLID");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_1=Colon
		{
			newLeafNode(otherlv_1, grammarAccess.getVariableAssignmentAccess().getColonKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getVariableAssignmentAccess().getValueExpressionParserRuleCall_2_0());
				}
				lv_value_2_0=ruleExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getVariableAssignmentRule());
					}
					set(
						$current,
						"value",
						lv_value_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Expression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		(
			this_NEWLINE_3=RULE_NEWLINE
			{
				newLeafNode(this_NEWLINE_3, grammarAccess.getVariableAssignmentAccess().getNEWLINETerminalRuleCall_3());
			}
		)+
	)
;

// Entry rule entryRuleExpression
entryRuleExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getExpressionRule()); }
	iv_ruleExpression=ruleExpression
	{ $current=$iv_ruleExpression.current; }
	EOF;

// Rule Expression
ruleExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	{
		/* */
	}
	{
		newCompositeNode(grammarAccess.getExpressionAccess().getConcatParserRuleCall());
	}
	this_Concat_0=ruleConcat
	{
		$current = $this_Concat_0.current;
		afterParserOrEnumRuleCall();
	}
;

// Entry rule entryRuleConcat
entryRuleConcat returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getConcatRule()); }
	iv_ruleConcat=ruleConcat
	{ $current=$iv_ruleConcat.current; }
	EOF;

// Rule Concat
ruleConcat returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getConcatAccess().getConcatExpressionParserRuleCall_0());
		}
		this_ConcatExpression_0=ruleConcatExpression
		{
			$current = $this_ConcatExpression_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getConcatAccess().getConcatLhsAction_1_0(),
						$current);
				}
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getConcatAccess().getRhsConcatExpressionParserRuleCall_1_1_0());
					}
					lv_rhs_2_0=ruleConcatExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getConcatRule());
						}
						set(
							$current,
							"rhs",
							lv_rhs_2_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.ConcatExpression");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
	)
;

// Entry rule entryRuleConcatExpression
entryRuleConcatExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getConcatExpressionRule()); }
	iv_ruleConcatExpression=ruleConcatExpression
	{ $current=$iv_ruleConcatExpression.current; }
	EOF;

// Rule ConcatExpression
ruleConcatExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getConcatExpressionAccess().getBracketedExpressionParserRuleCall_0());
		}
		this_BracketedExpression_0=ruleBracketedExpression
		{
			$current = $this_BracketedExpression_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getConcatExpressionAccess().getUnbracketedLiteralParserRuleCall_1());
		}
		this_UnbracketedLiteral_1=ruleUnbracketedLiteral
		{
			$current = $this_UnbracketedLiteral_1.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleBracketedExpression
entryRuleBracketedExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getBracketedExpressionRule()); }
	iv_ruleBracketedExpression=ruleBracketedExpression
	{ $current=$iv_ruleBracketedExpression.current; }
	EOF;

// Rule BracketedExpression
ruleBracketedExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=DollarSignLeftCurlyBracketLeftCurlyBracket
		{
			newLeafNode(otherlv_0, grammarAccess.getBracketedExpressionAccess().getDollarSignLeftCurlyBracketLeftCurlyBracketKeyword_0());
		}
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getBracketedExpressionAccess().getInsideBracketsExpressionParserRuleCall_1());
		}
		this_InsideBracketsExpression_1=ruleInsideBracketsExpression
		{
			$current = $this_InsideBracketsExpression_1.current;
			afterParserOrEnumRuleCall();
		}
		otherlv_2=RightCurlyBracketRightCurlyBracket
		{
			newLeafNode(otherlv_2, grammarAccess.getBracketedExpressionAccess().getRightCurlyBracketRightCurlyBracketKeyword_2());
		}
	)
;

// Entry rule entryRuleInsideBracketsExpression
entryRuleInsideBracketsExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getInsideBracketsExpressionRule()); }
	iv_ruleInsideBracketsExpression=ruleInsideBracketsExpression
	{ $current=$iv_ruleInsideBracketsExpression.current; }
	EOF;

// Rule InsideBracketsExpression
ruleInsideBracketsExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	{
		/* */
	}
	{
		newCompositeNode(grammarAccess.getInsideBracketsExpressionAccess().getOrParserRuleCall());
	}
	this_Or_0=ruleOr
	{
		$current = $this_Or_0.current;
		afterParserOrEnumRuleCall();
	}
;

// Entry rule entryRuleUnbracketedLiteral
entryRuleUnbracketedLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getUnbracketedLiteralRule()); }
	iv_ruleUnbracketedLiteral=ruleUnbracketedLiteral
	{ $current=$iv_ruleUnbracketedLiteral.current; }
	EOF;

// Rule UnbracketedLiteral
ruleUnbracketedLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnbracketedLiteralAccess().getStringLiteralParserRuleCall_0());
		}
		this_StringLiteral_0=ruleStringLiteral
		{
			$current = $this_StringLiteral_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnbracketedLiteralAccess().getIntegerLiteralParserRuleCall_1());
		}
		this_IntegerLiteral_1=ruleIntegerLiteral
		{
			$current = $this_IntegerLiteral_1.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnbracketedLiteralAccess().getDoubleLiteralParserRuleCall_2());
		}
		this_DoubleLiteral_2=ruleDoubleLiteral
		{
			$current = $this_DoubleLiteral_2.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnbracketedLiteralAccess().getBooleanLiteralParserRuleCall_3());
		}
		this_BooleanLiteral_3=ruleBooleanLiteral
		{
			$current = $this_BooleanLiteral_3.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleStringLiteral
entryRuleStringLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getStringLiteralRule()); }
	iv_ruleStringLiteral=ruleStringLiteral
	{ $current=$iv_ruleStringLiteral.current; }
	EOF;

// Rule StringLiteral
ruleStringLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				newCompositeNode(grammarAccess.getStringLiteralAccess().getValueYAMLStringParserRuleCall_0());
			}
			lv_value_0_0=ruleYAMLString
			{
				if ($current==null) {
					$current = createModelElementForParent(grammarAccess.getStringLiteralRule());
				}
				set(
					$current,
					"value",
					lv_value_0_0,
					"d.fe.up.pt.cicd.gha.dsl.GitHubActions.YAMLString");
				afterParserOrEnumRuleCall();
			}
		)
	)
;

// Entry rule entryRuleIntegerLiteral
entryRuleIntegerLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getIntegerLiteralRule()); }
	iv_ruleIntegerLiteral=ruleIntegerLiteral
	{ $current=$iv_ruleIntegerLiteral.current; }
	EOF;

// Rule IntegerLiteral
ruleIntegerLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			lv_value_0_0=RULE_INT
			{
				newLeafNode(lv_value_0_0, grammarAccess.getIntegerLiteralAccess().getValueINTTerminalRuleCall_0());
			}
			{
				if ($current==null) {
					$current = createModelElement(grammarAccess.getIntegerLiteralRule());
				}
				setWithLastConsumed(
					$current,
					"value",
					lv_value_0_0,
					"org.eclipse.xtext.common.Terminals.INT");
			}
		)
	)
;

// Entry rule entryRuleDoubleLiteral
entryRuleDoubleLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getDoubleLiteralRule()); }
	iv_ruleDoubleLiteral=ruleDoubleLiteral
	{ $current=$iv_ruleDoubleLiteral.current; }
	EOF;

// Rule DoubleLiteral
ruleDoubleLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			lv_value_0_0=RULE_DOUBLE
			{
				newLeafNode(lv_value_0_0, grammarAccess.getDoubleLiteralAccess().getValueDOUBLETerminalRuleCall_0());
			}
			{
				if ($current==null) {
					$current = createModelElement(grammarAccess.getDoubleLiteralRule());
				}
				setWithLastConsumed(
					$current,
					"value",
					lv_value_0_0,
					"d.fe.up.pt.cicd.gha.dsl.GitHubActions.DOUBLE");
			}
		)
	)
;

// Entry rule entryRuleBooleanLiteral
entryRuleBooleanLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getBooleanLiteralRule()); }
	iv_ruleBooleanLiteral=ruleBooleanLiteral
	{ $current=$iv_ruleBooleanLiteral.current; }
	EOF;

// Rule BooleanLiteral
ruleBooleanLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			lv_value_0_0=RULE_BOOLEAN
			{
				newLeafNode(lv_value_0_0, grammarAccess.getBooleanLiteralAccess().getValueBOOLEANTerminalRuleCall_0());
			}
			{
				if ($current==null) {
					$current = createModelElement(grammarAccess.getBooleanLiteralRule());
				}
				setWithLastConsumed(
					$current,
					"value",
					lv_value_0_0 != null,
					"d.fe.up.pt.cicd.gha.dsl.GitHubActions.BOOLEAN");
			}
		)
	)
;

// Entry rule entryRuleOr
entryRuleOr returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getOrRule()); }
	iv_ruleOr=ruleOr
	{ $current=$iv_ruleOr.current; }
	EOF;

// Rule Or
ruleOr returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getOrAccess().getAndParserRuleCall_0());
		}
		this_And_0=ruleAnd
		{
			$current = $this_And_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getOrAccess().getOrLhsAction_1_0(),
						$current);
				}
			)
			otherlv_2=VerticalLineVerticalLine
			{
				newLeafNode(otherlv_2, grammarAccess.getOrAccess().getVerticalLineVerticalLineKeyword_1_1());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getOrAccess().getRhsAndParserRuleCall_1_2_0());
					}
					lv_rhs_3_0=ruleAnd
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getOrRule());
						}
						set(
							$current,
							"rhs",
							lv_rhs_3_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.And");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
	)
;

// Entry rule entryRuleAnd
entryRuleAnd returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getAndRule()); }
	iv_ruleAnd=ruleAnd
	{ $current=$iv_ruleAnd.current; }
	EOF;

// Rule And
ruleAnd returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getAndAccess().getEqualityParserRuleCall_0());
		}
		this_Equality_0=ruleEquality
		{
			$current = $this_Equality_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getAndAccess().getAndLhsAction_1_0(),
						$current);
				}
			)
			otherlv_2=AmpersandAmpersand
			{
				newLeafNode(otherlv_2, grammarAccess.getAndAccess().getAmpersandAmpersandKeyword_1_1());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getAndAccess().getRhsEqualityParserRuleCall_1_2_0());
					}
					lv_rhs_3_0=ruleEquality
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getAndRule());
						}
						set(
							$current,
							"rhs",
							lv_rhs_3_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Equality");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
	)
;

// Entry rule entryRuleEquality
entryRuleEquality returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getEqualityRule()); }
	iv_ruleEquality=ruleEquality
	{ $current=$iv_ruleEquality.current; }
	EOF;

// Rule Equality
ruleEquality returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getEqualityAccess().getComparisonParserRuleCall_0());
		}
		this_Comparison_0=ruleComparison
		{
			$current = $this_Comparison_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getEqualityAccess().getEqualityLhsAction_1_0(),
						$current);
				}
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getEqualityAccess().getOpEQUALITY_OPEnumRuleCall_1_1_0());
					}
					lv_op_2_0=ruleEQUALITY_OP
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getEqualityRule());
						}
						set(
							$current,
							"op",
							lv_op_2_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.EQUALITY_OP");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getEqualityAccess().getRhsComparisonParserRuleCall_1_2_0());
					}
					lv_rhs_3_0=ruleComparison
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getEqualityRule());
						}
						set(
							$current,
							"rhs",
							lv_rhs_3_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.Comparison");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
	)
;

// Entry rule entryRuleComparison
entryRuleComparison returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getComparisonRule()); }
	iv_ruleComparison=ruleComparison
	{ $current=$iv_ruleComparison.current; }
	EOF;

// Rule Comparison
ruleComparison returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getComparisonAccess().getUnaryOpParserRuleCall_0());
		}
		this_UnaryOp_0=ruleUnaryOp
		{
			$current = $this_UnaryOp_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getComparisonAccess().getComparisonLhsAction_1_0(),
						$current);
				}
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getComparisonAccess().getOpCOMPARISON_OPEnumRuleCall_1_1_0());
					}
					lv_op_2_0=ruleCOMPARISON_OP
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getComparisonRule());
						}
						set(
							$current,
							"op",
							lv_op_2_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.COMPARISON_OP");
						afterParserOrEnumRuleCall();
					}
				)
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getComparisonAccess().getRhsUnaryOpParserRuleCall_1_2_0());
					}
					lv_rhs_3_0=ruleUnaryOp
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getComparisonRule());
						}
						set(
							$current,
							"rhs",
							lv_rhs_3_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.UnaryOp");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
	)
;

// Entry rule entryRuleUnaryOp
entryRuleUnaryOp returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getUnaryOpRule()); }
	iv_ruleUnaryOp=ruleUnaryOp
	{ $current=$iv_ruleUnaryOp.current; }
	EOF;

// Rule UnaryOp
ruleUnaryOp returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnaryOpAccess().getNotParserRuleCall_0());
		}
		this_Not_0=ruleNot
		{
			$current = $this_Not_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getUnaryOpAccess().getBlankParserRuleCall_1());
		}
		this_Blank_1=ruleBlank
		{
			$current = $this_Blank_1.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleNot
entryRuleNot returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getNotRule()); }
	iv_ruleNot=ruleNot
	{ $current=$iv_ruleNot.current; }
	EOF;

// Rule Not
ruleNot returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=ExclamationMark
		{
			newLeafNode(otherlv_0, grammarAccess.getNotAccess().getExclamationMarkKeyword_0());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getNotAccess().getChildExprVariableDereferenceParserRuleCall_1_0());
				}
				lv_childExpr_1_0=ruleVariableDereference
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getNotRule());
					}
					set(
						$current,
						"childExpr",
						lv_childExpr_1_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.VariableDereference");
					afterParserOrEnumRuleCall();
				}
			)
		)
	)
;

// Entry rule entryRuleBlank
entryRuleBlank returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getBlankRule()); }
	iv_ruleBlank=ruleBlank
	{ $current=$iv_ruleBlank.current; }
	EOF;

// Rule Blank
ruleBlank returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	{
		/* */
	}
	{
		newCompositeNode(grammarAccess.getBlankAccess().getVariableDereferenceParserRuleCall());
	}
	this_VariableDereference_0=ruleVariableDereference
	{
		$current = $this_VariableDereference_0.current;
		afterParserOrEnumRuleCall();
	}
;

// Entry rule entryRuleVariableDereference
entryRuleVariableDereference returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getVariableDereferenceRule()); }
	iv_ruleVariableDereference=ruleVariableDereference
	{ $current=$iv_ruleVariableDereference.current; }
	EOF;

// Rule VariableDereference
ruleVariableDereference returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getVariableDereferenceAccess().getPrimaryParserRuleCall_0());
		}
		this_Primary_0=rulePrimary
		{
			$current = $this_Primary_0.current;
			afterParserOrEnumRuleCall();
		}
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElementAndSet(
						grammarAccess.getVariableDereferenceAccess().getVariableDereferenceVariableAction_1_0(),
						$current);
				}
			)
			(
				(
					otherlv_2=FullStop
					{
						newLeafNode(otherlv_2, grammarAccess.getVariableDereferenceAccess().getFullStopKeyword_1_1_0_0());
					}
					(
						(
							lv_property_3_0=RULE_ID
							{
								newLeafNode(lv_property_3_0, grammarAccess.getVariableDereferenceAccess().getPropertyIDTerminalRuleCall_1_1_0_1_0());
							}
							{
								if ($current==null) {
									$current = createModelElement(grammarAccess.getVariableDereferenceRule());
								}
								setWithLastConsumed(
									$current,
									"property",
									lv_property_3_0,
									"org.eclipse.xtext.common.Terminals.ID");
							}
						)
					)
				)
				    |
				(
					(
						otherlv_4=LeftSquareBracket
						{
							newLeafNode(otherlv_4, grammarAccess.getVariableDereferenceAccess().getLeftSquareBracketKeyword_1_1_1_0_0());
						}
						(
							otherlv_5=QuotationMark
							{
								newLeafNode(otherlv_5, grammarAccess.getVariableDereferenceAccess().getQuotationMarkKeyword_1_1_1_0_1_0());
							}
							(
								(
									lv_property_6_0=RULE_ID
									{
										newLeafNode(lv_property_6_0, grammarAccess.getVariableDereferenceAccess().getPropertyIDTerminalRuleCall_1_1_1_0_1_1_0());
									}
									{
										if ($current==null) {
											$current = createModelElement(grammarAccess.getVariableDereferenceRule());
										}
										setWithLastConsumed(
											$current,
											"property",
											lv_property_6_0,
											"org.eclipse.xtext.common.Terminals.ID");
									}
								)
							)
							otherlv_7=QuotationMark
							{
								newLeafNode(otherlv_7, grammarAccess.getVariableDereferenceAccess().getQuotationMarkKeyword_1_1_1_0_1_2());
							}
						)
					)
					    |
					(
						(
							otherlv_8=Apostrophe
							{
								newLeafNode(otherlv_8, grammarAccess.getVariableDereferenceAccess().getApostropheKeyword_1_1_1_1_0_0());
							}
							(
								(
									lv_property_9_0=RULE_ID
									{
										newLeafNode(lv_property_9_0, grammarAccess.getVariableDereferenceAccess().getPropertyIDTerminalRuleCall_1_1_1_1_0_1_0());
									}
									{
										if ($current==null) {
											$current = createModelElement(grammarAccess.getVariableDereferenceRule());
										}
										setWithLastConsumed(
											$current,
											"property",
											lv_property_9_0,
											"org.eclipse.xtext.common.Terminals.ID");
									}
								)
							)
							otherlv_10=QuotationMark
							{
								newLeafNode(otherlv_10, grammarAccess.getVariableDereferenceAccess().getQuotationMarkKeyword_1_1_1_1_0_2());
							}
						)
						otherlv_11=RightSquareBracket
						{
							newLeafNode(otherlv_11, grammarAccess.getVariableDereferenceAccess().getRightSquareBracketKeyword_1_1_1_1_1());
						}
					)
				)
			)
		)*
	)
;

// Entry rule entryRulePrimary
entryRulePrimary returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getPrimaryRule()); }
	iv_rulePrimary=rulePrimary
	{ $current=$iv_rulePrimary.current; }
	EOF;

// Rule Primary
rulePrimary returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			otherlv_0=LeftParenthesis
			{
				newLeafNode(otherlv_0, grammarAccess.getPrimaryAccess().getLeftParenthesisKeyword_0_0());
			}
			{
				/* */
			}
			{
				newCompositeNode(grammarAccess.getPrimaryAccess().getInsideBracketsExpressionParserRuleCall_0_1());
			}
			this_InsideBracketsExpression_1=ruleInsideBracketsExpression
			{
				$current = $this_InsideBracketsExpression_1.current;
				afterParserOrEnumRuleCall();
			}
			otherlv_2=RightParenthesis
			{
				newLeafNode(otherlv_2, grammarAccess.getPrimaryAccess().getRightParenthesisKeyword_0_2());
			}
		)
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getPrimaryAccess().getCallExpressionParserRuleCall_1());
		}
		this_CallExpression_3=ruleCallExpression
		{
			$current = $this_CallExpression_3.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getPrimaryAccess().getAtomicParserRuleCall_2());
		}
		this_Atomic_4=ruleAtomic
		{
			$current = $this_Atomic_4.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleAtomic
entryRuleAtomic returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getAtomicRule()); }
	iv_ruleAtomic=ruleAtomic
	{ $current=$iv_ruleAtomic.current; }
	EOF;

// Rule Atomic
ruleAtomic returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getGitHubContextAction_0_0(),
						$current);
				}
			)
			(
				(
					{
						newCompositeNode(grammarAccess.getAtomicAccess().getContextCONTEXTEnumRuleCall_0_1_0());
					}
					lv_context_1_0=ruleCONTEXT
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getAtomicRule());
						}
						set(
							$current,
							"context",
							lv_context_1_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.CONTEXT");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getVariableAction_1_0(),
						$current);
				}
			)
			(
				(
					lv_name_3_0=RULE_ID
					{
						newLeafNode(lv_name_3_0, grammarAccess.getAtomicAccess().getNameIDTerminalRuleCall_1_1_0());
					}
					{
						if ($current==null) {
							$current = createModelElement(grammarAccess.getAtomicRule());
						}
						setWithLastConsumed(
							$current,
							"name",
							lv_name_3_0,
							"org.eclipse.xtext.common.Terminals.ID");
					}
				)
			)
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getIntegerLiteralAction_2_0(),
						$current);
				}
			)
			(
				(
					lv_value_5_0=RULE_INT
					{
						newLeafNode(lv_value_5_0, grammarAccess.getAtomicAccess().getValueINTTerminalRuleCall_2_1_0());
					}
					{
						if ($current==null) {
							$current = createModelElement(grammarAccess.getAtomicRule());
						}
						setWithLastConsumed(
							$current,
							"value",
							lv_value_5_0,
							"org.eclipse.xtext.common.Terminals.INT");
					}
				)
			)
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getDoubleLiteralAction_3_0(),
						$current);
				}
			)
			(
				(
					lv_value_7_0=RULE_DOUBLE
					{
						newLeafNode(lv_value_7_0, grammarAccess.getAtomicAccess().getValueDOUBLETerminalRuleCall_3_1_0());
					}
					{
						if ($current==null) {
							$current = createModelElement(grammarAccess.getAtomicRule());
						}
						setWithLastConsumed(
							$current,
							"value",
							lv_value_7_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.DOUBLE");
					}
				)
			)
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getBooleanLiteralAction_4_0(),
						$current);
				}
			)
			(
				(
					lv_value_9_0=RULE_BOOLEAN
					{
						newLeafNode(lv_value_9_0, grammarAccess.getAtomicAccess().getValueBOOLEANTerminalRuleCall_4_1_0());
					}
					{
						if ($current==null) {
							$current = createModelElement(grammarAccess.getAtomicRule());
						}
						setWithLastConsumed(
							$current,
							"value",
							lv_value_9_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.BOOLEAN");
					}
				)
			)
		)
		    |
		(
			(
				{
					/* */
				}
				{
					$current = forceCreateModelElement(
						grammarAccess.getAtomicAccess().getStringLiteralAction_5_0(),
						$current);
				}
			)
			(
				(
					lv_value_11_0=RULE_STRING
					{
						newLeafNode(lv_value_11_0, grammarAccess.getAtomicAccess().getValueSTRINGTerminalRuleCall_5_1_0());
					}
					{
						if ($current==null) {
							$current = createModelElement(grammarAccess.getAtomicRule());
						}
						setWithLastConsumed(
							$current,
							"value",
							lv_value_11_0,
							"org.eclipse.xtext.common.Terminals.STRING");
					}
				)
			)
		)
	)
;

// Entry rule entryRuleCallExpression
entryRuleCallExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getCallExpressionRule()); }
	iv_ruleCallExpression=ruleCallExpression
	{ $current=$iv_ruleCallExpression.current; }
	EOF;

// Rule CallExpression
ruleCallExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getContainsParserRuleCall_0());
		}
		this_Contains_0=ruleContains
		{
			$current = $this_Contains_0.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getStartsWithParserRuleCall_1());
		}
		this_StartsWith_1=ruleStartsWith
		{
			$current = $this_StartsWith_1.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getEndsWithParserRuleCall_2());
		}
		this_EndsWith_2=ruleEndsWith
		{
			$current = $this_EndsWith_2.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getFormatParserRuleCall_3());
		}
		this_Format_3=ruleFormat
		{
			$current = $this_Format_3.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getJoinParserRuleCall_4());
		}
		this_Join_4=ruleJoin
		{
			$current = $this_Join_4.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getToJSONParserRuleCall_5());
		}
		this_ToJSON_5=ruleToJSON
		{
			$current = $this_ToJSON_5.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getFromJSONParserRuleCall_6());
		}
		this_FromJSON_6=ruleFromJSON
		{
			$current = $this_FromJSON_6.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getHashFilesParserRuleCall_7());
		}
		this_HashFiles_7=ruleHashFiles
		{
			$current = $this_HashFiles_7.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getSuccessParserRuleCall_8());
		}
		this_Success_8=ruleSuccess
		{
			$current = $this_Success_8.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getAlwaysParserRuleCall_9());
		}
		this_Always_9=ruleAlways
		{
			$current = $this_Always_9.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getFailureParserRuleCall_10());
		}
		this_Failure_10=ruleFailure
		{
			$current = $this_Failure_10.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
			/* */
		}
		{
			newCompositeNode(grammarAccess.getCallExpressionAccess().getCancelledParserRuleCall_11());
		}
		this_Cancelled_11=ruleCancelled
		{
			$current = $this_Cancelled_11.current;
			afterParserOrEnumRuleCall();
		}
	)
;

// Entry rule entryRuleContains
entryRuleContains returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getContainsRule()); }
	iv_ruleContains=ruleContains
	{ $current=$iv_ruleContains.current; }
	EOF;

// Rule Contains
ruleContains returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=Contains
		{
			newLeafNode(otherlv_0, grammarAccess.getContainsAccess().getContainsKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getContainsAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getContainsAccess().getSearchInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_search_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getContainsRule());
					}
					set(
						$current,
						"search",
						lv_search_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=Comma
		{
			newLeafNode(otherlv_3, grammarAccess.getContainsAccess().getCommaKeyword_3());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getContainsAccess().getItemInsideBracketsExpressionParserRuleCall_4_0());
				}
				lv_item_4_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getContainsRule());
					}
					set(
						$current,
						"item",
						lv_item_4_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_5=RightParenthesis
		{
			newLeafNode(otherlv_5, grammarAccess.getContainsAccess().getRightParenthesisKeyword_5());
		}
	)
;

// Entry rule entryRuleStartsWith
entryRuleStartsWith returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getStartsWithRule()); }
	iv_ruleStartsWith=ruleStartsWith
	{ $current=$iv_ruleStartsWith.current; }
	EOF;

// Rule StartsWith
ruleStartsWith returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=StartsWith
		{
			newLeafNode(otherlv_0, grammarAccess.getStartsWithAccess().getStartsWithKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getStartsWithAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getStartsWithAccess().getSearchStringInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_searchString_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getStartsWithRule());
					}
					set(
						$current,
						"searchString",
						lv_searchString_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=Comma
		{
			newLeafNode(otherlv_3, grammarAccess.getStartsWithAccess().getCommaKeyword_3());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getStartsWithAccess().getSearchValueInsideBracketsExpressionParserRuleCall_4_0());
				}
				lv_searchValue_4_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getStartsWithRule());
					}
					set(
						$current,
						"searchValue",
						lv_searchValue_4_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_5=RightParenthesis
		{
			newLeafNode(otherlv_5, grammarAccess.getStartsWithAccess().getRightParenthesisKeyword_5());
		}
	)
;

// Entry rule entryRuleEndsWith
entryRuleEndsWith returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getEndsWithRule()); }
	iv_ruleEndsWith=ruleEndsWith
	{ $current=$iv_ruleEndsWith.current; }
	EOF;

// Rule EndsWith
ruleEndsWith returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=EndsWith
		{
			newLeafNode(otherlv_0, grammarAccess.getEndsWithAccess().getEndsWithKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getEndsWithAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getEndsWithAccess().getSearchStringInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_searchString_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getEndsWithRule());
					}
					set(
						$current,
						"searchString",
						lv_searchString_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=Comma
		{
			newLeafNode(otherlv_3, grammarAccess.getEndsWithAccess().getCommaKeyword_3());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getEndsWithAccess().getSearchValueInsideBracketsExpressionParserRuleCall_4_0());
				}
				lv_searchValue_4_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getEndsWithRule());
					}
					set(
						$current,
						"searchValue",
						lv_searchValue_4_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_5=RightParenthesis
		{
			newLeafNode(otherlv_5, grammarAccess.getEndsWithAccess().getRightParenthesisKeyword_5());
		}
	)
;

// Entry rule entryRuleFormat
entryRuleFormat returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getFormatRule()); }
	iv_ruleFormat=ruleFormat
	{ $current=$iv_ruleFormat.current; }
	EOF;

// Rule Format
ruleFormat returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=Format
		{
			newLeafNode(otherlv_0, grammarAccess.getFormatAccess().getFormatKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getFormatAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getFormatAccess().getStringInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_string_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getFormatRule());
					}
					set(
						$current,
						"string",
						lv_string_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		(
			otherlv_3=Comma
			{
				newLeafNode(otherlv_3, grammarAccess.getFormatAccess().getCommaKeyword_3_0());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getFormatAccess().getReplaceValuesInsideBracketsExpressionParserRuleCall_3_1_0());
					}
					lv_replaceValues_4_0=ruleInsideBracketsExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getFormatRule());
						}
						add(
							$current,
							"replaceValues",
							lv_replaceValues_4_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)*
		otherlv_5=RightParenthesis
		{
			newLeafNode(otherlv_5, grammarAccess.getFormatAccess().getRightParenthesisKeyword_4());
		}
	)
;

// Entry rule entryRuleJoin
entryRuleJoin returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getJoinRule()); }
	iv_ruleJoin=ruleJoin
	{ $current=$iv_ruleJoin.current; }
	EOF;

// Rule Join
ruleJoin returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=Join
		{
			newLeafNode(otherlv_0, grammarAccess.getJoinAccess().getJoinKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getJoinAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getJoinAccess().getArrayInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_array_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getJoinRule());
					}
					set(
						$current,
						"array",
						lv_array_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		(
			otherlv_3=Comma
			{
				newLeafNode(otherlv_3, grammarAccess.getJoinAccess().getCommaKeyword_3_0());
			}
			(
				(
					{
						newCompositeNode(grammarAccess.getJoinAccess().getSepInsideBracketsExpressionParserRuleCall_3_1_0());
					}
					lv_sep_4_0=ruleInsideBracketsExpression
					{
						if ($current==null) {
							$current = createModelElementForParent(grammarAccess.getJoinRule());
						}
						set(
							$current,
							"sep",
							lv_sep_4_0,
							"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
						afterParserOrEnumRuleCall();
					}
				)
			)
		)?
		otherlv_5=RightParenthesis
		{
			newLeafNode(otherlv_5, grammarAccess.getJoinAccess().getRightParenthesisKeyword_4());
		}
	)
;

// Entry rule entryRuleToJSON
entryRuleToJSON returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getToJSONRule()); }
	iv_ruleToJSON=ruleToJSON
	{ $current=$iv_ruleToJSON.current; }
	EOF;

// Rule ToJSON
ruleToJSON returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=ToJSON
		{
			newLeafNode(otherlv_0, grammarAccess.getToJSONAccess().getToJSONKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getToJSONAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getToJSONAccess().getValueInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_value_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getToJSONRule());
					}
					set(
						$current,
						"value",
						lv_value_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getToJSONAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleFromJSON
entryRuleFromJSON returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getFromJSONRule()); }
	iv_ruleFromJSON=ruleFromJSON
	{ $current=$iv_ruleFromJSON.current; }
	EOF;

// Rule FromJSON
ruleFromJSON returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=FromJSON
		{
			newLeafNode(otherlv_0, grammarAccess.getFromJSONAccess().getFromJSONKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getFromJSONAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getFromJSONAccess().getValueInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_value_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getFromJSONRule());
					}
					set(
						$current,
						"value",
						lv_value_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getFromJSONAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleHashFiles
entryRuleHashFiles returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getHashFilesRule()); }
	iv_ruleHashFiles=ruleHashFiles
	{ $current=$iv_ruleHashFiles.current; }
	EOF;

// Rule HashFiles
ruleHashFiles returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		otherlv_0=HashFiles
		{
			newLeafNode(otherlv_0, grammarAccess.getHashFilesAccess().getHashFilesKeyword_0());
		}
		otherlv_1=LeftParenthesis
		{
			newLeafNode(otherlv_1, grammarAccess.getHashFilesAccess().getLeftParenthesisKeyword_1());
		}
		(
			(
				{
					newCompositeNode(grammarAccess.getHashFilesAccess().getPathInsideBracketsExpressionParserRuleCall_2_0());
				}
				lv_path_2_0=ruleInsideBracketsExpression
				{
					if ($current==null) {
						$current = createModelElementForParent(grammarAccess.getHashFilesRule());
					}
					set(
						$current,
						"path",
						lv_path_2_0,
						"d.fe.up.pt.cicd.gha.dsl.GitHubActions.InsideBracketsExpression");
					afterParserOrEnumRuleCall();
				}
			)
		)
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getHashFilesAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleSuccess
entryRuleSuccess returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getSuccessRule()); }
	iv_ruleSuccess=ruleSuccess
	{ $current=$iv_ruleSuccess.current; }
	EOF;

// Rule Success
ruleSuccess returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getSuccessAccess().getSuccessAction_0(),
					$current);
			}
		)
		otherlv_1=Success
		{
			newLeafNode(otherlv_1, grammarAccess.getSuccessAccess().getSuccessKeyword_1());
		}
		otherlv_2=LeftParenthesis
		{
			newLeafNode(otherlv_2, grammarAccess.getSuccessAccess().getLeftParenthesisKeyword_2());
		}
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getSuccessAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleAlways
entryRuleAlways returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getAlwaysRule()); }
	iv_ruleAlways=ruleAlways
	{ $current=$iv_ruleAlways.current; }
	EOF;

// Rule Always
ruleAlways returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getAlwaysAccess().getAlwaysAction_0(),
					$current);
			}
		)
		otherlv_1=Always
		{
			newLeafNode(otherlv_1, grammarAccess.getAlwaysAccess().getAlwaysKeyword_1());
		}
		otherlv_2=LeftParenthesis
		{
			newLeafNode(otherlv_2, grammarAccess.getAlwaysAccess().getLeftParenthesisKeyword_2());
		}
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getAlwaysAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleCancelled
entryRuleCancelled returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getCancelledRule()); }
	iv_ruleCancelled=ruleCancelled
	{ $current=$iv_ruleCancelled.current; }
	EOF;

// Rule Cancelled
ruleCancelled returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getCancelledAccess().getCancelledAction_0(),
					$current);
			}
		)
		otherlv_1=Cancelled
		{
			newLeafNode(otherlv_1, grammarAccess.getCancelledAccess().getCancelledKeyword_1());
		}
		otherlv_2=LeftParenthesis
		{
			newLeafNode(otherlv_2, grammarAccess.getCancelledAccess().getLeftParenthesisKeyword_2());
		}
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getCancelledAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleFailure
entryRuleFailure returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getFailureRule()); }
	iv_ruleFailure=ruleFailure
	{ $current=$iv_ruleFailure.current; }
	EOF;

// Rule Failure
ruleFailure returns [EObject current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			{
				/* */
			}
			{
				$current = forceCreateModelElement(
					grammarAccess.getFailureAccess().getFailureAction_0(),
					$current);
			}
		)
		otherlv_1=Failure
		{
			newLeafNode(otherlv_1, grammarAccess.getFailureAccess().getFailureKeyword_1());
		}
		otherlv_2=LeftParenthesis
		{
			newLeafNode(otherlv_2, grammarAccess.getFailureAccess().getLeftParenthesisKeyword_2());
		}
		otherlv_3=RightParenthesis
		{
			newLeafNode(otherlv_3, grammarAccess.getFailureAccess().getRightParenthesisKeyword_3());
		}
	)
;

// Entry rule entryRuleYAMLID
entryRuleYAMLID returns [String current=null]:
	{ newCompositeNode(grammarAccess.getYAMLIDRule()); }
	iv_ruleYAMLID=ruleYAMLID
	{ $current=$iv_ruleYAMLID.current.getText(); }
	EOF;

// Rule YAMLID
ruleYAMLID returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			kw=QuotationMark
			{
				$current.merge(kw);
				newLeafNode(kw, grammarAccess.getYAMLIDAccess().getQuotationMarkKeyword_0_0());
			}
			this_ID_1=RULE_ID
			{
				$current.merge(this_ID_1);
			}
			{
				newLeafNode(this_ID_1, grammarAccess.getYAMLIDAccess().getIDTerminalRuleCall_0_1());
			}
			kw=QuotationMark
			{
				$current.merge(kw);
				newLeafNode(kw, grammarAccess.getYAMLIDAccess().getQuotationMarkKeyword_0_2());
			}
		)
		    |
		(
			kw=Apostrophe
			{
				$current.merge(kw);
				newLeafNode(kw, grammarAccess.getYAMLIDAccess().getApostropheKeyword_1_0());
			}
			this_ID_4=RULE_ID
			{
				$current.merge(this_ID_4);
			}
			{
				newLeafNode(this_ID_4, grammarAccess.getYAMLIDAccess().getIDTerminalRuleCall_1_1());
			}
			kw=Apostrophe
			{
				$current.merge(kw);
				newLeafNode(kw, grammarAccess.getYAMLIDAccess().getApostropheKeyword_1_2());
			}
		)
		    |
		this_ID_6=RULE_ID
		{
			$current.merge(this_ID_6);
		}
		{
			newLeafNode(this_ID_6, grammarAccess.getYAMLIDAccess().getIDTerminalRuleCall_2());
		}
	)
;

// Entry rule entryRuleYAMLString
entryRuleYAMLString returns [String current=null]:
	{ newCompositeNode(grammarAccess.getYAMLStringRule()); }
	iv_ruleYAMLString=ruleYAMLString
	{ $current=$iv_ruleYAMLString.current.getText(); }
	EOF;

// Rule YAMLString
ruleYAMLString returns [AntlrDatatypeRuleToken current=new AntlrDatatypeRuleToken()]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		this_STRING_0=RULE_STRING
		{
			$current.merge(this_STRING_0);
		}
		{
			newLeafNode(this_STRING_0, grammarAccess.getYAMLStringAccess().getSTRINGTerminalRuleCall_0());
		}
		    |
		this_UNQUOTED_STRING_1=RULE_UNQUOTED_STRING
		{
			$current.merge(this_UNQUOTED_STRING_1);
		}
		{
			newLeafNode(this_UNQUOTED_STRING_1, grammarAccess.getYAMLStringAccess().getUNQUOTED_STRINGTerminalRuleCall_1());
		}
	)
;

// Rule COMPARISON_OP
ruleCOMPARISON_OP returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=GreaterThanSign
			{
				$current = grammarAccess.getCOMPARISON_OPAccess().getGTEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getCOMPARISON_OPAccess().getGTEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=GreaterThanSignEqualsSign
			{
				$current = grammarAccess.getCOMPARISON_OPAccess().getGTEEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getCOMPARISON_OPAccess().getGTEEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=LessThanSign
			{
				$current = grammarAccess.getCOMPARISON_OPAccess().getLTEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getCOMPARISON_OPAccess().getLTEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=LessThanSignEqualsSign
			{
				$current = grammarAccess.getCOMPARISON_OPAccess().getLTEEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getCOMPARISON_OPAccess().getLTEEnumLiteralDeclaration_3());
			}
		)
	)
;

// Rule EQUALITY_OP
ruleEQUALITY_OP returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=EqualsSignEqualsSign
			{
				$current = grammarAccess.getEQUALITY_OPAccess().getEQUALSEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getEQUALITY_OPAccess().getEQUALSEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=ExclamationMarkEqualsSign
			{
				$current = grammarAccess.getEQUALITY_OPAccess().getNOT_EQUALSEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getEQUALITY_OPAccess().getNOT_EQUALSEnumLiteralDeclaration_1());
			}
		)
	)
;

// Rule CONTEXT
ruleCONTEXT returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Github
			{
				$current = grammarAccess.getCONTEXTAccess().getGITHUBEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getCONTEXTAccess().getGITHUBEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Env
			{
				$current = grammarAccess.getCONTEXTAccess().getENVEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getCONTEXTAccess().getENVEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=Vars
			{
				$current = grammarAccess.getCONTEXTAccess().getVARSEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getCONTEXTAccess().getVARSEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=Job
			{
				$current = grammarAccess.getCONTEXTAccess().getJOBEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getCONTEXTAccess().getJOBEnumLiteralDeclaration_3());
			}
		)
		    |
		(
			enumLiteral_4=Jobs
			{
				$current = grammarAccess.getCONTEXTAccess().getJOBSEnumLiteralDeclaration_4().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_4, grammarAccess.getCONTEXTAccess().getJOBSEnumLiteralDeclaration_4());
			}
		)
		    |
		(
			enumLiteral_5=Steps
			{
				$current = grammarAccess.getCONTEXTAccess().getSTEPSEnumLiteralDeclaration_5().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_5, grammarAccess.getCONTEXTAccess().getSTEPSEnumLiteralDeclaration_5());
			}
		)
		    |
		(
			enumLiteral_6=Runner
			{
				$current = grammarAccess.getCONTEXTAccess().getRUNNEREnumLiteralDeclaration_6().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_6, grammarAccess.getCONTEXTAccess().getRUNNEREnumLiteralDeclaration_6());
			}
		)
		    |
		(
			enumLiteral_7=Secrets
			{
				$current = grammarAccess.getCONTEXTAccess().getSECRETSEnumLiteralDeclaration_7().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_7, grammarAccess.getCONTEXTAccess().getSECRETSEnumLiteralDeclaration_7());
			}
		)
		    |
		(
			enumLiteral_8=Strategy
			{
				$current = grammarAccess.getCONTEXTAccess().getSTRATEGYEnumLiteralDeclaration_8().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_8, grammarAccess.getCONTEXTAccess().getSTRATEGYEnumLiteralDeclaration_8());
			}
		)
		    |
		(
			enumLiteral_9=Matrix
			{
				$current = grammarAccess.getCONTEXTAccess().getMATRIXEnumLiteralDeclaration_9().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_9, grammarAccess.getCONTEXTAccess().getMATRIXEnumLiteralDeclaration_9());
			}
		)
		    |
		(
			enumLiteral_10=Needs
			{
				$current = grammarAccess.getCONTEXTAccess().getNEEDSEnumLiteralDeclaration_10().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_10, grammarAccess.getCONTEXTAccess().getNEEDSEnumLiteralDeclaration_10());
			}
		)
		    |
		(
			enumLiteral_11=Inputs
			{
				$current = grammarAccess.getCONTEXTAccess().getINPUTSEnumLiteralDeclaration_11().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_11, grammarAccess.getCONTEXTAccess().getINPUTSEnumLiteralDeclaration_11());
			}
		)
	)
;

// Rule TYPE
ruleTYPE returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Boolean
			{
				$current = grammarAccess.getTYPEAccess().getBOOLEANEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getTYPEAccess().getBOOLEANEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Number
			{
				$current = grammarAccess.getTYPEAccess().getNUMBEREnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getTYPEAccess().getNUMBEREnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=String
			{
				$current = grammarAccess.getTYPEAccess().getSTRINGEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getTYPEAccess().getSTRINGEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=Environment
			{
				$current = grammarAccess.getTYPEAccess().getENVIRONMENTEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getTYPEAccess().getENVIRONMENTEnumLiteralDeclaration_3());
			}
		)
		    |
		(
			enumLiteral_4=Choice
			{
				$current = grammarAccess.getTYPEAccess().getCHOICEEnumLiteralDeclaration_4().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_4, grammarAccess.getTYPEAccess().getCHOICEEnumLiteralDeclaration_4());
			}
		)
	)
;

// Rule WEBHOOK_ACTIVITY_TYPE
ruleWEBHOOK_ACTIVITY_TYPE returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Assigned
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getASSIGNEDEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getASSIGNEDEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Auto_merge_disabled
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getAUTO_MERGE_DISABLEDEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getAUTO_MERGE_DISABLEDEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=Auto_merge_enabled
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getAUTO_MERGE_ENABLEDEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getAUTO_MERGE_ENABLEDEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=Closed
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCLOSEDEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCLOSEDEnumLiteralDeclaration_3());
			}
		)
		    |
		(
			enumLiteral_4=Converted_to_draft
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCONVERTED_TO_DRAFTEnumLiteralDeclaration_4().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_4, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCONVERTED_TO_DRAFTEnumLiteralDeclaration_4());
			}
		)
		    |
		(
			enumLiteral_5=Edited
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getEDITEDEnumLiteralDeclaration_5().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_5, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getEDITEDEnumLiteralDeclaration_5());
			}
		)
		    |
		(
			enumLiteral_6=Labeled
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getLABELEDEnumLiteralDeclaration_6().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_6, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getLABELEDEnumLiteralDeclaration_6());
			}
		)
		    |
		(
			enumLiteral_7=Locked
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getLOCKEDEnumLiteralDeclaration_7().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_7, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getLOCKEDEnumLiteralDeclaration_7());
			}
		)
		    |
		(
			enumLiteral_8=Opened
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getOPENEDEnumLiteralDeclaration_8().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_8, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getOPENEDEnumLiteralDeclaration_8());
			}
		)
		    |
		(
			enumLiteral_9=Ready_for_review
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREADY_FOR_REVIEWEnumLiteralDeclaration_9().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_9, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREADY_FOR_REVIEWEnumLiteralDeclaration_9());
			}
		)
		    |
		(
			enumLiteral_10=Reopened
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREOPENEDEnumLiteralDeclaration_10().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_10, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREOPENEDEnumLiteralDeclaration_10());
			}
		)
		    |
		(
			enumLiteral_11=Review_request_removed
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREVIEW_REQUEST_REMOVEDEnumLiteralDeclaration_11().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_11, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREVIEW_REQUEST_REMOVEDEnumLiteralDeclaration_11());
			}
		)
		    |
		(
			enumLiteral_12=Review_requested
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREVIEW_REQUESTEDEnumLiteralDeclaration_12().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_12, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getREVIEW_REQUESTEDEnumLiteralDeclaration_12());
			}
		)
		    |
		(
			enumLiteral_13=Synchronize
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getSYNCHRONIZEEnumLiteralDeclaration_13().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_13, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getSYNCHRONIZEEnumLiteralDeclaration_13());
			}
		)
		    |
		(
			enumLiteral_14=Unassigned
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNASSIGNEDEnumLiteralDeclaration_14().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_14, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNASSIGNEDEnumLiteralDeclaration_14());
			}
		)
		    |
		(
			enumLiteral_15=Unlabeled
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNLABELEDEnumLiteralDeclaration_15().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_15, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNLABELEDEnumLiteralDeclaration_15());
			}
		)
		    |
		(
			enumLiteral_16=Unlocked
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNLOCKEDEnumLiteralDeclaration_16().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_16, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getUNLOCKEDEnumLiteralDeclaration_16());
			}
		)
		    |
		(
			enumLiteral_17=Created
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCREATEDEnumLiteralDeclaration_17().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_17, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getCREATEDEnumLiteralDeclaration_17());
			}
		)
		    |
		(
			enumLiteral_18=Deleted
			{
				$current = grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getDELETEDEnumLiteralDeclaration_18().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_18, grammarAccess.getWEBHOOK_ACTIVITY_TYPEAccess().getDELETEDEnumLiteralDeclaration_18());
			}
		)
	)
;

// Rule EVENT
ruleEVENT returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Branch_protection_rule
			{
				$current = grammarAccess.getEVENTAccess().getBRANCH_PROTECTION_RULEEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getEVENTAccess().getBRANCH_PROTECTION_RULEEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Check_run
			{
				$current = grammarAccess.getEVENTAccess().getCHECK_RUNEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getEVENTAccess().getCHECK_RUNEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=Check_suite
			{
				$current = grammarAccess.getEVENTAccess().getCHECK_SUITEEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getEVENTAccess().getCHECK_SUITEEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=Create
			{
				$current = grammarAccess.getEVENTAccess().getCREATEEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getEVENTAccess().getCREATEEnumLiteralDeclaration_3());
			}
		)
		    |
		(
			enumLiteral_4=Delete
			{
				$current = grammarAccess.getEVENTAccess().getDELETEEnumLiteralDeclaration_4().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_4, grammarAccess.getEVENTAccess().getDELETEEnumLiteralDeclaration_4());
			}
		)
		    |
		(
			enumLiteral_5=Deployment
			{
				$current = grammarAccess.getEVENTAccess().getDEPLOYMENTEnumLiteralDeclaration_5().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_5, grammarAccess.getEVENTAccess().getDEPLOYMENTEnumLiteralDeclaration_5());
			}
		)
		    |
		(
			enumLiteral_6=Deployment_status
			{
				$current = grammarAccess.getEVENTAccess().getDEPLOYMENT_STATUSEnumLiteralDeclaration_6().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_6, grammarAccess.getEVENTAccess().getDEPLOYMENT_STATUSEnumLiteralDeclaration_6());
			}
		)
		    |
		(
			enumLiteral_7=Discussion
			{
				$current = grammarAccess.getEVENTAccess().getDISCUSSIONEnumLiteralDeclaration_7().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_7, grammarAccess.getEVENTAccess().getDISCUSSIONEnumLiteralDeclaration_7());
			}
		)
		    |
		(
			enumLiteral_8=Fork
			{
				$current = grammarAccess.getEVENTAccess().getFORKEnumLiteralDeclaration_8().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_8, grammarAccess.getEVENTAccess().getFORKEnumLiteralDeclaration_8());
			}
		)
		    |
		(
			enumLiteral_9=Gollum
			{
				$current = grammarAccess.getEVENTAccess().getGOLLUMEnumLiteralDeclaration_9().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_9, grammarAccess.getEVENTAccess().getGOLLUMEnumLiteralDeclaration_9());
			}
		)
		    |
		(
			enumLiteral_10=Issues
			{
				$current = grammarAccess.getEVENTAccess().getISSUESEnumLiteralDeclaration_10().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_10, grammarAccess.getEVENTAccess().getISSUESEnumLiteralDeclaration_10());
			}
		)
		    |
		(
			enumLiteral_11=Label
			{
				$current = grammarAccess.getEVENTAccess().getLABELEnumLiteralDeclaration_11().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_11, grammarAccess.getEVENTAccess().getLABELEnumLiteralDeclaration_11());
			}
		)
		    |
		(
			enumLiteral_12=Merge_group
			{
				$current = grammarAccess.getEVENTAccess().getMERGE_GROUPEnumLiteralDeclaration_12().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_12, grammarAccess.getEVENTAccess().getMERGE_GROUPEnumLiteralDeclaration_12());
			}
		)
		    |
		(
			enumLiteral_13=Milestone
			{
				$current = grammarAccess.getEVENTAccess().getMILESTONEEnumLiteralDeclaration_13().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_13, grammarAccess.getEVENTAccess().getMILESTONEEnumLiteralDeclaration_13());
			}
		)
		    |
		(
			enumLiteral_14=Page_build
			{
				$current = grammarAccess.getEVENTAccess().getPAGE_BUILDEnumLiteralDeclaration_14().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_14, grammarAccess.getEVENTAccess().getPAGE_BUILDEnumLiteralDeclaration_14());
			}
		)
		    |
		(
			enumLiteral_15=Project
			{
				$current = grammarAccess.getEVENTAccess().getPROJECTEnumLiteralDeclaration_15().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_15, grammarAccess.getEVENTAccess().getPROJECTEnumLiteralDeclaration_15());
			}
		)
		    |
		(
			enumLiteral_16=Project_card
			{
				$current = grammarAccess.getEVENTAccess().getPROJECT_CARDEnumLiteralDeclaration_16().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_16, grammarAccess.getEVENTAccess().getPROJECT_CARDEnumLiteralDeclaration_16());
			}
		)
		    |
		(
			enumLiteral_17=Project_column
			{
				$current = grammarAccess.getEVENTAccess().getPROJECT_COLUMNEnumLiteralDeclaration_17().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_17, grammarAccess.getEVENTAccess().getPROJECT_COLUMNEnumLiteralDeclaration_17());
			}
		)
		    |
		(
			enumLiteral_18=Public
			{
				$current = grammarAccess.getEVENTAccess().getPUBLICEnumLiteralDeclaration_18().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_18, grammarAccess.getEVENTAccess().getPUBLICEnumLiteralDeclaration_18());
			}
		)
		    |
		(
			enumLiteral_19=Pull_request_review
			{
				$current = grammarAccess.getEVENTAccess().getPULL_REQUEST_REVIEWEnumLiteralDeclaration_19().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_19, grammarAccess.getEVENTAccess().getPULL_REQUEST_REVIEWEnumLiteralDeclaration_19());
			}
		)
		    |
		(
			enumLiteral_20=Pull_request_review_comment
			{
				$current = grammarAccess.getEVENTAccess().getPULL_REQUEST_REVIEW_COMMENTEnumLiteralDeclaration_20().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_20, grammarAccess.getEVENTAccess().getPULL_REQUEST_REVIEW_COMMENTEnumLiteralDeclaration_20());
			}
		)
		    |
		(
			enumLiteral_21=Registry_package
			{
				$current = grammarAccess.getEVENTAccess().getREGISTRY_PACKAGEEnumLiteralDeclaration_21().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_21, grammarAccess.getEVENTAccess().getREGISTRY_PACKAGEEnumLiteralDeclaration_21());
			}
		)
		    |
		(
			enumLiteral_22=Release
			{
				$current = grammarAccess.getEVENTAccess().getRELEASEEnumLiteralDeclaration_22().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_22, grammarAccess.getEVENTAccess().getRELEASEEnumLiteralDeclaration_22());
			}
		)
		    |
		(
			enumLiteral_23=Repository_dispatch
			{
				$current = grammarAccess.getEVENTAccess().getREPOSITORY_DISPATCHEnumLiteralDeclaration_23().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_23, grammarAccess.getEVENTAccess().getREPOSITORY_DISPATCHEnumLiteralDeclaration_23());
			}
		)
		    |
		(
			enumLiteral_24=Watch
			{
				$current = grammarAccess.getEVENTAccess().getWATCHEnumLiteralDeclaration_24().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_24, grammarAccess.getEVENTAccess().getWATCHEnumLiteralDeclaration_24());
			}
		)
		    |
		(
			enumLiteral_25=Status
			{
				$current = grammarAccess.getEVENTAccess().getSTATUSEnumLiteralDeclaration_25().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_25, grammarAccess.getEVENTAccess().getSTATUSEnumLiteralDeclaration_25());
			}
		)
	)
;

// Rule PERMISSION_SCOPE
rulePERMISSION_SCOPE returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Actions
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getACTIONSEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getPERMISSION_SCOPEAccess().getACTIONSEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Checks
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getCHECKSEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getPERMISSION_SCOPEAccess().getCHECKSEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=Contents
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getCONTENTSEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getPERMISSION_SCOPEAccess().getCONTENTSEnumLiteralDeclaration_2());
			}
		)
		    |
		(
			enumLiteral_3=Deployments
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getDEPLOYMENTSEnumLiteralDeclaration_3().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_3, grammarAccess.getPERMISSION_SCOPEAccess().getDEPLOYMENTSEnumLiteralDeclaration_3());
			}
		)
		    |
		(
			enumLiteral_4=Discussions
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getDISCUSSIONSEnumLiteralDeclaration_4().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_4, grammarAccess.getPERMISSION_SCOPEAccess().getDISCUSSIONSEnumLiteralDeclaration_4());
			}
		)
		    |
		(
			enumLiteral_5=IdToken
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getID_TOKENEnumLiteralDeclaration_5().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_5, grammarAccess.getPERMISSION_SCOPEAccess().getID_TOKENEnumLiteralDeclaration_5());
			}
		)
		    |
		(
			enumLiteral_6=Issues
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getISSUESEnumLiteralDeclaration_6().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_6, grammarAccess.getPERMISSION_SCOPEAccess().getISSUESEnumLiteralDeclaration_6());
			}
		)
		    |
		(
			enumLiteral_7=Packages
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getPACKAGESEnumLiteralDeclaration_7().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_7, grammarAccess.getPERMISSION_SCOPEAccess().getPACKAGESEnumLiteralDeclaration_7());
			}
		)
		    |
		(
			enumLiteral_8=Pages
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getPAGESEnumLiteralDeclaration_8().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_8, grammarAccess.getPERMISSION_SCOPEAccess().getPAGESEnumLiteralDeclaration_8());
			}
		)
		    |
		(
			enumLiteral_9=Pull_requests
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getPULL_REQUESTSEnumLiteralDeclaration_9().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_9, grammarAccess.getPERMISSION_SCOPEAccess().getPULL_REQUESTSEnumLiteralDeclaration_9());
			}
		)
		    |
		(
			enumLiteral_10=Repository_projects
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getREPOSITORY_PROJECTSEnumLiteralDeclaration_10().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_10, grammarAccess.getPERMISSION_SCOPEAccess().getREPOSITORY_PROJECTSEnumLiteralDeclaration_10());
			}
		)
		    |
		(
			enumLiteral_11=Security_events
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getSECURITY_EVENTSEnumLiteralDeclaration_11().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_11, grammarAccess.getPERMISSION_SCOPEAccess().getSECURITY_EVENTSEnumLiteralDeclaration_11());
			}
		)
		    |
		(
			enumLiteral_12=Statuses
			{
				$current = grammarAccess.getPERMISSION_SCOPEAccess().getSTATUSESEnumLiteralDeclaration_12().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_12, grammarAccess.getPERMISSION_SCOPEAccess().getSTATUSESEnumLiteralDeclaration_12());
			}
		)
	)
;

// Rule PERMISSION_VALUE
rulePERMISSION_VALUE returns [Enumerator current=null]
@init {
	enterRule();
}
@after {
	leaveRule();
}:
	(
		(
			enumLiteral_0=Read
			{
				$current = grammarAccess.getPERMISSION_VALUEAccess().getREADEnumLiteralDeclaration_0().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_0, grammarAccess.getPERMISSION_VALUEAccess().getREADEnumLiteralDeclaration_0());
			}
		)
		    |
		(
			enumLiteral_1=Write
			{
				$current = grammarAccess.getPERMISSION_VALUEAccess().getWRITEEnumLiteralDeclaration_1().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_1, grammarAccess.getPERMISSION_VALUEAccess().getWRITEEnumLiteralDeclaration_1());
			}
		)
		    |
		(
			enumLiteral_2=None
			{
				$current = grammarAccess.getPERMISSION_VALUEAccess().getNONEEnumLiteralDeclaration_2().getEnumLiteral().getInstance();
				newLeafNode(enumLiteral_2, grammarAccess.getPERMISSION_VALUEAccess().getNONEEnumLiteralDeclaration_2());
			}
		)
	)
;
